...
spinor sections/connections. The complete geometric Fourier tower, general
metric sector and action--Hessian--mode--boundary-domain agreement remain
explicit inputs.
...
Jacobi and Fredholm realizations. General stratified Stokes and the complete
covariant LL parent remain open.
...
| 2. Fields/induced/gauge | compact effective D8 quotients; smooth/L²/graph-`H¹` fields, unconditional canonical physical trace and Dirichlet Hilbert sector; populated independent/induced package and exact variation chain; global scalar `p=d phi`; intrinsic abelian `U(1)^2` connection one-forms with `A+dλ`, diffeomorphism covariance and nilpotent BRST bridged to both ghosts; concrete ambient Pin⁻/PinC and D9 spinor bundles; diagonal diffeomorphism action; LL worldvolume action with exact PT covariance; null domain `Theta ≠ 0`; integrated scalar stress/holonomic variations and finite periodic Euler models | identification with an intrinsic Sobolev field space, general tensor metrics, nonlinear diffeomorphism/nonabelian/BV BRST, stationarity/stability, full covariant matter PDE/stress conservation and density law |
| 3. Bulk/boundary/worldvolume | typed ledger, exact inverse-compatible GHY curve, Gaussian-normal EH cancellation, finite-box Stokes, null/joint transgression, global finite-measure LL action/variation, zero-flux algebraic branch, differential PT-symmetric LL weak/strong equations, canonical-frame global IPP, Hessian/Jacobi/Fredholm realization and exact PT covariance | arbitrary-coordinate variable-field continuum flux, physical EH/GHY/null/joint completion, general stratified Stokes and the complete covariant LL parent |
