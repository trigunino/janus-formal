...
| **P — variational principle** | `lake build JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple` | **green** | diagonal and positive-diagonalizable roots plus a signature-certified, Sylvester-regular 4D Lorentz-Jordan stratum; scalar static Fredholm and holonomic-frame ellipticity `H¹` bridge; PT-covariant Robin and differential-LL Fredholm; concrete Pin⁻/PinC/D9 spinor geometry, geometric zero/first signed Dirac levels and abstract signed Fredholm completion; linearized BRST, ordinary-ghost no-go and explicit nonzero square-zero graded coefficient witness |
