...
| **D8 — topology and representations** | `lake build JanusFormal.Branches.FundamentalGeometryD8TopologyRepresentation` | **green** | free analytic mapping torus, smooth throat embedding, analytic normal line, exact total differential-normal `Diffeomorph` and zero/nonzero strata, normal principal `Pin⁻(1) ≃ Z4` bundle and associated roots; Lorentzian null/joint strata and ambient Pin/SpinC remain open |
...
| **P — variational principle** | `lake build JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple` | **green** | global diagonal root/Candidate-A variation; deck-invariant Lorentz cocycle/local frames; geometric twisted normal flux; scalar global weak Euler/Jacobi with static-positive Riesz/Fredholm sector; PT-covariant Robin and differential-LL H¹/Riesz/Fredholm realizations; linearized BRST plus ordinary-ghost nonlinear no-go |
