...
| **D9 — immersed SpinC elliptic complex** | `lake build JanusFormal.Branches.FundamentalGeometryD9ImmersedSpinCEllipticComplex` | **green head** | P-independent symbol/linear-BRST closure, typed diagonal P-field projection and local D8-normal-section bridge with exact `-1 = Z4²`; the Program-P facade additionally constructs the actual doubled D9 spinor bundle, sections/connections, geometric Dirac action, zero and first signed levels, their injective finite-circle-mode synthesis in each root sector, and the abstract signed self-adjoint Fredholm completion. Arbitrary sphere levels, the completed geometric Fourier identification, nonlinear ghost-difféo and variable metric/gauge-coupled Dirac family remain open |
