...
| **D8 — topology and representations** | `lake build JanusFormal.Branches.FundamentalGeometryD8TopologyRepresentation` | **green** | free analytic mapping torus, smooth throat embedding, exact differential-normal `Diffeomorph` and strata, real ambient tangent transition/orientation cocycle, normal principal `Pin⁻(1) ≃ Z4` bundle and associated roots; quadratic reduction, Lorentzian null/joint strata and ambient Pin/SpinC remain open |
...
| **P — variational principle** | `lake build JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple` | **green** | diagonal and positive-diagonalizable roots plus a genuine 4D Lorentz-Jordan stratum; deck-invariant Lorentz cocycle/local frames; scalar weak Euler/Jacobi with static-positive Fredholm and uniform-ellipticity-to-`H¹` bridge; PT-covariant Robin and differential-LL H¹/Riesz/Fredholm realizations; linearized BRST plus ordinary-ghost nonlinear no-go |
