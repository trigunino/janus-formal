...
| **Program D — fundamental geometry** | `lake build JanusFormal.Branches.FundamentalGeometryD` | active top-priority branch: global twisted-Hopf candidate, throat monopole, Pin and geometry-to-physics interfaces |
| **Program D2 — twisted Dirac spectral geometry** | `lake build JanusFormal.Branches.FundamentalGeometryDiracSpectral` | primitive monopole spectrum, product pairing, eta/holonomy, parity anomaly and LL spectral lock; corrects the old ratio so `alpha` matches the `S2` radius while `1/(2*sqrt(2))` is the compact-circle 
...
| **Program D8 — topology and representations** | `lake build JanusFormal.Branches.FundamentalGeometryD8TopologyRepresentation` | audits the quotient as a free smooth mapping torus, proves one-sided-throat and cover/quotient algebra, classifies the two quarter-phase orientation lifts, distinguishes order-four holonomy from `pi1`, and proves cyclic holonomy does not select rank five |
| **Program D9 — immersed SpinC elliptic complex** | `lake build JanusFormal.Branches.FundamentalGeometryD9ImmersedSpinCEllipticComplex` | assembles immersion, normal Jacobi, Maxwell/de Rham, metric/ghost and twisted-Dirac principal symbols; global bundles, lower-order operators, BRST/BV and zeta superdeterminant remain open |
| **Program D10 — Quillen anomaly geometry** | `lake build JanusFormal.Branches.FundamentalGeometryD10QuillenAnomaly` | distinguishes determinant lines from the odd-family index gerbe, proves that scalar potentials depend on anomaly trivialization and finite counterterms, and gives the exact conditional closure matrix for a predictive Janus partition function |
