...
| **D8 — topology and representations** | `lake build JanusFormal.Branches.FundamentalGeometryD8TopologyRepresentation` | **green** | free analytic mapping torus, smooth throat embedding, analytic normal line, global algebraic differential-normal comparison, normal principal `Pin⁻(1) ≃ Z4` bundle and its two associated PT-conjugate root-character bundles; smooth differential-normal and ambient Pin/SpinC identification remain open |
...
| **P — variational principle** | `lake build JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple` | **green** | global diagonal root/Candidate-A variation; local-frame replacement for the obstructed global frame; geometric twisted scalar normal flux; completed `L²`/graph-`H¹`; PT-covariant Robin and differential-LL H¹/Riesz/Fredholm realizations; abelian plus tangent-diffeomorphism linearized BRST |
