...
| **D8 — topology and representations** | `lake build JanusFormal.Branches.FundamentalGeometryD8TopologyRepresentation` | **green** | free analytic mapping torus, global smooth one-sided throat embedding, analytic rank-one normal `VectorBundle` with one-loop `-id`, and quarter-lift audits; smooth global normal identification remains open |
...
| **P — variational principle** | `lake build JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple` | **green** | causally compatible global diagonal root/domain/frontier with controlled one-sided limits and exact Minkowski overlap, smooth D8 Candidate-A/induced-field variation, completed `L²` plus finite-frame graph-`H¹` and throat-supported norm-one trace, fixed-frame holonomic scalar action/variation, diagonal diffeomorphisms, LL throat action/variation with algebraic zero-flux branch, and null admissible domain |
