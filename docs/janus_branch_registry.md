...
| **P — variational principle** | `lake build JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple` | **green** | global diagonal root/Candidate-A variation, completed `L²` and graph-`H¹`, fixed-frame holonomic scalar action, diagonal diffeomorphisms, intrinsic abelian `U(1)^2` BRST, and LL action/variation with algebraic zero-flux branch plus PT covariance |
