...
| **Program D2 — twisted Dirac spectral geometry** | `lake build JanusFormal.Branches.FundamentalGeometryDiracSpectral` | primitive monopole spectrum, product pairing, eta/holonomy, parity anomaly and LL spectral lock; corrects the old ratio so `alpha` matches the `S2` radius while `1/(2*sqrt(2))` is the compact-circle ratio |
