...
| Non-throat PT transitions | `lake build JanusFormal.Branches.JanusNonThroatPTTransitions` | bottom frontier reached: `g(+)` Weyl kinematics, `S4/RP4` conformal background, plus-sector source contract, and 00 projection are closed; solving `Omega` still needs `L`, Lorentzian time, pre-drag source scalings, and boundary data |
...
| Candidate mechanism matrix | `lake build JanusFormal.Branches.CandidateMechanismMatrix` | matrix of exits; no closed state law |
