...
| **D8 — topology and representations** | `lake build JanusFormal.Branches.FundamentalGeometryD8TopologyRepresentation` | **green** | free analytic mapping torus, one-sided throat, analytic rank-one normal `VectorBundle` with one-loop `-id`, and quarter-lift audits; `IsSmoothEmbedding` and smooth global normal identification remain open |
| **D9 — immersed SpinC elliptic complex** | `lake build JanusFormal.Branches.FundamentalGeometryD9ImmersedSpinCEllipticComplex` | **green head** | P-independent symbol/linear-BRST closure plus typed diagonal P-field projection; normal/SpinC/general-metric completion remains open |
...
| **P — variational principle** | `lake build JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple` | **green** | causally compatible global diagonal root/domain/frontier with controlled one-sided limits and exact Minkowski overlap, smooth D8 Candidate-A/induced-field variation, completed `L²` plus finite-frame graph-`H¹` and throat-supported norm-one trace, fixed-frame holonomic scalar action/variation, diagonal diffeomorphisms, LL throat action and null admissible domain |
