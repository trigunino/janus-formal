...
| **D8 — topology and representations** | `lake build JanusFormal.Branches.FundamentalGeometryD8TopologyRepresentation` | **green** | free analytic mapping torus, smooth throat embedding, analytic normal line, global algebraic differential-normal comparison, two global smooth-underlier complex `Z4` root lines, exact PT-conjugation exchange, and the normal principal `Pin⁻(1) ≃ Z4` bundle; smooth differential-normal and ambient Pin/SpinC identification remain open |
...
| **P — variational principle** | `lake build JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple` | **green** | global diagonal root/Candidate-A variation; regular general-Lorentz scalar action/variation; completed `L²` and graph-`H¹`; PT-covariant Robin weak action/Hessian/Fredholm operator; abelian plus tangent-diffeomorphism linearized BRST; PT-symmetric differential LL weak Euler/Jacobi dynamics |
