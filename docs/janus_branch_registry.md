...
| **D8 — topology and representations** | `lake build JanusFormal.Branches.FundamentalGeometryD8TopologyRepresentation` | **green** | free analytic mapping torus, exact differential-normal `Diffeomorph` and strata, real ambient tangent orientation cocycle, positive nondegenerate model quadratic form and transported transition isometries, normal principal `Pin⁻(1) ≃ Z4` bundle; global orthonormal compatibility, Lorentzian null/joint strata and ambient Pin/SpinC remain open |
...
| **P — variational principle** | `lake build JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple` | **green** | diagonal and positive-diagonalizable roots plus a signature-certified, Sylvester-regular 4D Lorentz-Jordan stratum; scalar weak Euler/Jacobi with static-positive Fredholm and holonomic-frame-to-uniform-ellipticity `H¹` bridge; PT-covariant Robin and differential-LL H¹/Riesz/Fredholm; linearized BRST plus ordinary-ghost nonlinear no-go |
