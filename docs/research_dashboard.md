...
| --- | --- | --- | --- |
| **D0/D8** | global geometry and normal lift | analytic compact mapping-torus quotients, exact differential-normal `Diffeomorph` and strata, ambient tangent orientation/quadratic transition data, genuine Clifford Spin action, explicit Spin lifts of every finite even unit-reflection factorization, normal principal `Pin⁻(1) ≃ Z4` bundle | prove Cartan--Dieudonné with determinant parity, regularize the resulting atlas lifts and trivialize their global Čech defect, compare the normal lift, then extend to SpinC |
...
| P-D low-rank pairings/fusion | focused Lean head, global normal `Z4` root lines, abelian `U(1)^2` BRST, a true tangent-ghost Lie algebra, three-generator exterior scalar closure and the closed-triple global Koszul extension theorem are green | instantiate the nonabelian D8 triple; global SpinC and full-field/BV classification remain open |
...
1. Close Cartan--Dieudonné with determinant parity for the ambient `SO(4)`, then regularize the now-explicit even-reflection Spin lifts into atlas-wide Pin/SpinC transition data; the rank-one normal principal lift is already constructed.
