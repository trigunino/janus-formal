...
| **D9/D11** | elliptic and natural-operator gates | green D9 symbol/BRST and D11 naturality/jet heads; true tangent ghost with linearized BRST, genuine three-generator exterior coefficients coupled to the global D8 ghost Lie algebra, vanishing total cubic/scalar obstruction, and a global square-zero Koszul differential on the full exterior-scalar algebra for any supplied closed ghost triple | instantiate a nonabelian closed D8 ghost triple, extend the differential to every field/antifield, prove BV/SpinC completion and construct a concrete Janus Fredholm family |
...
| **P0/P-A** | reject shortcuts and define relative selection | all four positive-root characteristic coefficients are continuous without an eigenbasis choice; one signature-certified Sylvester-regular Lorentz-Jordan stratum; canonical global intrinsic `(3,1)` metric with an unconditional finite nonzero quotient-volume/action; physical-volume graph `H¹` with trace reduced to one codimension-one inequality; a nontrivial complete analytic D8 time flow acts on the full current independent-field package; global scalar weak Euler/Jacobi and static-positive index-zero Fredholm/H¹ bridge | finish the matrix-selector continuity/regularity bridge and physical trace inequality, identify intrinsic Sobolev sections, classify/glue all Jordan strata, then nonlinear/BV and boundary stability |
...
| P-A relative uniqueness | yes; finite tangent generators and first-jet graph `H¹`, unconditional continuity of all four positive-root coefficients, one non-diagonalizable Lorentz-Jordan family, a canonical intrinsic global `(3,1)` metric, an unconditional finite nonzero quotient-volume/action, a complete analytic D8 time flow acting on all current independent fields, global Candidate-A plus induced-field variation, and a static scalar ellipticity bridge | finish global root-selector regularity, then physical-volume trace inequality, Jordan classification/gluing, parent action, physical boundary flux and curved global PDE |
...
| P-D low-rank pairings/fusion | focused Lean head, global normal `Z4` root lines, abelian `U(1)^2` BRST, a true tangent-ghost Lie algebra, three-generator exterior scalar closure, closed-triple global Koszul extension, and three explicit faithful deck-equivariant spatial `so(3)` cover sections are green | prove bracket naturality under quotient projection to close the nonabelian D8 triple; global SpinC and full-field/BV classification remain open |
