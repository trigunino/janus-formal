...
| --- | --- | --- | --- |
| **D0/D8** | global geometry and normal lift | analytic compact mapping-torus quotients, global smooth throat embedding, sign-clutched analytic normal line, exact total differential-normal `Diffeomorph` with closed zero/open nonzero strata, normal principal `Pin⁻(1) ≃ Z4` bundle and associated roots | supply the missing Lorentzian quadratic data for null/non-null/joint strata and construct the ambient tangent Pin/SpinC principal bundles |
...
| **P0/P-A** | reject shortcuts and define relative selection | diagonal Candidate-A domain/variation; deck-invariant Lorentz cocycle plus local frames; geometric twisted normal flux; global scalar weak Euler/Jacobi and static-positive self-adjoint index-zero Fredholm realization; linearized BRST; PT-covariant Robin and differential-LL H¹/Riesz/Fredholm realizations | descend the global general Lorentz tensor/action, intrinsic physical-volume Sobolev trace, nonlinear/BV covariance, Lorentzian strong formula via volume/Stokes, Israel/null junctions, full Candidate-A Hessian and constrained stability |
