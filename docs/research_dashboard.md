...
| --- | --- | --- | --- |
| **D0/D8** | global geometry and normal lift | analytic compact mapping-torus quotients, global smooth throat embedding, exact total differential-normal `Diffeomorph` with closed zero/open nonzero strata, real ambient tangent transition/orientation cocycle, normal principal `Pin⁻(1) ≃ Z4` bundle and associated roots | derive the quadratic/orthonormal reduction, compare ambient and normal orientation cocycles, then construct the ambient tangent Pin/SpinC principal bundles |
...
| **P0/P-A** | reject shortcuts and define relative selection | diagonal/positive-diagonalizable roots and one genuine 4D Lorentz-Jordan stratum; deck-invariant Lorentz cocycle plus local frames; global scalar weak Euler/Jacobi, static-positive index-zero Fredholm realization and uniform-ellipticity-to-`H¹` bridge; linearized BRST; PT-covariant Robin and differential-LL H¹/Riesz/Fredholm realizations | classify/glue all admissible Lorentz root strata, descend the general tensor/action, derive quantitative frame control and intrinsic physical-volume Sobolev trace, then nonlinear/BV and boundary stability |
...
| P-A relative uniqueness | yes; PT-matched smooth D8 fields, finite tangent generators and first-jet graph `H¹`, causally compatible global diagonal root/domain/frontier, positive-diagonalizable roots, one non-diagonalizable 4D Lorentz-Jordan family, global Candidate-A plus induced-field variation, and a static scalar uniform-ellipticity bridge to the `H¹` graph | quantitative holonomic frame control, intrinsic physical-volume Sobolev trace, general tensorial root/action and Jordan classification/gluing, parent action, physical boundary flux and curved global PDE not derived |
