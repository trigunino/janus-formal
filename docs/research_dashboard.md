...
| --- | --- | --- | --- |
| **D0/D8** | global geometry and normal lift | the effective spacetime and throat quotients carry analytic manifold atlases, analytic PT diffeomorphisms, compactness, a global smooth throat embedding with rank-one normal complement, plus a genuine analytic rank-one normal `VectorBundle` whose one-loop transition is `-id` and whose fibers are pointwise linearly equivalent to the differential normal quotients | assemble the pointwise normal equivalences smoothly, construct the non-null/null/joint strata and global Pin/root bundle |
...
| **D9/D11** | elliptic and natural-operator gates | green D9 symbol/BRST and D11 naturality/jet heads; typed P projection for diagonal metric, tangential gauge, U(1) ghost and matter; local smooth D8-normal-section bridge with exact `-1 = Z4²` transition; proved non-surjectivity toward general D9 metric perturbations | diffeomorphism-ghost/SpinC completion, canonical global normal description and concrete Janus Fredholm family |
...
1. Complete the smooth normal-root/SpinC decoration of the constructed mapping torus and throat embedding.
