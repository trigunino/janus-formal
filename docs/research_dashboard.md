...
| --- | --- | --- | --- |
| **D0/D8** | global geometry and normal lift | analytic compact mapping-torus quotients, global smooth throat embedding, sign-clutched analytic normal line, global algebraic comparison, normal principal `Pin⁻(1) ≃ Z4` bundle and two associated root-character bundles with exact square/PT laws | construct the differential-normal quotient atlas and smooth comparison, the non-null/null/joint strata and the ambient tangent Pin/SpinC principal bundles |
...
| **P0/P-A** | reject shortcuts and define relative selection | diagonal Candidate-A domain/variation; local frames and partition replacing the obstructed global frame; geometric twisted normal flux; smooth/L²/H¹/PT/trace spaces; linearized BRST; PT-covariant Robin and differential-LL H¹/Riesz/self-adjoint index-zero Fredholm realizations | glue a true global general Lorentz tensor/action, intrinsic physical-volume Sobolev trace, nonlinear/BV covariance, differential strong formula via volume/Stokes, Israel/null junctions, full Candidate-A Hessian and constrained stability |
