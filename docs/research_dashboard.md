...
| **P0/P-A** | reject shortcuts and define relative selection | exact Sylvester-regular roots now exist unconditionally for every raw `4×4` real matrix with split strictly-positive charpoly; fixed-similarity and double-`J₂` zero collisions have exact divergent roots and independent collapsing Sylvester modes; the canonical intrinsic `(3,1)` metric has finite nonzero quotient action, unconditional smooth pullback by every D8 self-diffeomorphism, an integrated general-metric scalar action, finite covariance and Noether pairing, weak covariant Euler/flux decomposition, concrete Dirichlet flux cancellation, measured diffeomorphism/exchange covariance of scalar stress, and genuine IPP on the canonical latitude collar; physical-volume graph `H¹`, coarea, canonical trace/operator/Dirichlet/Hilbert renorming, complete D8 time action, global scalar weak Euler/Jacobi and static index-zero Fredholm bridges are green | extend collar IPP to global manifold Stokes and identify the abstract boundary functional, then construct the local Noether current; prove the single local normal-projection regularity lemma, identify intrinsic Sobolev sections, close moving-similarity/Jordan-type and nonpositive/complex residuals, select the physical spectral domain, then general-metric BV, boundary stability and the curved global PDE |
...
| **P-D** | invariant pairings | green low-rank pairing, spinor, fusion and `Z4` neutrality head; fixed-background D8 self-diffeomorphism category is exact | extend from the fixed background to the global moduli symmetry category, multiplicity spaces and normalizations |
| **P-E** | finite jets/equivariance | green corrected jet-universality head and contravariant smooth-field functor on the fixed D8 category | concrete global Janus jet group/bundles and elliptic classification |
...
| P-A relative uniqueness | yes; finite tangent generators and first-jet graph `H¹`, unconditional canonical trace/Dirichlet/Hilbert renorming, raw split-positive `4×4` Sylvester-regular root selection, fixed-similarity and double-Jordan zero-frontier obstructions, canonical intrinsic global `(3,1)` metric, finite nonzero quotient action, unconditional arbitrary-diffeomorphism metric pullback, integrated scalar/Noether covariance, weak covariant Euler/flux, concrete Dirichlet cancellation, canonical-collar IPP and measured stress covariance, complete D8 time flow, Candidate-A induced variation and static scalar ellipticity bridge | globalize collar IPP to manifold Stokes and identify the abstract boundary functional, then construct the local Noether current; prove normal-projection regularity, identify intrinsic Sobolev sections, then moving-similarity/nonpositive classification, parent action, general-metric/nonlocal BV and the curved global PDE |
