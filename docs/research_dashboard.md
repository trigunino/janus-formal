...
| --- | --- | --- | --- |
| **D0/D8** | global geometry and normal lift | analytic compact mapping-torus quotients, exact differential-normal `Diffeomorph`, smooth bundle atlas and causal/joint stratification from any continuous intrinsic normal quadratic form; global product derivative equivalence with exact canonical latitude product/ambient image `(e₀, 0)`, intrinsic cover Lorentz square `1`, fixed-throat orthogonality and spacelike/non-null classification, plus an explicit canonical local quotient-normal lift whose one-deck sign law and quadratic-square invariance are exact; its splitting proves no tangential radical and a nondegenerate smooth intrinsic throat metric; ambient tangent orientation/quadratic transition data, genuine Clifford Spin action, unconditional `Spin(4) → SO(4)` surjectivity/lifting function, and exact reduction of global lifting to one kernel-valued Čech class; normal principal `Pin⁻(1) ≃ Z4` bundle | glue the local normal lift continuously into a global section; classify arbitrary general-metric throat restrictions; prove global orientation, trivialize the Čech class and regularize the lifts, then extend to SpinC |
...
| **P0/P-A** | reject shortcuts and define relative selection | exact Sylvester-regular roots now exist unconditionally for every raw `4×4` real matrix with split strictly-positive charpoly; a proof-carrying retained frontier certificate assembles the diagonal, fixed-similarity, double-`J₂`, explicit moving-shear, singular-diagonal and two explicit Jordan-type-change witnesses; the canonical intrinsic `(3,1)` metric has finite nonzero quotient action, unconditional smooth pullback by every D8 self-diffeomorphism, an integrated general-metric scalar action, finite covariance and Noether pairing, weak covariant Euler/flux decomposition and scalar-stress covariance; the general-Lorentz packet has pointwise PT/exchange-natural throat traces, invariant nondegenerate/no-radical domain and functional metric-reference matching that transports full Dirichlet equality; the intrinsic metric lies in that domain; collar IPP has exact intrinsic oriented flux; the local normal lift has exact deck sign and square invariance; a first-level general-tensor BV doublet and odd bracket are explicit; physical-volume graph `H¹`, coarea, canonical trace/Dirichlet/Hilbert renorming, complete time action and static scalar Fredholm bridges are green | globalize collar Stokes/Noether and stress divergence; glue the local normal lift continuously, identify intrinsic Sobolev sections, classify arbitrary matrix paths/Jordan/nonpositive domains and select the physical spectral domain; construct smooth arbitrary-throat-tensor PT pullback/classify general metric restrictions; prove Lorentz preservation and functional CME for general-tensor BV, antifield/full boundary stability and the curved global PDE |
...
| P-A relative uniqueness | yes; finite tangent generators and graph `H¹`, unconditional trace/Dirichlet/Hilbert renorming, split-positive `4×4` root selection and positive-locus IFT atlas, explicit Jordan/frontier witnesses, canonical intrinsic `(3,1)` metric and action, general-metric scalar/Noether covariance, collar IPP/flux/current/stress identities, pointwise PT/exchange-natural metric throat traces with invariant nondegenerate/no-radical domain and functional Dirichlet-reference matching, a nondegenerate intrinsic throat instance, exact deck sign/square laws for the local normal lift, and scalar Fredholm bridges | extend roots beyond the positive real-diagonalizable locus; globalize collar Stokes/Noether and stress divergence; glue the local normal projection continuously, classify arbitrary general-metric throat restrictions and construct smooth arbitrary-tensor throat PT pullback, identify intrinsic Sobolev sections, then general-tensor functional BV and the curved global PDE |
