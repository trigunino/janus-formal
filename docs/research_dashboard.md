...
| **P0/P-A** | reject shortcuts and define relative selection | diagonal Candidate-A domain/variation; frame/diffeomorphism-covariant general Lorentz scalar density and integrated regular-field action; smooth/L²/H¹/PT/trace spaces; abelian and tangent-diffeomorphism linearized BRST; PT-covariant Robin balance/Hessian/index-zero `L²` Fredholm realization; PT-symmetric differential LL action with weak Euler/Jacobi | construct a global regular general-metric witness from the diagonal branch, intrinsic physical-volume Sobolev trace, nonlinear/BV covariance, strong curved Euler--flux/LL equations, geometric Israel/null junctions, full Candidate-A Hessian and constrained stability |
