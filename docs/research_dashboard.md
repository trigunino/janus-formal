...
| **P0/P-A** | reject shortcuts and define relative selection | exact Sylvester-regular roots now exist unconditionally for every raw `4×4` real matrix with split strictly-positive charpoly: explicit Taylor/Hermite/Lagrange cubics close all five multiplicity profiles, Cayley--Hamilton selects one exhaustively, and a basis-free polynomial-centralizer argument proves Sylvester bijectivity; the non-Hermitian `Fin 4` Jordan-chain presentation remains only when an explicit Jordan form is required; the canonical intrinsic `(3,1)` metric has finite nonzero quotient action, unconditional smooth analytic-PT pullback, tied musical equivalence and an integrated general-metric scalar action and fixed-metric first variation with exact PT/integrability transport; physical-volume graph `H¹`, coarea, canonical trace/operator/Dirichlet/Hilbert renorming, complete D8 time action, global scalar weak Euler/Jacobi and static index-zero Fredholm bridges are green | identify intrinsic Sobolev sections, close nonpositive/complex raw residuals, select the physical spectral domain, then general-metric BV, boundary stability and the curved global PDE; construct non-Hermitian Jordan chains only if an explicit normal form is needed |
...
| P-D low-rank pairings/fusion | focused Lean head, global normal `Z4` root lines, abelian `U(1)^2` BRST, unconditional quotient/throat `so(3)` Koszul and LL closure, positive diagonal throat-metric BRST, and a finite `32`-dimensional master BV model promoted both to smooth real-throat fields and smooth constant-fibre spacetime fields with exact first variation, ultralocal odd functional antibracket, integrated CME and unconditional PT covariance of represented values, variations and brackets on both bases | nonlocal/completed BV, general spacetime tensor-metric action and global SpinC |
