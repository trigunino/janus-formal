...
| **P0/P-A** | reject shortcuts and define relative selection | causally compatible global diagonal Candidate-A domain and variation; smooth/L²/H¹/PT/trace spaces; intrinsic abelian `U(1)^2` connections with diffeomorphism-covariant nilpotent BRST; fixed-frame global scalar action/variation; diagonal diffeomorphism action; LL throat action with finite-measure variation, algebraic zero-flux branch and exact PT covariance | intrinsic Sobolev/physical-volume trace, general tensorial Lorentz and diffeomorphism/nonabelian/BV covariance, full curved Euler--flux and differential LL PDEs, same-action Hessian and constrained stability |
...
| P-D low-rank pairings/fusion | focused Lean head, global normal `Z4` root lines and abelian `U(1)^2` BRST are green | global SpinC and diffeomorphism/nonabelian/BV classification open |
