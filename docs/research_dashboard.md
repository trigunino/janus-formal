...
| **P0/P-A** | reject shortcuts and define relative selection | exact Sylvester-regular roots now exist unconditionally for every raw `4×4` real matrix with split strictly-positive charpoly: explicit Taylor/Hermite/Lagrange cubics close all five multiplicity profiles, Cayley--Hamilton selects one exhaustively, and a basis-free polynomial-centralizer argument proves Sylvester bijectivity; the non-Hermitian `Fin 4` Jordan-chain presentation remains only when an explicit Jordan form is required; the canonical intrinsic `(3,1)` metric has finite nonzero quotient action, unconditional smooth analytic-PT pullback, tied musical equivalence, an integrated general-metric scalar action and fixed-metric first variation with exact PT/integrability transport, finite arbitrary-diffeomorphism covariance under the exact metric-pullback contract, and a zero integrated Noether pairing on supplied smooth orbits under the exact first-variation contract; physical-volume graph `H¹`, coarea, canonical trace/operator/Dirichlet/Hilbert renorming, complete D8 time action, global scalar weak Euler/Jacobi and static index-zero Fredholm bridges are green | construct the unconditional arbitrary metric pullback and local current/divergence law with IPP, flux and enough test ghosts, identify intrinsic Sobolev sections, close nonpositive/complex raw residuals, select the physical spectral domain, then general-metric BV, boundary stability and the curved global PDE; construct non-Hermitian Jordan chains only if an explicit normal form is needed |
...
| P-A relative uniqueness | yes; finite tangent generators and first-jet graph `H¹`, unconditional canonical trace/Dirichlet/Hilbert renorming, raw split-positive `4×4` Sylvester-regular root selection, continuous/IFT-stable diagonal root with inverse-Sylvester derivative, canonical intrinsic global `(3,1)` metric, finite nonzero quotient action, smooth analytic-PT pullback with integrated general-metric scalar and fixed-metric first-variation covariance, arbitrary finite-diffeomorphism covariance under the metric-pullback contract, and an integrated Noether-orbit identity under the exact first-variation contract, complete D8 time flow, Candidate-A induced variation and static scalar ellipticity bridge | construct the unconditional arbitrary metric pullback and local current/divergence law with IPP, flux and enough test ghosts, identify intrinsic Sobolev sections, then nonpositive classification, parent action, physical boundary flux, general-metric/nonlocal BV and the curved global PDE; raw Jordan presentation is optional unless an explicit normal form is required |
