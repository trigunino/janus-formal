...
| --- | --- | --- | --- |
| **D0/D8** | global geometry and normal lift | analytic compact mapping-torus quotients, exact differential-normal `Diffeomorph`, smooth bundle atlas and causal/joint stratification from any continuous intrinsic normal quadratic form; global product derivative equivalence with exact canonical latitude product/ambient image `(e₀, 0)` and algebraic nonvanishing; ambient tangent orientation/quadratic transition data, genuine Clifford Spin action, unconditional `Spin(4) → SO(4)` surjectivity/lifting function, and exact reduction of global lifting to one kernel-valued Čech class; normal principal `Pin⁻(1) ≃ Z4` bundle | prove intrinsic Lorentz orthogonality and causal null/non-null classification of the latitude normal, then the local regular projection lemma; derive the normal quadratic form from the restricted spacetime metric, prove global orientation, trivialize the Čech class and regularize the lifts, then extend to SpinC |
...
| **P0/P-A** | reject shortcuts and define relative selection | exact Sylvester-regular roots now exist unconditionally for every raw `4×4` real matrix with split strictly-positive charpoly; fixed-similarity, double-`J₂`, explicit moving-shear and singular-diagonal zero collisions are exact, the last one regularizing the root while its inverse blows up and Sylvester degenerates; two explicit Jordan-type-change paths distinguish a regular affine root with Sylvester eigenvalue `2` from a zero-frontier root with collapsing Sylvester eigenvalue; the canonical intrinsic `(3,1)` metric has finite nonzero quotient action, unconditional smooth pullback by every D8 self-diffeomorphism, an integrated general-metric scalar action, finite covariance and Noether pairing, weak covariant Euler/flux decomposition, concrete Dirichlet flux cancellation, measured diffeomorphism/exchange covariance of scalar stress, genuine IPP and an exactly conserved equal-mass Green--Wronskian current on the canonical latitude collar; physical-volume graph `H¹`, coarea, canonical trace/operator/Dirichlet/Hilbert renorming, complete D8 time action, global scalar weak Euler/Jacobi and static index-zero Fredholm bridges are green | extend collar IPP to global manifold Stokes, identify the abstract boundary functional and promote the collar current to a covariant local Noether current; prove the single local normal-projection regularity lemma, identify intrinsic Sobolev sections, then arbitrary singular-frame/general Jordan-type classification and nonpositive/complex residuals, select the physical spectral domain, then general-metric BV, boundary stability and the curved global PDE |
...
| P-A relative uniqueness | yes; finite tangent generators and first-jet graph `H¹`, unconditional canonical trace/Dirichlet/Hilbert renorming, raw split-positive `4×4` Sylvester-regular root selection, fixed-similarity, double-Jordan, explicit moving-shear/singular-diagonal zero-frontier control and two explicit Jordan-type-change paths, canonical intrinsic global `(3,1)` metric, finite nonzero quotient action, unconditional arbitrary-diffeomorphism metric pullback, integrated scalar/Noether covariance, weak covariant Euler/flux, concrete Dirichlet cancellation, canonical-collar IPP, conserved equal-mass collar Green current and measured stress covariance, complete D8 time flow, Candidate-A induced variation and static scalar ellipticity bridge | globalize collar IPP to manifold Stokes, identify the abstract boundary functional and promote the collar current to a covariant local Noether current; prove normal-projection regularity, identify intrinsic Sobolev sections, then arbitrary singular-frame/general Jordan-type and nonpositive classification, parent action, general-metric/nonlocal BV and the curved global PDE |
