...
| **D9/D11** | elliptic and natural-operator gates | green D9 symbol/BRST and D11 naturality/jet heads; unconditional quotient/throat `so(3)` Koszul and LL completion; finite `32`-dimensional BV master model promoted to smooth throat and true-spacetime fields, then coupled to the actual smooth positive diagonal/log metric cone with ghosts, antifields, nonzero action, pointwise/integrated CME and unconditional PT covariance; the general-tensor BV level now has an odd square-zero doublet, a genuine bulk ultralocal master with exact affine derivative, action-`4` witness, PT covariance and pointwise/`L¹`-conditional integrated CME, plus smooth PT-matched throat traces whose retained intrinsic nondegenerate metric supplies the analogous inverse pairing/bracket and boundary CME | discharge the smooth inverse-field/continuity contracts, prove Lorentz-domain preservation, arbitrary general-throat inversion/classification and the general functional/derivative-dependent tensor BV CME, then arbitrary nonlocal/completed functionals, SpinC and a concrete Janus Fredholm family |
...
| **P0/P-A** | reject shortcuts and define relative selection | split-positive raw `4×4` root selection with Jordan/singular witnesses; the intrinsic `(3,1)` metric has nonzero action and is PT fixed together with its equal-sector pair and nondegenerate throat trace; the general-tensor bulk BV master now has an exact affine derivative, action-`4` witness, PT covariance and pointwise/`L¹`-conditional integrated CME, while the retained trace has the analogous inverse pairing/bracket and boundary CME; scalar stress has arbitrary-coordinate jet conservation and, on every supplied holonomic quotient patch, a smooth Levi-Civita realization plus `C∞` scalar gradient/Hessians, covariant jet, Euler residual and realized divergence satisfying exactly `div T = E·raisedGradient`; the general-Lorentz packet has PT-natural nondegenerate traces and functional Dirichlet matching; collar IPP/flux and all-winding local-normal curve/tangent/quadratic/scalar-cocycle deck laws are green, as are physical trace/Hilbert and scalar Fredholm bridges | construct the holonomic patches generically, prove overlap compatibility and glue their smooth connection/scalar-jet data, then prove global `div_g T = 0`; globalize collar Stokes/Noether; descend the normal scalar cocycle to its quotient class/lift and glue globally; identify intrinsic Sobolev sections, classify Jordan/nonpositive/root and general throat-metric domains; discharge the smooth inverse-field contracts and prove general functional/derivative-dependent/nonlocal tensor BV and the curved global PDE |
...
| P-A relative uniqueness | yes; graph `H¹`/trace/Hilbert, split-positive root IFT atlas and Jordan witnesses, PT-fixed intrinsic metric/action and nondegenerate throat trace, arbitrary-coordinate scalar-stress conservation with a smooth Levi-Civita patch realization, actual `C∞` scalar/covariant jets and the exact local identity `div T = E·raisedGradient`, functional metric Dirichlet matching, all-winding local-normal curve/tangent/quadratic/scalar-cocycle deck laws, and scalar Fredholm bridges | extend roots beyond the positive locus; construct the holonomic patches generically, prove overlap compatibility and glue their local connection/scalar-jet data into smooth global fields, then prove global `div_g T = 0` plus collar Stokes/Noether; descend the normal scalar cocycle to the quotient class/lift and glue globally, classify general throat metrics, identify intrinsic Sobolev sections, then functional tensor BV and the curved global PDE |
...
| P-D low-rank pairings/fusion | focused Lean head, global normal `Z4` root lines, abelian `U(1)^2` BRST, unconditional quotient/throat `so(3)` Koszul and LL closure, a finite `32`-dimensional master BV model promoted to smooth throat/spacetime and positive diagonal/log metrics, plus a general symmetric-tensor doublet whose bulk ultralocal action has exact pointwise and `L¹`-conditional integrated affine `HasDerivAt`, action-`4` witness, generation of `(h⁺,0)`, PT-covariant represented bracket and pointwise/integrated CME; its smooth BRST-compatible throat trace has the corresponding intrinsic inverse pairing/bracket, action-`3` witness and boundary CME | smooth inverse-field/continuity contracts, Lorentz preservation, arbitrary general-throat inversion/classification, general functional antibracket/CME, derivative/nonlocal/completed functionals and global SpinC |
...
Mechanical count: **494 closed / 619 total; 125 open**.
