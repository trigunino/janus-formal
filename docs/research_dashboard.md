...
| --- | --- | --- | --- |
| **D0/D8** | global geometry and normal lift | analytic compact mapping-torus quotients, exact differential-normal `Diffeomorph`, smooth bundle atlas and causal/joint stratification from any continuous intrinsic normal quadratic form; ambient tangent orientation/quadratic transition data, genuine Clifford Spin action, unconditional `Spin(4) → SO(4)` surjectivity/lifting function, and exact reduction of global lifting to one kernel-valued Čech class; normal principal `Pin⁻(1) ≃ Z4` bundle | derive the normal quadratic form from the restricted spacetime metric, prove global orientation, trivialize the Čech class and regularize the lifts, then extend to SpinC |
...
| **P0/P-A** | reject shortcuts and define relative selection | exact Sylvester-regular roots now exist unconditionally for every raw `4×4` real matrix with split strictly-positive charpoly: explicit Taylor/Hermite/Lagrange cubics close all five multiplicity profiles, Cayley--Hamilton selects one exhaustively, and a basis-free polynomial-centralizer argument proves Sylvester bijectivity; the non-Hermitian `Fin 4` Jordan-chain presentation remains only when an explicit Jordan form is required; the canonical intrinsic `(3,1)` metric has finite nonzero quotient action, unconditional smooth pullback by every D8 self-diffeomorphism with transported musical equivalence, an integrated general-metric scalar action and fixed-metric first variation with exact PT/integrability transport, finite arbitrary-diffeomorphism covariance, a zero integrated Noether pairing on supplied smooth orbits, a weak covariant scalar Euler/flux decomposition with zero-flux stationarity and intrinsic-D8 specialization under the exact IPP interface, and pointwise diffeomorphism covariance of intrinsic scalar stress; physical-volume graph `H¹`, coarea, canonical trace/operator/Dirichlet/Hilbert renorming, complete D8 time action, global scalar weak Euler/Jacobi and static index-zero Fredholm bridges are green | geometrically discharge Stokes/IPP and identify its boundary functional with the now-vanishing concrete Dirichlet flux, then construct the local Noether current with enough test ghosts; prove the single local normal-projection regularity lemma, identify intrinsic Sobolev sections, close nonpositive/complex raw residuals, select the physical spectral domain, then general-metric BV, boundary stability and the curved global PDE; construct non-Hermitian Jordan chains only if an explicit normal form is needed |
...
| P-A relative uniqueness | yes; finite tangent generators and first-jet graph `H¹`, unconditional canonical trace/Dirichlet/Hilbert renorming, raw split-positive `4×4` Sylvester-regular root selection, continuous/IFT-stable diagonal root with inverse-Sylvester derivative, canonical intrinsic global `(3,1)` metric, finite nonzero quotient action, unconditional smooth arbitrary-diffeomorphism metric pullback, integrated general-metric scalar and fixed-metric first-variation covariance, an integrated Noether-orbit identity, a weak covariant Euler/flux bridge with zero-flux stationarity and intrinsic-D8 specialization under the exact IPP interface, concrete Dirichlet normal-flux cancellation, and pointwise stress covariance, complete D8 time flow, Candidate-A induced variation and static scalar ellipticity bridge | geometrically discharge Stokes/IPP and identify its boundary functional with the concrete flux, then construct the local Noether current with enough test ghosts; prove normal-projection regularity, identify intrinsic Sobolev sections, then nonpositive classification, parent action, general-metric/nonlocal BV and the curved global PDE; raw Jordan presentation is optional unless an explicit normal form is required |
