...
| --- | --- | --- | --- | --- |
| **D** | Fundamental geometry | derive throat, Pin/Z4, gauge and spectrum from one PT-twisted geometry | active priority | actual global operator, effective action, absolute scale |
...
| **D4.1** | scalar spectral modulus | **C** | equal first scalar modes give `T^2=2*pi^2` | `P0EFTJanusSpectralIsotropyAlphaRatio.lean` |
| **D4.2** | spectral weights | **T/N** | weights are physical determinant data, not topology | `P0EFTJanusWeightedSpectralLock.lean` |
...
| **D5** | spectral effective action | **P** | must derive modulus, quarter holonomy and even-gap charge law | next analytic program |
...
| **P-D-SCALAR-ISOTROPY** | equal first scalar modes select `T^2=2*pi^2` | **conditional** | mode weights and vacuum law must come from the determinant |
...
| **P-D-DIRAC-LOCK** | paired quarter-holonomy Dirac gaps generate `q_LL` | **strongest conditional candidate** | reproduces `1/8` charge coefficient and `A=L` |
...
This is a compatibility theorem, not an absolute-scale prediction.

## Next theorem queue

| Priority | Target | Success criterion | Failure meaning |
| ---: | --- | --- | --- |
| **1** | global Pin lift on the mapping torus | prove quarter holonomy from geometry | Z4 remains only an algebraic candidate |
| **2** | analytic SpinC Dirac operator on `S2 x S1` | prove full separated spectrum and multiplicities | integer spectrum model is not physical |
| **3** | Hurwitz-zeta / APS eta calculation | derive eta `+/-1/2` and paired determinant phase | parity cancellation law must be revised |
| **4** | determinant-derived charge law | derive `q_LL=gap_+^2+gap_-^2` | the `1/8` lock is only a normalization ansatz |
| **5** | renormalized effective action | select `T` and a dimensionful scale without observation | Program D cannot close alpha autonomously |
...
python scripts/audit_janus_z4_dirac_alpha_lock.py
python -m pytest -q tests/test_janus_fundamental_geometry_d.py
python -m pytest -q tests/test_janus_z4_dirac_alpha_lock.py
```
