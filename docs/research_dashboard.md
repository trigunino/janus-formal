...
| **P0/P-A** | reject shortcuts and define relative selection | causally compatible global diagonal Candidate-A domain with exact spectral frontier and global-field functional variation on the same compact smooth D8 quotient; populated independent/induced configuration; smooth/L²/PT/trace/Dirichlet spaces; complete graph-`H¹` and conditional continuous trace; fixed-frame global holonomic scalar action and variation; diagonal diffeomorphism action; actual LL throat action; explicit null domain `Theta ≠ 0` | intrinsic trace inequality/frame construction, general tensorial Lorentz/gauge covariance, full curved Euler--flux action/PDE, same-action Hessian and constrained stability |
...
| P-A relative uniqueness | yes; PT-matched smooth D8 fields, weighted `Z^4` exactness, complete first-jet graph `H¹`, causally compatible global diagonal root/domain/frontier, global Candidate-A metric-field variation, same-field/same-metric holonomic scalar action and fixed-metric variation, plus flat weak Euler pairing | intrinsic Sobolev trace inputs, general tensorial root/action, parent action, automatic domination, physical boundary flux and curved global PDE not derived |
