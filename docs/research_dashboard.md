...
| **D9/D11** | elliptic and natural-operator gates | green D9 symbol/BRST and D11 naturality/jet heads; true tangent ghost with linearized BRST, an unconditional faithful nonabelian quotient `so(3)` ghost triple, explicit CE and corrected global square-zero Koszul differentials, componentwise square-zero extension to current linear matter/gauge/ghost/auxiliary sectors, and explicit descended throat `so(3)` scalar/Koszul/LL maps | prove the single throat square-zero contract, then metrics/antifields/BV/SpinC and a concrete Janus Fredholm family |
...
| **P0/P-A** | reject shortcuts and define relative selection | the global positive-diagonalizable root selector is continuous, locally IFT-stable and has the exact inverse-Sylvester derivative; one exhaustive inductive selector unifies every strictly-positive real Jordan partition of dimension four with exact square, bijective Sylvester and per-stratum continuity, while the raw split-positive matrix case is reduced to one missing Jordan-basis bridge absent from Mathlib; determinant/simple-negative no-go criteria, a positive-determinant counterexample, paired-negative roots and the exact Jordan parity/zero-block criterion cover the real nonpositive frontier conditionally on one raw Jordan bridge; pure complex-conjugate pairs, `2+2` sums and non-semisimple complex Jordan chains have explicit real roots with exact cut/zero behavior, leaving one raw nonreal presentation bridge; canonical global intrinsic `(3,1)` metric with an unconditional finite nonzero quotient-volume/action; physical-volume graph `H¹` with exact FTC/Fubini, twisted latitude collar, throat pushforward, `L²` trace identity and exact finite-frame normal reconstruction, with coarea reduced to one `Measure.toSphere` measure domination and frame control conditional only on tangent-lift continuity; a jointly analytic nontrivial complete D8 time action acts nontrivially on a full current independent-field configuration and has its exact set-theoretic orbit quotient; global scalar weak Euler/Jacobi and static-positive index-zero Fredholm/H¹ bridge | prove the single collar-measure domination and tangent-lift continuity, identify intrinsic Sobolev sections, supply the three raw Jordan bridges and select the physical spectral domain, then nonlinear/BV and boundary stability |
...
| P-D low-rank pairings/fusion | focused Lean head, global normal `Z4` root lines, abelian `U(1)^2` BRST, a true tangent-ghost Lie algebra, an unconditional faithful nonabelian quotient `so(3)` ghost triple with explicit CE/corrected Koszul differential, and linear matter/gauge/ghost/auxiliary square-zero extension are green | LL/throat action, positive metrics, then global SpinC/antifields/BV |
