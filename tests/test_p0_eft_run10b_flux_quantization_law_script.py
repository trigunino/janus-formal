from __future__ import annotations

import unittest

from scripts.build_p0_eft_run10b_flux_quantization_law import build_payload, render_markdown


class P0EFTRun10BFluxQuantizationLawTests(unittest.TestCase):
    def test_quantization_law_interface_ready(self) -> None:
        status = build_payload()["theorem_status"]

        self.assertTrue(status["run10b_flux_quantization_law_interface_ready"])
        self.assertTrue(status["quantization_law_to_integer_flux_arrow_formalized"])
        self.assertTrue(status["run10b_holonomy_quantum_normalization_interface_ready"])
        self.assertTrue(status["normalization_to_flux_law_arrow_formalized"])

    def test_integrality_remains_open(self) -> None:
        status = build_payload()["theorem_status"]

        self.assertTrue(status["spin_connection_restricts_to_orbifold_cycle_proved"])
        self.assertTrue(status["holonomy_quantum_normalized_proved"])
        self.assertTrue(status["normalized_flux_integer_proved"])
        self.assertTrue(status["integer_flux_law_proved"])
        self.assertFalse(status["full_cosmology_prediction_ready_no_fit"])

    def test_report_names_normalized_flux(self) -> None:
        markdown = render_markdown(build_payload())

        self.assertIn("P0EFTOrbifoldHolonomyQuantumNormalization", markdown)
        self.assertIn("P0EFTOrbifoldSpinConnectionGaugeFix", markdown)
        self.assertIn("P0EFTOrbifoldHolonomyQuantumNormalized", markdown)
        self.assertIn("P0EFTOrbifoldNormalizedFluxInteger", markdown)
        self.assertIn("P0EFTOrbifoldIntegerFluxLaw", markdown)
        self.assertIn("normalized flux is an integer", markdown)
        self.assertIn("integerFluxDataClosed", markdown)


if __name__ == "__main__":
    unittest.main()
