...
from scripts.build_p0_eft_janus_z2_alpha_state_sector_closure_readiness_gate import build_payload


class AlphaStateSectorClosureReadinessGateTests(unittest.TestCase):
    def test_live_state_sector_closure_is_blocked(self):
        payload = build_payload()

        self.assertTrue(payload["gate_passed"])
        self.assertFalse(payload["alpha_generated_no_fit"])
        self.assertTrue(payload["published_janus_equivalence_on_alpha"])
        self.assertFalse(payload["any_closure_route_ready"])
        self.assertTrue(payload["state_sector_closure_blocked"])

    def test_nonzero_kks_or_valpha_unblocks_closure_route(self):
        ...
        self.assertTrue(payload["any_closure_route_ready"])
        self.assertFalse(payload["state_sector_closure_blocked"])
        self.assertTrue(
            payload["closure_routes"]["nonzero_PT_KKS_Souriau_boundary_density"]["ready"]
        )
        self.assertTrue(
            payload["closure_routes"][
