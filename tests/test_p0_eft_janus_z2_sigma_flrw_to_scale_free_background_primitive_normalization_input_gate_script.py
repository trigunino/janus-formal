...
class FLRWToScaleFreeBackgroundPrimitiveNormalizationInputGateTests(unittest.TestCase):
    ...
    def test_flrw_and_omega_write_normalization_and_background_primitive(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            flrw = root / "flrw_components.json"
            omega = root / "omega.json"
            norm = root / "background_norm.json"
        ...
        self.assertTrue(payload["downstream_background_writer_passed"])
        self.assertEqual(norm_payload["source"], "active_derived")
        self.assertIn("E_Z2Sigma", norm_payload)
