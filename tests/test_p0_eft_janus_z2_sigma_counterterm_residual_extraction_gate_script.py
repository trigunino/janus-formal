...
class P0EFTJanusZ2SigmaCountertermResidualExtractionGateTests(unittest.TestCase):
    ...
    def test_residual_extraction_remains_open_until_primitive_exists(self):
        ...
        self.assertFalse(payload["upstream_frontiers"]["one_form_decomposition"]["ready"])
        self.assertFalse(payload["upstream_frontiers"]["integrability"]["ready"])
        self.assertFalse(payload["upstream_frontiers"]["primitive_integration"]["ready"])
        self.assertIn("residual_one_form_exact = false", payload["upstream_frontiers"]["primitive_integration"]["current_frontier"])
        ...
        self.assertIn("L_ct_local_expansion_derived = false", payload["current_frontier"])
