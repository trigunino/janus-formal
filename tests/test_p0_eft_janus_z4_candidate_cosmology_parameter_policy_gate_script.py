...
class P0EFTJanusZ4CandidateCosmologyParameterPolicyGateTests(unittest.TestCase):
    def test_cosmology_policy(self):
        ...
        self.assertTrue(payload["backend_uses_static_spectra_tables"])
        self.assertFalse(payload["cosmological_transfer_regeneration_available"])
        self.assertTrue(all(payload["cosmology_parameters_fixed"].values()))
        self.assertFalse(payload["standard_cosmology_profiled"])
