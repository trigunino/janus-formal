...
class P0EFTJanusZ4StandaloneTEEEHandshakeGateTests(unittest.TestCase):
    def test_standalone_teee_handshake_gate(self):
        ...
        self.assertFalse(payload["standalone_highl_TE_available"])
        self.assertFalse(payload["standalone_highl_EE_available"])
        self.assertFalse(payload["standalone_highl_TE_EE_acquired"])
        ...
        self.assertFalse(payload["closed_boltzmann_candidate_highl_decomposition_trial_allowed"])
        self.assertFalse(payload["full_planck_validation_allowed"])
        self.assertFalse(payload["standalone_teee_handshake_gate_passed"])
