...
from scripts.derive_p0_eft_janus_z2_cover_master_projected_equations import build_payload
...
class JanusZ2CoverMasterProjectedEquationsScriptTest(unittest.TestCase):
    def test_blocks_without_manifest(self):
        with tempfile.TemporaryDirectory() as tmp:
            payload = build_payload(Path(tmp) / "missing.json", Path(tmp) / "out.json")
        self.assertFalse(payload["gate_passed"])
        self.assertEqual(payload["primary_blocker"], "master_action_projection_inputs_missing")

    def test_writes_projected_equations(self):
        with tempfile.TemporaryDirectory() as tmp:
        ...
        self.assertTrue(written["projected_equations_ready"])
        self.assertIn("G_plus", written["projected_equations"]["E_plus"])
