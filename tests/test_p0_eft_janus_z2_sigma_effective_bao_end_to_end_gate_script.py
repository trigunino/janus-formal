...
from scripts.build_p0_eft_janus_z2_sigma_effective_bao_end_to_end_gate import build_payload
...
def _write_primitives(path: Path, closure: Path) -> None:
...
class JanusZ2SigmaEffectiveBAOEndToEndGateTest(unittest.TestCase):
    def test_missing_manifests_block_end_to_end(self):
        payload = build_payload(
            closure_input_path=Path("__missing_closure__.json"),
            primitive_input_path=Path("__missing_primitive__.json"),
        )
        self.assertFalse(payload["gate_passed"])
        self.assertFalse(payload["full_no_fit_prediction_ready"])

    def test_valid_effective_manifests_evaluate_end_to_end(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _write_primitives(primitives, closure)
            payload = build_payload(
                closure_input_path=closure,
                primitive_input_path=primitives,
            )
