...
REPORT_PATH = Path("outputs/reports/cosmological_pm_power_observables.md")
...
def fields_for_power(bodies, grid_shape: tuple[int, int], box_size: float):
...
def main() -> None:
    ...
    for field_name in ("positive", "negative", "signed"):
        initial_spectrum = radial_power_spectrum_2d(
            initial_fields[field_name],
            box_size=box_size,
            bin_edges=bin_edges,
        )
        final_spectrum = radial_power_spectrum_2d(
    ...
    payload = {
        "description": "Internal band-power observables from the Gaussian-IC comoving PM prototype.",
        "parameters": {
        ...
        "boundary": "Prototype internal observables only; not survey-calibrated.",
    }
    JSON_PATH.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    lines = [
        "# Cosmological PM Power Observables",
        "",
        "Internal structure-observable export for the Gaussian-IC comoving PM prototype.",
        "",
        "| signed k bin | initial power | final power | growth ratio |",
        "|---:|---:|---:|---:|",
    ]
    for row in signed_rows:
        lines.append(
            f"| {row['bin']} | {row['initial_power']:.6g} | "
    ...
    lines.extend(
        [
            ...
            f"JSON: `{JSON_PATH}`",
            "",
            "Boundary: these are internal band-power observables from a prototype simulation. They are not survey-calibrated and are not yet comparable to physical Mpc-scale spectra without units, transfer functions and normalization.",
            "",
        ]
    )
