...
REPORT_PATH = Path("outputs/reports/p0_eft_janus_z4_weak_surface_branch_archive_gate.md")
JSON_PATH = Path("outputs/reports/p0_eft_janus_z4_weak_surface_branch_archive_gate.json")


def build_payload() -> dict:
    refinement = refinement_payload()
    archive = refinement["parallel_fraction_full_surface_refined"] >= 0.70
    return {
        "status": "janus-z4-weak-surface-branch-archive-gate",
        ...
        "surface_branch_status_before_archive": refinement["branch_status"],
        "archive_weak_surface_branch": archive,
...
def write_reports() -> dict:
    ...
    lines = [
        "# Janus Z4 Weak Surface Branch Archive Gate",
        "",
        f"Archive weak surface branch: `{payload['archive_weak_surface_branch']}`",
