...
from janus_lab.z2_sigma_active_pipeline import (
    write_scale_free_background_primitive_normalization_inputs_from_flrw_and_omega_k_manifests,
)
...
REPORT_PATH = Path(
    "outputs/reports/p0_eft_janus_z2_sigma_flrw_to_scale_free_background_primitive_normalization_input_gate.md"
)
JSON_PATH = Path(
    "outputs/reports/p0_eft_janus_z2_sigma_flrw_to_scale_free_background_primitive_normalization_input_gate.json"
)
    ...
    *,
    ...
    if all(input_exists.values()):
        try:
            ...
            written = True
            writer = build_background_writer_payload(
    ...
    return {
        "status": "janus-z2-sigma-flrw-to-scale-free-background-primitive-normalization-input-gate",
        "active_core": "Z2_tunnel_Sigma",
        "input_exists": input_exists,
        "normalization_manifest": str(normalization_path),
        "normalization_manifest_written": written,
        "downstream_background_writer_passed": writer_passed,
...
def write_reports() -> dict:
    ...
    lines = [
        "# Janus Z2/Sigma FLRW To Scale-Free Background Primitive Normalization Input Gate",
        "",
        f"Input exists: `{payload['input_exists']}`",
        f"Normalization manifest written: `{payload['normalization_manifest_written']}`",
        f"Downstream background writer passed: `{payload['downstream_background_writer_passed']}`",
