...
from scripts.build_p0_eft_janus_z4_master_observed_planck_wrapper_handshake_gate import (
    build_payload as wrapper_payload,
)
...
REPORT_PATH = Path("outputs/reports/p0_eft_janus_z4_master_no_retuning_replay_gate.md")
JSON_PATH = Path("outputs/reports/p0_eft_janus_z4_master_no_retuning_replay_gate.json")
...
def build_payload() -> dict:
    ...
    replay_passed = bool(
        wrapper["observed_planck_wrapper_handshake_gate_passed"]
    ...
    return {
        "status": "janus-z4-master-no-retuning-replay-gate",
        "observed_planck_wrapper_handshake_gate_passed": wrapper[
            "observed_planck_wrapper_handshake_gate_passed"
        ],
        "source_payload_hash": spectra["source_payload_hash"],
        "normalization_parameter": spectra["regularization_limit"],
        "normalization_fixed_to_a_sigma": spectra["regularization_limit"] == 2.0 / 3.0,
        ...
        "next_required_gate": "P0EFTJanusZ4MasterObservedPlanckDiagnosticTrialGate"
        if replay_passed
        else "repair_master_no_retuning_replay",
...
def write_reports() -> dict:
    ...
    lines = [
        "# Janus Z4 Master No-Retuning Replay Gate",
        "",
        f"Replay without retuning: `{payload['candidate_replayed_without_retuning']}`",
        f"Normalization fixed to a_sigma: `{payload['normalization_fixed_to_a_sigma']}`",
