...
REPORT_PATH = Path("outputs/reports/p0_eft_two_mode_primordial_scan.md")
JSON_PATH = Path("outputs/reports/p0_eft_two_mode_primordial_scan.json")
CSV_PATH = Path("outputs/reports/p0_eft_two_mode_primordial_scan.csv")
YAML_PATH = Path("outputs/cmb_bridge/cobaya_planck_two_mode_primordial_scan.yaml")
FORK_PATH = Path("external/camb_janus_fork").resolve()
SOURCE_PATH = Path("external/camb_janus_fork/fortran/JanusHolstSources.f90")

PRE_DRAG_GRID = [-0.015, 0.0, 0.015]
VISIBILITY_GRID = [-0.05, 0.0, 0.05]
...
def set_modes(theta_pre: float, theta_vis: float) -> None:
    text = SOURCE_PATH.read_text(encoding="utf-8")
    text = replace_param(text, "c_sound", theta_pre)
    text = replace_param(text, "c_opacity", theta_vis)
    text = replace_param(text, "c_geff", -2.0 * theta_pre)
    text = replace_param(text, "c_immirzi", -2.0 * theta_pre)
...
def grid() -> list[dict[str, float]]:
    return [
        {"theta_pre": theta_pre, "theta_visibility": theta_vis}
        for theta_pre in PRE_DRAG_GRID
        for theta_vis in VISIBILITY_GRID
    ]


def run_point(point: dict[str, float]) -> dict:
    set_modes(point["theta_pre"], point["theta_visibility"])
...
def build_payload(points: list[dict[str, float]] | None = None, execute: bool = True) -> dict:
    ...
    rows: list[dict] = []
    try:
        rows = [run_point(point) for point in points] if execute else points
    finally:
    ...
    return {
        "description": "Two-mode primordial scan: pre-drag transfer mode plus independent visibility mode.",
        "status": "two-mode-primordial-scan-run" if execute else "two-mode-primordial-scan-dry",
        ...
        "next_required": "If viable, derive both modes from Janus geometry; otherwise test nonlocal visibility source.",
    }
...
def write_reports(report_path: Path = REPORT_PATH, json_path: Path = JSON_PATH) -> dict:
    ...
    headers = ["theta_pre", "theta_visibility", "chi2_CMB", "chi2_highl", "chi2_lowl_EE", "chi2_lensing", "returncode"]
    CSV_PATH.write_text(
...
def render_markdown(payload: dict) -> str:
    ...
    return "\n".join(
        [
            "# P0 EFT Two-Mode Primordial Scan",
            "",
            payload["description"],
            "",
            ...
            "## Best",
            "",
            f"- theta_pre: `{best.get('theta_pre')}`",
            f"- theta_visibility: `{best.get('theta_visibility')}`",
