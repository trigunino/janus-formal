...
REPORT_PATH = Path("outputs/reports/janus_tomographic_lensing_kernel.md")
JSON_PATH = Path("outputs/reports/janus_tomographic_lensing_kernel.json")
...
def row_for_projection(name: str, fields, weights: np.ndarray, axis: int, box_size_mpc: float, bin_edges: np.ndarray):
    ...
    first = next((i for i, count in enumerate(spectrum.mode_counts) if count > 0), 0)
    return {
        "family": name,
        "kappa_janus_kernel_rms": rms(kappa),
        "kappa_janus_kernel_min": float(np.min(kappa)),
        "kappa_janus_kernel_max": float(np.max(kappa)),
        "first_k_center_inv_mpc": float(spectrum.k_centers[first]),
        "first_projected_power": float(spectrum.power[first]),
        "first_mode_count": int(spectrum.mode_counts[first]),
    }
...
def main() -> None:
    ...
    axis_len = grid_shape[args.axis]
    z_slices = np.linspace(0.0, args.source_redshift, axis_len)
    weights = janus_tomographic_lensing_weights(
        z_slices,
        z_source=args.source_redshift,
        model=model,
    )
    ...
    payload = {
        "description": "Janus open-distance geometric lensing-kernel projection.",
        "kernel_formula": "K(z_l,z_s)=r_l*r_ls/r_s, r=sinh(2*(u0-u(z)))",
        ...
        "boundary": "Janus open-distance geometric kernel only; no source-redshift distribution, growth model, shear estimator or survey likelihood.",
    }
    JSON_PATH.write_text(json.dumps(payload, indent=2), encoding="utf-8")

    lines = [
        "# Janus Tomographic Lensing Kernel",
        "",
        "Projection using the Janus open marker distance from M18.",
        "",
        "```text",
        payload["kernel_formula"],
        "```",
        "",
        f"Source redshift: `{args.source_redshift}`, peak weight redshift: `{payload['weight_argmax_redshift']:.6g}`.",
        "",
        "| family | kappa kernel RMS | min | max | first k [1/Mpc] | first projected power | modes |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    for row in rows:
        lines.append(
            f"| {row['family']} | {row['kappa_janus_kernel_rms']:.6g} | "
            f"{row['kappa_janus_kernel_min']:.6g} | {row['kappa_janus_kernel_max']:.6g} | "
