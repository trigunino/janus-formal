...
JSON_PATH = REPORTS / "p0_eft_janus_z2_alpha_state_sector_closure_readiness_gate.json"
REPORT_PATH = REPORTS / "p0_eft_janus_z2_alpha_state_sector_closure_readiness_gate.md"
...
def build_payload(
    ...
    any_closure_route_ready = any(route["ready"] for route in closure_routes.values())

    return {
        "status": "janus-z2-alpha-state-sector-closure-readiness-gate",
        ...
        "any_closure_route_ready": any_closure_route_ready,
        "state_sector_closure_blocked": not any_closure_route_ready,
        "closure_meaning": "closed as inactive/conditional until one listed non-rustine input exists",
        "required_non_rustine_inputs": [
...
def write_reports() -> dict[str, Any]:
    ...
    REPORT_PATH.write_text(
        "\n".join(
            [
                "# Janus Z2 Alpha State-Sector Closure Readiness Gate",
                "",
                f"Classification: `{payload['current_classification']}`",
                f"Any closure route ready: `{payload['any_closure_route_ready']}`",
                f"State-sector closure blocked: `{payload['state_sector_closure_blocked']}`",
                "",
                "This classifies alpha as a conditional state sector.",
                "It can become predictive only with a non-rustine boundary density, torsionful/null theta, matter/gauge phase space, or a finite minisuperspace V(alpha) prescription.",
            ]
        ),
        encoding="utf-8",
    )
