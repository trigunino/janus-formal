...
REPORT_PATH = Path("outputs/reports/janus_absolute_convergence.md")
JSON_PATH = Path("outputs/reports/janus_absolute_convergence.json")
...
def source_weights_for(args) -> tuple[np.ndarray, np.ndarray]:
    ...
    weights = parse_float_list(args.source_weights) if args.source_weights.strip() else np.ones_like(redshifts)
...
def row_for_absolute(name: str, fields, coefficients: np.ndarray, axis: int, box_size_mpc: float, bin_edges: np.ndarray):
    positive_density, negative_density = densities_from_contrasts(fields)
    lensing_contrast = positive_photon_lensing_contrast(positive_density, negative_density)
...
def main() -> None:
    ...
    rows = [
        row_for_absolute("lognormal_positive_density", lognormal_fields, coefficients, args.axis, args.box_size_mpc, bin_edges),
        row_for_absolute(
    ...
    payload = {
        "description": "Absolute convergence proxy using Janus open-distance geometry and standard weak-lensing prefactor.",
        "normalization_formula": "coeff=(3/2)*Omega_abs*(H0/c)^2*Delta_chi*(1+z_l)*D_l*D_ls/D_s",
        ...
        "coefficient_argmax_index": int(np.argmax(coefficients)),
        "coefficient_argmax_redshift": float(z_slices[int(np.argmax(coefficients))]),
        ...
        "boundary": "Absolute normalization scaffold: standard weak-lensing prefactor applied to Janus source/open geometry; not a full tensor Janus derivation or survey likelihood.",
    }
    ...
    lines = [
        ...
        "",
        "Absolute convergence proxy using Janus open-distance geometry and the standard weak-lensing prefactor.",
        "",
        "```text",
        payload["normalization_formula"],
        "```",
        "",
        f"Peak coefficient redshift: `{payload['coefficient_argmax_redshift']:.6g}`.",
        "",
        "| family | kappa abs RMS | min | max | shear abs RMS | first shear power |",
        "|---|---:|---:|---:|---:|---:|",
    ]
    for row in rows:
        lines.append(
            f"| {row['family']} | {row['kappa_abs_rms']:.6g} | "
            f"{row['kappa_abs_min']:.6g} | {row['kappa_abs_max']:.6g} | "
