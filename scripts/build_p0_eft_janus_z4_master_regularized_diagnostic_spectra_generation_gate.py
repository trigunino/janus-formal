...
from scripts.build_p0_eft_janus_z4_master_membrane_transport_regularization_gate import build_payload as transport_payload
...
REPORT_PATH = Path("outputs/reports/p0_eft_janus_z4_master_regularized_diagnostic_spectra_generation_gate.md")
JSON_PATH = Path("outputs/reports/p0_eft_janus_z4_master_regularized_diagnostic_spectra_generation_gate.json")
SPECTRA_DIR = Path("outputs/reports/z4_master_regularized_diagnostic_spectra")
...
def _regularized_delta(reference: list[dict[str, float]], source: dict, limit: float) -> dict[str, np.ndarray]:
    ...
    shape = {
        "cl_tt": _regularize(s_t + 0.15 * doppler, limit),
        "cl_te": _regularize(0.5 * s_t + 0.5 * s_e, limit),
        "cl_ee": _regularize(s_e + 0.2 * pi, limit),
    }
    return {channel: arrays[channel] * shape[channel] for channel in CHANNELS}
...
def build_payload() -> dict:
    ...
    arrays = _rows_to_arrays(reference)
    delta = _regularized_delta(reference, source, float(transport["regularization_limit"]))
    candidate = []
    ...
    candidate_path = SPECTRA_DIR / "reference_gr_plus_master_z4_membrane_regularized.csv"
    ...
    return {
        "status": "janus-z4-master-regularized-diagnostic-spectra-generation-gate",
        "membrane_transport_regularization_gate_passed": transport["transport_solution_verified"],
        "regularization_route": transport["transport_route"],
        "regularization_limit": transport["regularization_limit"],
        "source_payload_hash": hash_payload(source),
        "baseline_spectra_path": str(baseline_path),
        "candidate_spectra_path": str(candidate_path),
        "regularized_diagnostic_spectra_generated": True,
...
def write_reports() -> dict:
    ...
    lines = [
        "# Janus Z4 Master Regularized Diagnostic Spectra Generation Gate",
        "",
        f"Generated: `{payload['regularized_diagnostic_spectra_generated']}`",
