...
REPORT_PATH = Path("outputs/reports/p0_eft_polarization_source_shape_scan.md")
JSON_PATH = Path("outputs/reports/p0_eft_polarization_source_shape_scan.json")
CSV_PATH = Path("outputs/reports/p0_eft_polarization_source_shape_scan.csv")
YAML_PATH = Path("outputs/cmb_bridge/cobaya_planck_polarization_source_shape_scan.yaml")
FORK_PATH = Path("external/camb_janus_fork").resolve()
SOURCE_PATH = Path("external/camb_janus_fork/fortran/JanusHolstSources.f90")
POLARIZATION_SOURCE_GRID = [-1.0, -0.5, 0.0, 0.5, 1.0]
...
def set_polarization_source(amplitude: float) -> None:
    text = SOURCE_PATH.read_text(encoding="utf-8")
    text = replace_param(text, "c_polarization_source", amplitude)
...
def run_point(amplitude: float) -> dict:
    set_polarization_source(amplitude)
    build_code = build_fork()
    result = run_cobaya() if build_code == 0 else {"returncode": build_code, "chi2_CMB": None}
    return {"c_polarization_source": amplitude, "build_returncode": build_code, **result}


def build_payload(points: list[float] | None = None, execute: bool = True) -> dict:
    points = points or POLARIZATION_SOURCE_GRID
    if not execute:
        return {
            "description": "Direct polarization source-shape scan for Planck lowE/high-l.",
            "status": "polarization-source-shape-scan-dry",
            "grid_size": len(points),
            "valid_points": 0,
            "rows": [{"c_polarization_source": point} for point in points],
            "best": None,
    ...
    return {
        "description": "Direct polarization source-shape scan for Planck lowE/high-l.",
        "status": "polarization-source-shape-scan-run",
        "grid_size": len(points),
        ...
        "full_cosmology_prediction_ready_no_fit": False,
        "next_required": "If lowE stays high, the missing physics is not a scalar polarization amplitude factor.",
    }
...
def render_markdown(payload: dict) -> str:
    ...
    return "\n".join(
        [
            "# P0 EFT Polarization Source Shape Scan",
            "",
            payload["description"],
            "",
            ...
            "## Best",
            "",
            f"- c_polarization_source: `{best.get('c_polarization_source')}`",
...
def write_reports(report_path: Path = REPORT_PATH, json_path: Path = JSON_PATH) -> dict:
    ...
    headers = ["c_polarization_source", "chi2_CMB", "chi2_highl", "chi2_lowl_EE", "chi2_lensing", "returncode"]
    CSV_PATH.write_text(
