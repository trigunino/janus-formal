...
INPUT_PATH = Path("outputs/active_z2_sigma/background_H0_normalization_inputs.json")
OUTPUT_PATH = Path("outputs/active_z2_sigma/background_H0_inputs.json")
REPORT_PATH = Path("outputs/reports/p0_eft_janus_z2_sigma_background_h0_input_writer_gate.md")
JSON_PATH = Path("outputs/reports/p0_eft_janus_z2_sigma_background_h0_input_writer_gate.json")
...
def build_payload(*, input_path: Path = INPUT_PATH, output_path: Path = OUTPUT_PATH) -> dict:
    ...
    return {
        "status": "janus-z2-sigma-background-h0-input-writer-gate",
        ...
        "primary_blocker": "none" if output_written else "active_H0_scale_normalization_from_RSigma",
...
def write_reports() -> dict:
    ...
    REPORT_PATH.write_text(
        "\n".join([
            "# Janus Z2/Sigma Background H0 Input Writer Gate",
            "",
            f"Input exists: `{payload['input_exists']}`",
            f"Missing active artifact: `{payload['missing_active_artifact']}`",
            f"H0 input written: `{payload['background_h0_input_written']}`",
            "Scale-free `H0*R_curv/c` alone cannot determine dimensional `H0`.",
