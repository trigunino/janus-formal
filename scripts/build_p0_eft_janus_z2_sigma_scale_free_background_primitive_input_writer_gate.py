...
INPUT_PATH = Path("outputs/active_z2_sigma/bao_scale_free_background_primitive_normalization_inputs.json")
OUTPUT_PATH = Path("outputs/active_z2_sigma/bao_scale_free_background_primitive_inputs.json")
REPORT_PATH = Path(
    "outputs/reports/p0_eft_janus_z2_sigma_scale_free_background_primitive_input_writer_gate.md"
)
JSON_PATH = Path(
    "outputs/reports/p0_eft_janus_z2_sigma_scale_free_background_primitive_input_writer_gate.json"
)
...
def _build_output(input_path: Path, output_path: Path) -> Path:
    ...
    e_values = np.asarray(payload["E_Z2Sigma"], dtype=float)
    ...
    if e_values.shape != z_grid.shape:
        raise ValueError("E_Z2Sigma must align with z_grid")
    if np.any(e_values <= 0.0):
        raise ValueError("E_Z2Sigma values must be positive")
    return write_active_z2sigma_scale_free_background_primitive_manifest(
        output_path,
        z_grid,
        _grid_function(z_grid, e_values),
        float(payload["omega_k_Z2Sigma"]),
...
def build_payload(*, input_path: Path = INPUT_PATH, output_path: Path = OUTPUT_PATH) -> dict:
    ...
    return {
        "status": "janus-z2-sigma-scale-free-background-primitive-input-writer-gate",
        ...
        "next_required": [
            ...
            "supply_outputs_active_z2_sigma_bao_scale_free_background_primitive_normalization_inputs_json",
...
def write_reports() -> dict:
    ...
    lines = [
        "# Janus Z2/Sigma Scale-Free Background Primitive Input Writer Gate",
        "",
