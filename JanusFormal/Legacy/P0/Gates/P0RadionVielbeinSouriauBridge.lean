import JanusFormal.Legacy.P0.Gates.P0RadionDilatonBranchSelection
import JanusFormal.Legacy.P0.Gates.P0SouriauPoincareEnergyBranch
...
theorem conformal_phi_four_dimensional_scaling_does_not_force_time_flip :
  ...
  exact h bad ⟨trivial, trivial, trivial, trivial⟩
...
theorem temporal_leg_radion_geometry_forces_odd_orientation
  ...
  exact ⟨h.right.right.left, h.right.right.right.left, h.right.right.right.right.right.left⟩
  ...
  { souriauData := c.souriauData
    ...
    confinement := by
      rcases c.confinement with
        ⟨hSheet, hTopo, hNoMix, hMetric, _hBranch, hStable⟩
      exact
        ⟨hSheet,
          hTopo,
          hNoMix,
          hMetric,
          ⟨radion_certificate_selects_minus_branch c.radionCertificate,
            temporal_leg_operator_closes_antichronous_selection c.geometry c.geometryClosed⟩,
          hStable⟩ }
