...
theorem closed_spec_gives_exact_derivation_closed
  ...
  rcases h with
    âŸ¨hSig, hBulkPlus, hBulkMinus, hSolder, hGhy, hMembrane,
      hMatterPlus, hMatterMinus, hTetrad, hAether, hTwin, hHROnSigma,
      hHRBetas, hGauge, hLapseShift, hReinject, hPerturb, hIsrael, hZetaâŸ©
  dsimp [scalarVectorExactDerivationClosed, exactParamsFromFullSVTSpec]
  exact And.intro hGauge
    (And.intro âŸ¨hLapseShift, hReinjectâŸ©
      (And.intro âŸ¨hIsrael, hZetaâŸ©
        (And.intro âŸ¨hBulkPlus, hBulkMinus, hSolderâŸ©
          (And.intro hPerturb
            (And.intro âŸ¨hHROnSigma, hHRBetasâŸ© hAether)))))
...
theorem prediction_ready_from_souriau_source_and_frontiers
  ...
  exact âŸ¨c.sourceClosed, c.specClosed,
    closed_spec_gives_exact_derivation_closed c.spec c.specClosed,
    âŸ¨fullSVTExactCertificateFromSpec
      c.spec c.specClosed c.physicalDomainâŸ©âŸ©
...
theorem canonical_full_svt_spec_gives_exact_certificate :
  ...
  exact âŸ¨fullSVTExactCertificateFromSpec
    canonicalFullSVTSpec
    canonical_full_svt_spec_closed
    canonical_full_svt_domainâŸ©
