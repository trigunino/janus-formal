...
theorem radion_certificate_selects_minus_branch
  ...
  rcases c.brokenPhase with
    âŸ¨_hField, _hOdd, _hPotential, _hHigh, _hTransition,
      _hVEV, _hPlus, hMinusâŸ©
  rcases c.sortingClosed with
    âŸ¨_hMatter, _hYukawa, _hTracks, _hPlusSelect,
      hMinusSelect, hConfinement, hEnergyâŸ©
  exact âŸ¨hMinus, hMinusSelect, hConfinement, hEnergyâŸ©
...
theorem double_well_alone_does_not_sort_matter :
  ...
  have hBroken : brokenRadionPhase p :=
    âŸ¨trivial, trivial, trivial, trivial, trivial, trivial, trivial, trivialâŸ©
...
theorem radion_selected_bimembrane_can_close_if_other_obligations_hold
  ...
  rcases hBase with
    âŸ¨hPlus, hMinus, hInterface, hTopo, hLeak, hNoMix,
      hMetric, hDefect, hCharge, hPTâŸ©
  exact
    âŸ¨hPlus,
      hMinus,
      hInterface,
      ...
      âŸ¨hStable, c.sortingClosed.right.right.right.right.right.leftâŸ©,
      hDefect,
      hCharge,
      hPTâŸ©
