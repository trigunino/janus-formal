...
theorem pre_likelihood_lock_v2_never_promotes
  ...
  cases h with
  | inl hCleared =>
      rcases hCleared with âŸ¨_, _, _, _, _, _, _, _, _, _, _, hNoPlanck, _, hNoPromotion, _, hNoFullâŸ©
      exact âŸ¨hNoPlanck, hNoPromotion, hNoFullâŸ©
  | inr hBlocked =>
      rcases hBlocked with âŸ¨_, _, _, _, _, hNoPlanck, _, hNoPromotion, _, hNoFullâŸ©
      exact âŸ¨hNoPlanck, hNoPromotion, hNoFullâŸ©
...
theorem cleared_v2_allows_handshake_only
  ...
  rcases h with âŸ¨_, _, _, _, _, _, _, _, _, _, hHandshake, _, hNoLikelihood, _, _, _âŸ©
  exact âŸ¨hHandshake, hNoLikelihoodâŸ©
