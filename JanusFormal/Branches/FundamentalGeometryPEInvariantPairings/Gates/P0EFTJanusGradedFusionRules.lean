...
/-- Low-spin labels used by the Janus sector audit. -/
...
/-- A scalar occurs in a bilinear product exactly for equal self-dual labels. -/
def Spin3BilinearSinglet
    (first second : TwiceSpin) : Prop :=
  first = second

@[simp] theorem scalar_scalar_bilinear_singlet :
    Spin3BilinearSinglet scalarSpin scalarSpin := by rfl

@[simp] theorem vector_vector_bilinear_singlet :
    Spin3BilinearSinglet vectorSpin vectorSpin := by rfl

@[simp] theorem spin_two_spin_two_bilinear_singlet :
    Spin3BilinearSinglet spinTwoSpin spinTwoSpin := by rfl

@[simp] theorem spinor_spinor_bilinear_singlet :
    Spin3BilinearSinglet fundamentalSpinorSpin fundamentalSpinorSpin := by rfl
...
@[simp] theorem vector_vector_scalar_trilinear_singlet :
    Spin3TripleSinglet vectorSpin vectorSpin scalarSpin := by native_decide

@[simp] theorem vector_vector_spin_two_trilinear_singlet :
    Spin3TripleSinglet vectorSpin vectorSpin spinTwoSpin := by native_decide

@[simp] theorem spin_two_cubic_trilinear_singlet :
    Spin3TripleSinglet spinTwoSpin spinTwoSpin spinTwoSpin := by native_decide

@[simp] theorem vector_cubic_so3_trilinear_singlet :
    Spin3TripleSinglet vectorSpin vectorSpin vectorSpin := by native_decide

@[simp] theorem spinor_spinor_scalar_trilinear_singlet :
    Spin3TripleSinglet
      fundamentalSpinorSpin fundamentalSpinorSpin scalarSpin := by native_decide

@[simp] theorem spinor_spinor_vector_trilinear_singlet :
    Spin3TripleSinglet
      fundamentalSpinorSpin fundamentalSpinorSpin vectorSpin := by native_decide

@[simp] theorem spinor_spinor_spin_two_not_trilinear_singlet :
    Not (Spin3TripleSinglet
      fundamentalSpinorSpin fundamentalSpinorSpin spinTwoSpin) := by native_decide
...
def zeroGrade : QuantumGrade :=
  { inversion := 0, z4 := 0, u1 := 0, ghost := 0, grassmann := 0 }
...
@[simp] theorem grade_pairs_with_dual
  ...
  unfold GradeNeutral tensorGrade dualGrade zeroGrade
  apply QuantumGrade.ext <;> ring
...
@[simp] theorem normal_pair_neutral :
    GradeNeutral (tensorGrade normalModeGrade normalModeGrade) := by native_decide

@[simp] theorem normal_periodic_mixing_not_neutral :
    Not (GradeNeutral
      (tensorGrade normalModeGrade periodicEvenGrade)) := by native_decide

@[simp] theorem quarter_conjugate_pair_neutral :
    GradeNeutral
      (tensorGrade positiveQuarterSpinorGrade
        negativeQuarterSpinorGrade) := by native_decide

@[simp] theorem same_positive_quarter_pair_not_neutral :
    Not (GradeNeutral
      (tensorGrade positiveQuarterSpinorGrade
        positiveQuarterSpinorGrade)) := by native_decide

@[simp] theorem ghost_antighost_pair_neutral :
    GradeNeutral (tensorGrade ghostGrade antighostGrade) := by native_decide
...
@[simp] theorem normal_linear_term_forbidden : Not (4 ∣ 2 * 1) := by norm_num
@[simp] theorem normal_quadratic_term_allowed : 4 ∣ 2 * 2 := by norm_num
@[simp] theorem normal_cubic_term_forbidden : Not (4 ∣ 2 * 3) := by norm_num
@[simp] theorem normal_quartic_term_allowed : 4 ∣ 2 * 4 := by norm_num

@[simp] theorem normal_times_conjugate_spinor_bilinear_forbidden :
    Not (CountNeutral 1 1 1) := by norm_num [CountNeutral]

@[simp] theorem normal_squared_times_conjugate_spinor_bilinear_allowed :
    CountNeutral 2 1 1 := by norm_num [CountNeutral]

@[simp] theorem normal_times_same_positive_spinors_still_forbidden :
    Not (CountNeutral 1 2 0) := by norm_num [CountNeutral]
...
@[simp] theorem spinor_four_point_has_two_raw_channels :
    IdenticalFourPointChannelCount fundamentalSpinorSpin = 2 := by rfl

@[simp] theorem vector_four_point_has_three_raw_channels :
    IdenticalFourPointChannelCount vectorSpin = 3 := by rfl

@[simp] theorem spin_two_four_point_has_five_raw_channels :
    IdenticalFourPointChannelCount spinTwoSpin = 5 := by rfl
