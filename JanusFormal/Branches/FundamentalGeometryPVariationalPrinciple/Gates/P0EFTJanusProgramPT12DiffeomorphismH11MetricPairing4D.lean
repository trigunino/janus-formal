...
namespace P0EFTJanusProgramPT12DiffeomorphismH11MetricPairing4D
...
def metricPhysicalSmoothTest (test : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod) :
  ...
  diagonalExtendedBulkL2SmoothEmbedding period hPeriod (globalCandidateAMetricBySector period hPeriod data)
    couplings.matterMassSquared data analysis (diffeomorphismCore period hPeriod configuration analysis test)

variable (test field : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod)

theorem diffeomorphismH11_metric_pairing :
    ...
    strongPhysicalBRSTMetricHessian period hPeriod configuration data analysis realization
      plusBase minusBase hBase test field := by
  rw [real_inner_comm, diffeomorphismH11Smooth_pairing]
  change physical.form
    ...
    (diagonalExtendedBulkL2SmoothEmbedding period hPeriod (globalCandidateAMetricBySector period hPeriod data)
      couplings.matterMassSquared data analysis (diffeomorphismCore period hPeriod configuration analysis test)) = _
  rw [actualDiffeomorphismInclusion_smooth, physical.symmetric]
  exact physical.smooth_agreement _ _

theorem diffeomorphismH11_metric_pairing_eq_interaction_add_covector :
    inner Real (metricPhysicalSmoothTest period hPeriod configuration data analysis test)
      (diffeomorphismH11Smooth period hPeriod configuration data analysis realization
        plusBase minusBase hBase hCenter physical field) =
    strongInteractionBRSTHessian period hPeriod configuration data analysis realization
      plusBase minusBase hBase test field +
    strongEinsteinMaxwellBRSTCovector period hPeriod configuration plusBase minusBase test
      (globalCandidateAMetricBySector period hPeriod data .plus) (couplings := couplings)
      (diffeomorphismL2Smooth period hPeriod (globalCandidateAMetricBySector period hPeriod data .plus) field) := by
  rw [diffeomorphismH11_metric_pairing]
  exact strongPhysicalBRSTMetricHessian_eq_interaction_add_covector period hPeriod configuration data analysis realization
    plusBase minusBase hBase test field (globalCandidateAMetricBySector period hPeriod data .plus)

theorem diffeomorphismH11_metric_pairing_sub_interaction_bound :
    ‖inner Real (metricPhysicalSmoothTest period hPeriod configuration data analysis test)
        (diffeomorphismH11Smooth period hPeriod configuration data analysis realization
          plusBase minusBase hBase hCenter physical field) -
      strongInteractionBRSTHessian period hPeriod configuration data analysis realization
  ...
  rw [diffeomorphismH11_metric_pairing]
  exact strongPhysicalBRSTMetricHessian_sub_interaction_metric_bound period hPeriod configuration data analysis realization
    plusBase minusBase hBase test field (globalCandidateAMetricBySector period hPeriod data .plus)

end
end
end P0EFTJanusProgramPT12DiffeomorphismH11MetricPairing4D
end JanusFormal
