...
import JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple.Gates.P0EFTJanusProgramPT12ClosedRangeEquation4D

/-! Closed quotient realization of the full H11 adjoint, with exact lifted domain. -/
namespace JanusFormal
namespace P0EFTJanusProgramPT12DiffeomorphismH11QuotientAdjoint4D
...
theorem diffeomorphismGraphToL2_adjoint_injective :
    Function.Injective (diffeomorphismGraphToL2 period hPeriod (globalCandidateAMetricBySector period hPeriod data)).adjoint := by
  intro first second hEqual
  apply ext_inner_right Real
  intro test
  refine (diffeomorphismGraphToL2_denseRange period hPeriod (globalCandidateAMetricBySector period hPeriod data)).induction_on test ?_ ?_
  · apply isClosed_eq <;> fun_prop
  · intro graph
    rw [← ContinuousLinearMap.adjoint_inner_left, hEqual, ContinuousLinearMap.adjoint_inner_left]

theorem diffeomorphismH11GraphTranspose_jointNull :
    jointGhostLLNullSpace period hPeriod configuration data analysis ≤ (diffeomorphismH11GraphTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical).ker := by
  intro input hInput
  change actualDiffeomorphismReadout period hPeriod configuration data analysis
    (strongPhysicalRiesz (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical input) = 0
  rw [show strongPhysicalRiesz (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical input = 0 from
    jointNull_le_physicalRiesz_kernel period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical hInput, map_zero]

def diffeomorphismH11QuotientTranspose : JointGhostLLQuotient period hPeriod configuration data analysis →L[Real]
    ActualDiffeomorphismHilbert period hPeriod configuration data :=
  (jointGhostLLNullSpace period hPeriod configuration data analysis).liftQL (diffeomorphismH11GraphTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical)
    (diffeomorphismH11GraphTranspose_jointNull period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical)

theorem diffeomorphismH11QuotientTranspose_mk (input : CommonAugmentedHilbert period hPeriod configuration data analysis) :
    diffeomorphismH11QuotientTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical ((jointGhostLLNullSpace period hPeriod configuration data analysis).mkQ input) =
      diffeomorphismH11GraphTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical input := rfl

/-- Descent of the entire actual H11 adjoint through the joint null space. -/
def diffeomorphismH11QuotientAdjoint : JointGhostLLQuotient period hPeriod configuration data analysis →ₗ.[Real] DiffeomorphismL2 period hPeriod (globalCandidateAMetricBySector period hPeriod data .plus) :=
  closedRangeEquation (diffeomorphismH11QuotientTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical) (diffeomorphismGraphToL2 period hPeriod (globalCandidateAMetricBySector period hPeriod data)).adjoint

theorem diffeomorphismH11QuotientAdjoint_graph_iff (input : JointGhostLLQuotient period hPeriod configuration data analysis) (output : DiffeomorphismL2 period hPeriod (globalCandidateAMetricBySector period hPeriod data .plus)) :
    (input, output) ∈ (diffeomorphismH11QuotientAdjoint period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical).graph ↔
      (diffeomorphismGraphToL2 period hPeriod (globalCandidateAMetricBySector period hPeriod data)).adjoint output = diffeomorphismH11QuotientTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical input :=
  closedRangeEquation_graph_iff (diffeomorphismH11QuotientTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical)
    (diffeomorphismGraphToL2 period hPeriod (globalCandidateAMetricBySector period hPeriod data)).adjoint
    (diffeomorphismGraphToL2_adjoint_injective period hPeriod configuration data) input output

theorem diffeomorphismH11QuotientAdjoint_isClosed :
    (diffeomorphismH11QuotientAdjoint period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical).IsClosed :=
  closedRangeEquation_isClosed (diffeomorphismH11QuotientTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical)
    (diffeomorphismGraphToL2 period hPeriod (globalCandidateAMetricBySector period hPeriod data)).adjoint
    (diffeomorphismGraphToL2_adjoint_injective period hPeriod configuration data)

theorem diffeomorphismH11QuotientAdjoint_mk_graph_iff (input : CommonAugmentedHilbert period hPeriod configuration data analysis) (output : DiffeomorphismL2 period hPeriod (globalCandidateAMetricBySector period hPeriod data .plus)) :
    ((jointGhostLLNullSpace period hPeriod configuration data analysis).mkQ input, output) ∈ (diffeomorphismH11QuotientAdjoint period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical).graph ↔
  ...
  rw [diffeomorphismH11QuotientAdjoint_graph_iff, diffeomorphismH11QuotientTranspose_mk,
    diffeomorphismH11Adjoint_graph_iff_graphTranspose]

theorem diffeomorphismH11QuotientAdjoint_mk_domain_iff (input : CommonAugmentedHilbert period hPeriod configuration data analysis) :
    (jointGhostLLNullSpace period hPeriod configuration data analysis).mkQ input ∈ (diffeomorphismH11QuotientAdjoint period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical).domain ↔
      input ∈ (diffeomorphismH11Adjoint period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical).domain := by
  rw [show diffeomorphismH11QuotientAdjoint period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical =
    closedRangeEquation (diffeomorphismH11QuotientTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical) (diffeomorphismGraphToL2 period hPeriod (globalCandidateAMetricBySector period hPeriod data)).adjoint from rfl,
    closedRangeEquation_domain_iff _ _ (diffeomorphismGraphToL2_adjoint_injective period hPeriod configuration data),
    diffeomorphismH11QuotientTranspose_mk, diffeomorphismH11Adjoint_domain_iff_graphRange]

end
end
end P0EFTJanusProgramPT12DiffeomorphismH11QuotientAdjoint4D
end JanusFormal
