import JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple.Gates.P0EFTJanusFullMatterRobinTrueLLTaylorPolarizationEnrichedD9Observation4D

namespace JanusFormal
namespace P0EFTJanusFullMatterRobinTrueLLTaylorPolarizationActiveQuotient4D
...
open P0EFTJanusFullMatterRobinTrueLLTaylorPolarizationEnrichedD9Observation4D
...
/-- Polarization on two quotient classes, expressed using the explicit chosen
representatives.  No additive structure on `ActiveQuotient` is asserted. -/
    ...
    quotientHessian period hPeriod matterData kPlus kMinus robinMeasure frame llMeasure fields
      ...
      (1 / 2 : Real) *
        (fullMatterRobinTrueLLTaylorC2 period hPeriod matterData kPlus kMinus robinMeasure
            frame llMeasure fields (addDirection period hPeriod x y) -
         fullMatterRobinTrueLLTaylorC2 period hPeriod matterData kPlus kMinus robinMeasure
            frame llMeasure fields (subDirection period hPeriod x y)) := by
  rw [quotientHessian_mk,
    assembledHessian_eq_difference_C2 period hPeriod matterData kPlus kMinus
      robinMeasure frame llMeasure fields x y]

/-- The same quotient-class value reconstructed from the enriched-D9 diagonal
readings of the explicit sum and difference representatives. -/
theorem quotientHessian_mk_eq_difference_enrichedD9_diagonal
    ...
    quotientHessian period hPeriod matterData kPlus kMinus robinMeasure frame llMeasure fields
        ⟦x⟧ ⟦y⟧ =
      (1 / 4 : Real) *
        (enrichedD9ActiveHessian period hPeriod matterData kPlus kMinus robinMeasure frame
            llMeasure fields
  ...
  rw [quotientHessian_mk]
  exact mixedHessian_eq_difference_enrichedD9_diagonal_C2 period hPeriod matterData
    kPlus kMinus robinMeasure frame llMeasure fields sector column point x y

end
end P0EFTJanusFullMatterRobinTrueLLTaylorPolarizationActiveQuotient4D
end JanusFormal
