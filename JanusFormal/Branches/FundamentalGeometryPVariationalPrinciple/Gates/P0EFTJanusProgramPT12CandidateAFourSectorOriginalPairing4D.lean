...
namespace P0EFTJanusProgramPT12CandidateAFourSectorOriginalPairing4D
...
/-- Actual completed matter and full LL representatives before taking quotients. -/
def candidateAFourSectorMatterLL
    (x : CandidateAFourSectorSmoothFields period hPeriod configuration analysis) :
    ActualMatterLLHilbert period hPeriod configuration data analysis :=
  WithLp.toLp 2 (matterFiniteGraphEmbedding period hPeriod couplings.matterMassSquared x.2,
    globalCandidateAFullLLSmoothEmbedding period hPeriod data analysis x.1.2)

/-- The four actual sectors in the original common augmented Hilbert space. -/
def candidateAFourSectorOriginal
    (x : CandidateAFourSectorSmoothFields period hPeriod configuration analysis) :
    CommonAugmentedHilbert period hPeriod configuration data analysis :=
  candidateACoupledBRSTOriginal period hPeriod configuration data analysis
    (globalCandidateADiagonalDiffeomorphismOffShellSmoothEmbedding period hPeriod
      (globalCandidateAMetricBySector period hPeriod data) x.1.1.1) x.1.1.2 +
  actualMatterLLInclusion period hPeriod configuration data analysis
    (candidateAFourSectorMatterLL period hPeriod configuration data analysis x)

private theorem matterLL_orthogonal_brst
    (z : ActualMatterLLHilbert period hPeriod configuration data analysis)
    (x : CandidateAFourSectorSmoothFields period hPeriod configuration analysis) :
    inner Real (actualMatterLLInclusion period hPeriod configuration data analysis z)
      (candidateACoupledBRSTOriginal period hPeriod configuration data analysis
        (globalCandidateADiagonalDiffeomorphismOffShellSmoothEmbedding period hPeriod
          (globalCandidateAMetricBySector period hPeriod data) x.1.1.1) x.1.1.2) = 0 := by
  rw [actualMatterLLInclusion_inner]
  change inner Real z (0 + 0) = 0
  simp

private theorem matterLL_original_pairing
    (x y : CandidateAFourSectorSmoothFields period hPeriod configuration analysis) :
    inner Real (actualMatterLLGraphRiesz period hPeriod configuration data analysis
      (candidateAFourSectorMatterLL period hPeriod configuration data analysis x))
      (candidateAFourSectorMatterLL period hPeriod configuration data analysis y) =
    programPPrimitiveSpinCMatterGraphForm period hPeriod couplings.matterMassSquared
      (programPPrimitiveSpinCMatterGraphFiniteRealLinearMap period hPeriod couplings.matterMassSquared x.2)
      (programPPrimitiveSpinCMatterGraphFiniteRealLinearMap period hPeriod couplings.matterMassSquared y.2) +
      globalCandidateAFullLLSameActionHessian period hPeriod data x.1.2 y.1.2 := by
  rw [actualMatterLLGraphRiesz_pairing]
  apply congrArg₂ (fun r s : Real => r + s)
  · exact matterFiniteGraphEmbedding_pairing period hPeriod couplings.matterMassSquared x.2 y.2
  · exact (globalCandidateAFullLLGraphForm_apply period hPeriod data analysis _ _).trans
      (globalCandidateAFullLLContinuousHessian_smooth period hPeriod data analysis x.1.2 y.1.2)

/-- Orthogonal matter--LL cancellation gives the exact original four-sector pairing. -/
theorem candidateAFourSectorOriginal_pairing
    (x y : CandidateAFourSectorSmoothFields period hPeriod configuration analysis) :
    inner Real (strongAugmentedRiesz (measure := measure) period hPeriod configuration data analysis realization
      ...
      (candidateAFourSectorOriginal period hPeriod configuration data analysis y) =
    (inner Real (strongAugmentedRiesz (measure := measure) period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical
      (candidateACoupledBRSTOriginal period hPeriod configuration data analysis
        (globalCandidateADiagonalDiffeomorphismOffShellSmoothEmbedding period hPeriod
      ...
      (candidateACoupledBRSTOriginal period hPeriod configuration data analysis
        (globalCandidateADiagonalDiffeomorphismOffShellSmoothEmbedding period hPeriod
          (globalCandidateAMetricBySector period hPeriod data) y.1.1.1) y.1.1.2) +
      globalCandidateAFullLLSameActionHessian period hPeriod data x.1.2 y.1.2) +
    programPPrimitiveSpinCMatterGraphForm period hPeriod couplings.matterMassSquared
      (programPPrimitiveSpinCMatterGraphFiniteRealLinearMap period hPeriod couplings.matterMassSquared x.2)
      (programPPrimitiveSpinCMatterGraphFiniteRealLinearMap period hPeriod couplings.matterMassSquared y.2) := by
  have hSym := (globalCandidateACommonAugmentedRieszOperator_isSelfAdjoint period hPeriod configuration data analysis
    (strongChart (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase)
    (strongBridge (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter)
    physical).isSymmetric
  have h := orthogonalColumn_pairing
    (strongAugmentedRiesz (measure := measure) period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical)
    (actualMatterLLGraphRiesz period hPeriod configuration data analysis)
    (actualMatterLLInclusion period hPeriod configuration data analysis) hSym
    (actualMatterLL_augmented_column period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical)
    _ _ (candidateAFourSectorMatterLL period hPeriod configuration data analysis x)
    (candidateAFourSectorMatterLL period hPeriod configuration data analysis y)
    (fun z => matterLL_orthogonal_brst period hPeriod configuration data analysis z x)
    (fun z => matterLL_orthogonal_brst period hPeriod configuration data analysis z y)
  rw [matterLL_original_pairing] at h
  unfold candidateAFourSectorOriginal
  convert h using 1 <;> ac_rfl

end
end
end P0EFTJanusProgramPT12CandidateAFourSectorOriginalPairing4D
end JanusFormal
