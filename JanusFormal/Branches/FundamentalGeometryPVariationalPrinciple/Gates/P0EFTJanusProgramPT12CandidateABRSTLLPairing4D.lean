...
namespace P0EFTJanusProgramPT12CandidateABRSTLLPairing4D
...
include hZero in
/-- Exact BRST augmented pairing plus the actual full LL Hessian at zero flux. -/
theorem candidateABRSTLLOperator_smooth_sameAction_pairing
    (pair test : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod × GlobalPairedAbelianBRSTState period hPeriod)
    ...
    inner Real (candidateABRSTLLOperator period hPeriod configuration data analysis realization
      ...
      (candidateABRSTLLSmoothDomain period hPeriod configuration data analysis realization
        plusBase minusBase hBase hCenter physical hMetric hWeights pair first))
      (candidateABRSTLLSmooth period hPeriod configuration data analysis (test, second)) =
    inner Real (strongAugmentedRiesz (measure := measure) period hPeriod configuration data analysis realization
      ...
      (candidateACoupledBRSTOriginal period hPeriod configuration data analysis
        (globalCandidateADiagonalDiffeomorphismOffShellSmoothEmbedding period hPeriod
          (globalCandidateAMetricBySector period hPeriod data) test.1) test.2) +
      globalCandidateAFullLLSameActionHessian period hPeriod data first second := by
  rw [candidateABRSTLLOperator_pairing]
  apply congrArg₂ (fun r s : Real => r + s)
  · exact (candidateAReducedBRSTOperator_pairing_project period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights
      ...
      (candidateACoupledBRSTSmooth period hPeriod configuration data test)).trans
        (candidateACoupledBRSTOperator_state_actual period hPeriod configuration data analysis realization
          plusBase minusBase hBase hCenter physical _ _ pair.2 test.2)
  · exact quotientLLFriedrichs_smooth_sameAction_pairing period hPeriod analysis data hZero first second

end
end
end P0EFTJanusProgramPT12CandidateABRSTLLPairing4D
end JanusFormal
