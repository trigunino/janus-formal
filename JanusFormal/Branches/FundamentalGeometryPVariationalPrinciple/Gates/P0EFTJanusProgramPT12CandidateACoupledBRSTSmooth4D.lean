...
namespace P0EFTJanusProgramPT12CandidateACoupledBRSTSmooth4D
...
/-- Simultaneous realization of both genuine smooth BRST field families. -/
def candidateACoupledBRSTSmooth :
    (GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod × GlobalPairedAbelianBRSTState period hPeriod) →ₗ[Real]
      CandidateACoupledBRSTHilbert period hPeriod configuration data where
  toFun pair := candidateACoupledBRSTState period hPeriod configuration data
  ...
  map_add' x y := by
    ...
    exact Prod.ext
      ((globalCandidateADiagonalDiffeomorphismOffShellSmoothEmbedding period hPeriod _).map_add _ _)
      ((candidateAAbelianMixedSmoothLinearMap period hPeriod data).map_add _ _)
  map_smul' r x := by
    apply WithLp.ofLp_injective 2
    exact Prod.ext
      ((globalCandidateADiagonalDiffeomorphismOffShellSmoothEmbedding period hPeriod _).map_smul r _)
      ((candidateAAbelianMixedSmoothLinearMap period hPeriod data).map_smul r _)

theorem candidateACoupledBRSTSmooth_injective :
    Function.Injective (candidateACoupledBRSTSmooth period hPeriod configuration data) := by
  intro x y h
  exact Prod.ext
    (globalCandidateADiagonalDiffeomorphismOffShellSmoothEmbedding_injective period hPeriod _ (congrArg WithLp.fst h))
    (candidateAAbelianMixedSmooth_injective period hPeriod data (congrArg WithLp.snd h))

/-- Hilbert density only; this does not assert density in the operator graph norm. -/
theorem candidateACoupledBRSTSmooth_denseRange :
    DenseRange (candidateACoupledBRSTSmooth period hPeriod configuration data) := by
  have h := (globalCandidateADiagonalDiffeomorphismOffShellSmoothEmbedding_denseRange period hPeriod
    (globalCandidateAMetricBySector period hPeriod data)).prodMap
      (candidateAAbelianMixedSmooth_denseRange period hPeriod data)
  exact (WithLp.prodContinuousLinearEquiv 2 Real
    (ActualDiffeomorphismHilbert period hPeriod configuration data)
    (CandidateAAbelianMixedHilbert period hPeriod data)).symm.surjective.denseRange.comp h
    (WithLp.prodContinuousLinearEquiv 2 Real
      (ActualDiffeomorphismHilbert period hPeriod configuration data)
      (CandidateAAbelianMixedHilbert period hPeriod data)).symm.continuous

theorem candidateACoupledBRSTSmooth_mem
...
end P0EFTJanusProgramPT12CandidateACoupledBRSTSmooth4D
end JanusFormal
