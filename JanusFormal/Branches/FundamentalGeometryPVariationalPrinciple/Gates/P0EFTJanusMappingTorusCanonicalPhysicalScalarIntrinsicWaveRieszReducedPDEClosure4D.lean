...
def compatibilityData
  ...
  data.geometric.toCanonicalLocalDivergenceData.toCanonicalWaveCauchyJetData
    |>.toCanonicalCauchyJetCompatibilityData

/-- Explicit geometric Cauchy-jet data. -/
def jetGeometric
    (data : CanonicalPhysicalScalarIntrinsicWaveRieszReducedPDEData
      period hPeriod massSquared) :=
  data.compatibilityData.toCauchyJetGeometricData
...
def eulerEstimate
    (data : CanonicalPhysicalScalarIntrinsicWaveRieszReducedPDEData
      period hPeriod massSquared) :=
  data.eulerCoefficients.toEulerProductEstimateData
...
def rieszBoundaryData
  ...
  geometric := data.compatibilityData
...
theorem completedBoundaryTrace_extension
  ...
  data.rieszBoundaryData.completedBoundaryTrace_extension boundary
...
theorem certificate
  ...
  ⟨data.geometric.intrinsicWave.toWaveAtlasNaturality,
    data.rieszBoundaryData.minimalCoreDense,
    data.rieszBoundaryData.graphInclusion_injective,
    data.rieszBoundaryData.boundedSmoothExtension.rieszBoundaryTrace_surjective,
    data.completedBoundaryTrace_extension,
    data.rieszBoundaryData.boundedSmoothExtension.rieszBoundaryTrace_smooth⟩
