...
namespace P0EFTJanusProgramPT12NativeMaxwellExplicitHessian4D
...
/-- All derivatives of the native chart have been replaced by their computed values. -/
def nativeMaxwellExplicitHessianDensity
    (first second : RegularGeneralMetricC2Core period hPeriod metric) (point : EffectiveQuotient period hPeriod) : Real :=
  let input := (metric.volume point, nativeMobileMaxwellJet period hPeriod metric potential 0 point)
  maxwellSymbolHessian input
    (nativeMaxwellJetVelocity period hPeriod metric potential point first)
    (nativeMaxwellJetVelocity period hPeriod metric potential point second) +
  maxwellSymbolGradient input (nativeMaxwellJetAcceleration period hPeriod metric potential point first second)

theorem nativeMobileMaxwellJetHessianDensity_eq_explicit
    (first second : RegularGeneralMetricC2Core period hPeriod metric) (point : EffectiveQuotient period hPeriod) :
    nativeMobileMaxwellJetHessianDensity period hPeriod metric potential first second point =
      nativeMaxwellExplicitHessianDensity period hPeriod metric potential first second point := by
  simp only [nativeMobileMaxwellJetHessianDensity, nativeMobileMaxwellJet_fderiv_zero,
    nativeMobileMaxwellJet_hessian_zero, nativeMaxwellExplicitHessianDensity]

attribute [local irreducible] nativeMobileMaxwellHessian nativeMobileMaxwellJetHessianDensity
  nativeMaxwellExplicitHessianDensity

/-- Exact integral formula for the actual metric-metric Maxwell Hessian. -/
theorem nativeMobileMaxwellHessian_eq_explicitIntegral
    ...
    nativeMobileMaxwellHessian period hPeriod metric potential (first, 0) (second, 0) =
      ∫ point, nativeMaxwellExplicitHessianDensity period hPeriod metric potential first second point
        ∂intrinsicCanonicalLorentzVolumeMeasure period hPeriod := by
  refine (nativeMobileMaxwellHessian_eq_jetIntegral period hPeriod metric potential first second).trans ?_
  apply integral_congr_ae
  filter_upwards [] with point
  exact nativeMobileMaxwellJetHessianDensity_eq_explicit period hPeriod metric potential first second point

end
end P0EFTJanusProgramPT12NativeMaxwellExplicitHessian4D
end JanusFormal
