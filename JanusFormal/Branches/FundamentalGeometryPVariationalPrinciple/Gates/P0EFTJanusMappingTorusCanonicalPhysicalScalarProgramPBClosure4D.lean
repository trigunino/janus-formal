...
5. dense smooth boundary cores and a bounded Cauchy extension construct the
   surjective completed trace;
...
The fully reduced endpoint is
`P0EFTJanusMappingTorusCanonicalPhysicalScalarFullyReducedAnalyticClosure4D`.
It does not assume smooth surjectivity, closability, domain density, completed
trace surjectivity, shifted surjectivity, semiboundedness or compact resolvent as
independent inputs; all of these are constructed from the concrete data above.
-/
