...
namespace P0EFTJanusProgramPT12CandidateAReducedBRSTMinimal4D
...
def candidateAReducedDiffeomorphismSmooth :
...
theorem candidateAReducedDiffeomorphismSmooth_denseRange :
    DenseRange (candidateAReducedDiffeomorphismSmooth period hPeriod configuration data) :=
...
theorem candidateAReducedBRSTSmooth_range :
    (candidateAReducedBRSTSmooth period hPeriod configuration data).range =
      productCore (candidateAReducedDiffeomorphismSmooth period hPeriod configuration data).range
...
theorem candidateAReducedDiffeomorphismSignedRiesz_hasCore :
    ((reducedDiffeomorphismSignedRiesz period hPeriod configuration data hMetric hWeights).toPMap ⊤).HasCore
      (candidateAReducedDiffeomorphismSmooth period hPeriod configuration data).range :=
  bounded_range_hasCore _
    (bounded_toPMap_selfAdjoint _ (ContinuousLinearMap.isSelfAdjoint_iff_isSymmetric.mp
      (reducedDiffeomorphismSignedRiesz_isSelfAdjoint period hPeriod configuration data hMetric hWeights))).isClosed _
    (candidateAReducedDiffeomorphismSmooth_denseRange period hPeriod configuration data)

/-- The actual minimal BRST on the closed shared-ghost quotient, including all H11 blocks. -/
def candidateAReducedBRSTMinimal : CandidateAReducedBRSTHilbert period hPeriod configuration data →ₗ.[Real]
...
theorem candidateAReducedBRSTMinimal_isClosed :
    (candidateAReducedBRSTMinimal period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights).IsClosed :=
  coupledBRSTOperator_isClosed _ _ _ _ _
    (bounded_toPMap_selfAdjoint _ (ContinuousLinearMap.isSelfAdjoint_iff_isSymmetric.mp
      (reducedDiffeomorphismSignedRiesz_isSelfAdjoint period hPeriod configuration data hMetric hWeights))).isClosed
    (candidateAAbelianMixedMinimal_isClosed period hPeriod data)

theorem candidateAReducedBRSTMinimal_hasCore :
    (candidateAReducedBRSTMinimal period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights).HasCore
      (candidateAReducedBRSTSmooth period hPeriod configuration data).range := by
  rw [candidateAReducedBRSTSmooth_range]
  exact coupledBRSTOperator_hasCore _ _ _ _ _
    (bounded_toPMap_selfAdjoint _ (ContinuousLinearMap.isSelfAdjoint_iff_isSymmetric.mp
      (reducedDiffeomorphismSignedRiesz_isSelfAdjoint period hPeriod configuration data hMetric hWeights))).isClosed
    (candidateAAbelianMixedMinimal_isClosed period hPeriod data) _ _
    (candidateAReducedDiffeomorphismSignedRiesz_hasCore period hPeriod configuration data hMetric hWeights)
    (candidateAAbelianMixedMinimal_hasCore period hPeriod data)

theorem candidateAReducedBRST_smoothClosure_eq_minimal :
    ((candidateAReducedBRSTOperator period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights).domRestrict
      (candidateAReducedBRSTSmooth period hPeriod configuration data).range).closure =
    candidateAReducedBRSTMinimal period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights := by
  rw [candidateAReducedBRSTSmooth_range]
  unfold candidateAReducedBRSTOperator
...
theorem candidateAReducedBRSTMinimal_project_domain_iff
  ...
  change _ ∈ (coupledBRSTOperator _ _ _ _ _).domain ↔ _ ∈ (coupledBRSTOperator _ _ _ _ _).domain
  rw [coupledBRSTOperator_domain_iff, coupledBRSTOperator_domain_iff]
  rfl

end
end
end P0EFTJanusProgramPT12CandidateAReducedBRSTMinimal4D
end JanusFormal
