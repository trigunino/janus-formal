...
namespace P0EFTJanusProgramPT12DiffeomorphismH11MetricDependence4D
...
theorem diffeomorphismCore_physicalTangent_eq_of_metric_eq
    ...
    diagonalExtendedBulkMinimalPhysicalTangentLinearMap period hPeriod configuration data analysis
      (diffeomorphismCore period hPeriod configuration analysis first) =
    diagonalExtendedBulkMinimalPhysicalTangentLinearMap period hPeriod configuration data analysis
      (diffeomorphismCore period hPeriod configuration analysis second) := by
  change (diagonalDiffeomorphismGaugeFixedTangentLinearMap period hPeriod configuration first +
    globalPairedAbelianBRSTStateGaugeFixedTangentLinearMap period hPeriod configuration data 0 +
    extendedMatterGaugeFixedTangentLinearMap period hPeriod configuration 0 +
    extendedLLGaugeFixedTangentLinearMap period hPeriod configuration analysis 0).1 =
    (diagonalDiffeomorphismGaugeFixedTangentLinearMap period hPeriod configuration second +
    globalPairedAbelianBRSTStateGaugeFixedTangentLinearMap period hPeriod configuration data 0 +
    extendedMatterGaugeFixedTangentLinearMap period hPeriod configuration 0 +
    extendedLLGaugeFixedTangentLinearMap period hPeriod configuration analysis 0).1
  simp only [map_zero, add_zero]
  change globalMetricPerturbationMinimalPhysicalTangentLinearMap period hPeriod configuration.physical
    first.metricPerturbation = globalMetricPerturbationMinimalPhysicalTangentLinearMap period hPeriod
      configuration.physical second.metricPerturbation
  rw [hMetric]

theorem diffeomorphismPhysicalRiesz_eq_of_metric_eq
    (first second : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod)
    (hMetric : first.metricPerturbation = second.metricPerturbation) :
    strongPhysicalRiesz (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical
    ...
    strongPhysicalRiesz (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical
      (actualDiffeomorphismInclusion period hPeriod configuration data analysis
        (globalCandidateADiagonalDiffeomorphismOffShellSmoothEmbedding period hPeriod
          (globalCandidateAMetricBySector period hPeriod data) second)) := by
  have hTangent := diffeomorphismCore_physicalTangent_eq_of_metric_eq period hPeriod
    configuration data analysis first second hMetric
  have hCore := physicalRiesz_core_zero_of_hessian_column_zero period hPeriod configuration data analysis
    (strongChart (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase)
    (strongBridge (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter)
    physical (diffeomorphismCore period hPeriod configuration analysis (first - second)) (by
      intro test
      unfold diagonalExtendedBulkMinimalPhysicalLocalActionHessianOnCore
      rw [map_sub, map_sub, hTangent, sub_self]
      simp)
  have hZero := (congrArg (strongPhysicalRiesz (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical)
    (actualDiffeomorphismInclusion_smooth period hPeriod configuration data analysis (first - second))).trans hCore
  rw [map_sub, map_sub, map_sub] at hZero
  exact sub_eq_zero.mp hZero

theorem diffeomorphismPhysicalRiesz_metricTransfer
    (state : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod) :
    strongPhysicalRiesz (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical
      (actualDiffeomorphismInclusion period hPeriod configuration data analysis
        (globalCandidateADiagonalDiffeomorphismOffShellSmoothEmbedding period hPeriod
          (globalCandidateAMetricBySector period hPeriod data)
          (diffeomorphismMetricTransfer period hPeriod
            (globalCandidateAMetricBySector period hPeriod data .plus) state))) =
    strongPhysicalRiesz (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical
      (actualDiffeomorphismInclusion period hPeriod configuration data analysis
        (globalCandidateADiagonalDiffeomorphismOffShellSmoothEmbedding period hPeriod
          (globalCandidateAMetricBySector period hPeriod data) state)) :=
  diffeomorphismPhysicalRiesz_eq_of_metric_eq period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical _ state
    (diffeomorphismMetricTransfer_metric period hPeriod _ state)

end
end
end P0EFTJanusProgramPT12DiffeomorphismH11MetricDependence4D
end JanusFormal
