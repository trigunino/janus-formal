...
/-!
# Candidate-A H10--H14 from a principal Gårding operator plus a bounded perturbation

The augmented Hessian is naturally assembled as an elliptic graph operator and
bounded physical terms.  This façade therefore accepts a reference operator
with a named-mode Gårding estimate and a bounded perturbation whose norm is
strictly smaller than the reference coercivity constant.  The full Gårding
estimate, actual-kernel gap and H10--H14 terminal certificate are derived.
-/

namespace JanusFormal
namespace P0EFTJanusProgramPGlobalHessianCanonicalSixPerturbativeGardingFrontier4D
...
private abbrev PerturbativeHilbert
...
@[implicit_reducible] private def perturbativeNormedAddCommGroup
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    (configuration : GlobalGaugeFixedFieldConfiguration period hPeriod)
...
@[implicit_reducible] private def perturbativeInnerProductSpace
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    (configuration : GlobalGaugeFixedFieldConfiguration period hPeriod)
...
@[implicit_reducible] private def perturbativeNormedSpace
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    (configuration : GlobalGaugeFixedFieldConfiguration period hPeriod)
...
@[implicit_reducible] private def perturbativeModule
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    (configuration : GlobalGaugeFixedFieldConfiguration period hPeriod)
...
@[implicit_reducible] private def perturbativeCompleteSpace
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    (configuration : GlobalGaugeFixedFieldConfiguration period hPeriod)
...
/-- Candidate-A terminal from a reference Gårding estimate and a small bounded
perturbation. -/
def global_candidateA_hessian_canonicalSix_perturbativeGarding_frontier_gate
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    ...
    (reference perturbation :
      PerturbativeHilbert period hPeriod configuration data analysis →L[Real]
        PerturbativeHilbert period hPeriod configuration data analysis)
    (perturbative : FiniteKernelNamedReferenceGardingPerturbationData
      ...
      (measure := measure) configuration data analysis
        ...
        (globalCandidateACanonicalSixPhysicalExtension_of_chartBound period
          hPeriod (measure := measure) configuration data analysis einsteinScale hTransverse family
            chartBound)
        ZeroMode :=
    { garding := perturbative.toNamedGarding
...
/-- The perturbative terminal still has two analytic packets after the local
family: the chart bound and the principal-plus-bounded Gårding package. -/
theorem global_candidateA_hessian_canonicalSix_perturbativeGarding_two_inputs :
    Nonempty (Unit × Unit) :=
  ⟨((), ())⟩

end
end P0EFTJanusProgramPGlobalHessianCanonicalSixPerturbativeGardingFrontier4D
end JanusFormal
