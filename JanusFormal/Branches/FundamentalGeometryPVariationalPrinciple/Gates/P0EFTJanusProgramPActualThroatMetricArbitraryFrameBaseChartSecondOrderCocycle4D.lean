...
/-!
# Triple metric cocycle for arbitrary throat frame/base-chart pairs
...
varying covariant rank-two transition.

This is local second-order descent data, not yet a global jet-bundle section.
-/

namespace JanusFormal
namespace P0EFTJanusProgramPActualThroatMetricArbitraryFrameBaseChartSecondOrderCocycle4D
...
theorem throatMetricFrameBaseChartTransition_cocycle_eventuallyEq
  ...
  have hCocycle :
      (throatCovariantTwoTensorFrameTransitionAt period hPeriod
          ...
          ((extChartAt throatCoverModelWithCorners firstCenter).symm coordinate)).trans
        (throatCovariantTwoTensorFrameTransitionAt period hPeriod
          secondAnchor thirdAnchor
          ((extChartAt throatCoverModelWithCorners firstCenter).symm coordinate)) =
        throatCovariantTwoTensorFrameTransitionAt period hPeriod
          firstAnchor thirdAnchor
          ((extChartAt throatCoverModelWithCorners firstCenter).symm coordinate) := by
    apply ContinuousLinearEquiv.ext
    funext currentTensor
    apply ContinuousLinearMap.ext
    intro first
    apply ContinuousLinearMap.ext
    intro second
    simp only [ContinuousLinearEquiv.trans_apply,
      throatCovariantTwoTensorFrameTransitionAt_apply]
    rw [← hTangent]
    rfl
...
theorem throatMetricFrameBaseChartTransition_value_cocycle
  ...
  exact (throatMetricFrameBaseChartTransition_cocycle_eventuallyEq period hPeriod
    firstAnchor secondAnchor thirdAnchor firstCenter secondCenter thirdCenter
...
theorem throatMetricFrameBaseChartTransition_firstDerivative_cocycle
  ...
  have hProductApplied := congrArg
    (fun derivative : ThroatCoverCoordinates →L[Real]
        (TensorModel →L[Real]
          TensorModel) ↦ derivative direction)
    hProduct
...
theorem throatMetricFrameBaseChartTransition_secondDerivative_cocycle_apply
  ...
  have hGermSecondApplied := congrArg
    (fun derivative : ThroatCoverCoordinates →L[Real]
        (TensorModel →L[Real]
          TensorModel) ↦ derivative first tensor)
    hGermSecond
...
end P0EFTJanusProgramPActualThroatMetricArbitraryFrameBaseChartSecondOrderCocycle4D
end JanusFormal
