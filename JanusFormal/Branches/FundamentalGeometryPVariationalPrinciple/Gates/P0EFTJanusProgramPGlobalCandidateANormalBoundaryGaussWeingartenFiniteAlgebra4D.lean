...
/-!
# Finite regular-frame Gauss--Weingarten algebra

Pure finite-dimensional rearrangement used by the H10 same-action bridge.
It contains no geometric choice: the only hypothesis is symmetry of the
metric coefficients together with the differentiated orthogonality identity.
-/
...
private theorem candidateANormalBoundary_sum_swap_first_fourth
...
private theorem candidateANormalBoundary_sum_swap_second_fourth
    (f : Fin 4 → Fin 4 → Fin 4 → Fin 4 → Real) :
    (∑ a, ∑ b, ∑ c, ∑ d, f a b c d) =
...
/-- Finite regular-frame Gauss--Weingarten rearrangement. A differentiated
orthogonality identity is solved for the covariant acceleration pairing. -/
theorem candidateANormalBoundaryGaussWeingartenFin4Algebra
  ...
  have hFirst := candidateANormalBoundary_sum_swap_first_fourth
    (fun row column regular upper =>
      normal row * outerTangent regular *
        christoffel upper regular row * metric upper column *
          innerTangent column)
  have hSecond := candidateANormalBoundary_sum_swap_second_fourth
    (fun row column regular upper =>
      normal row * outerTangent regular *
        christoffel upper regular column * metric upper row *
          innerTangent column)
