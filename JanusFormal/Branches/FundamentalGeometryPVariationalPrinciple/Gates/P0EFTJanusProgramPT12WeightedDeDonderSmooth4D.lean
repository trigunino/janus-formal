...
namespace JanusFormal.P0EFTJanusProgramPT12WeightedDeDonderSmooth4D
...
def weightedDeDonderSmooth : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod →ₗ[Real]
    DiffeomorphismTripletComponentL2 period hPeriod (metric .plus) 2 :=
  (diffeomorphismTripletL2 period hPeriod (metric .plus) 2 2).rangeRestrict.toLinearMap.comp
    ((hessianSmoothRieszLinearMap period hPeriod reference metric couplings).comp
      (diffeomorphismMetricTransfer period hPeriod (metric .plus)))

def weightedDeDonderTransposeSmooth : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod →ₗ[Real]
    DiffeomorphismMetricL2 period hPeriod (metric .plus) :=
  (diffeomorphismMetricProjection period hPeriod (metric .plus)).rangeRestrict.toLinearMap.comp
    ((hessianSmoothRieszLinearMap period hPeriod reference metric couplings).comp
      (diffeomorphismTripletTransferLinearMap period hPeriod (metric .plus) 2 2))

theorem weightedDeDonderSmooth_pairing
    ...
    inner Real (diffeomorphismMetricSmooth period hPeriod (metric .plus) first)
      (weightedDeDonderTransposeSmooth period hPeriod reference metric couplings second) := by
  change inner Real
    (diffeomorphismTripletL2 period hPeriod (metric .plus) 2 2
      (hessianSmoothRiesz period hPeriod reference metric couplings
        (diffeomorphismMetricTransfer period hPeriod (metric .plus) first)))
    (diffeomorphismTripletL2 period hPeriod (metric .plus) 2 2
      (diffeomorphismL2Smooth period hPeriod (metric .plus) second)) =
    inner Real (diffeomorphismMetricProjection period hPeriod (metric .plus)
      (diffeomorphismL2Smooth period hPeriod (metric .plus) first))
    (diffeomorphismMetricProjection period hPeriod (metric .plus)
      (hessianSmoothRiesz period hPeriod reference metric couplings
        (diffeomorphismTripletTransfer period hPeriod 2 2 second)))
  rw [diffeomorphismTripletL2_pairing, diffeomorphismTripletL2_comp_apply,
    diffeomorphismTripletL2_smooth, hessianSmoothRiesz_symmetric, ← diffeomorphismMetricTransfer_L2]
  have h := diffeomorphismMetricProjection_pairing period hPeriod (metric .plus)
    (diffeomorphismMetricProjection period hPeriod (metric .plus)
      (diffeomorphismL2Smooth period hPeriod (metric .plus) first))
    (hessianSmoothRiesz period hPeriod reference metric couplings
      (diffeomorphismTripletTransfer period hPeriod 2 2 second))
  rw [diffeomorphismMetricProjection_apply_twice] at h
  exact h

theorem weightedDeDonderTransposeSmooth_pairing
    (first second : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod) :
    inner Real (weightedDeDonderTransposeSmooth period hPeriod reference metric couplings first)
      (diffeomorphismMetricSmooth period hPeriod (metric .plus) second) =
    inner Real (diffeomorphismTripletComponentSmooth period hPeriod (metric .plus) 2 first)
      (weightedDeDonderSmooth period hPeriod reference metric couplings second) :=
  (real_inner_comm _ _).trans
    ((weightedDeDonderSmooth_pairing period hPeriod reference metric couplings second first).symm.trans
      (real_inner_comm _ _))

theorem weightedDeDonderSmooth_actual_pairing
    (first second : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod) :
    inner Real (weightedDeDonderSmooth period hPeriod reference metric couplings first)
      (diffeomorphismTripletComponentSmooth period hPeriod (metric .plus) 2 second) =
    globalCandidateADiagonalDiffeomorphismGaugeFermionBRSTPolarizationAction period hPeriod couplings metric
      (diffeomorphismMetricTransfer period hPeriod (metric .plus) first)
      (diffeomorphismTripletTransfer period hPeriod 2 2 second) := by
  change inner Real
    (diffeomorphismTripletL2 period hPeriod (metric .plus) 2 2
      (hessianSmoothRiesz period hPeriod reference metric couplings
        (diffeomorphismMetricTransfer period hPeriod (metric .plus) first)))
...
end JanusFormal.P0EFTJanusProgramPT12WeightedDeDonderSmooth4D