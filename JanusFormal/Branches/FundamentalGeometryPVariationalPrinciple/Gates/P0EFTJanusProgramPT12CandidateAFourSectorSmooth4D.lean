...
namespace P0EFTJanusProgramPT12CandidateAFourSectorSmooth4D
...
abbrev CandidateAFourSectorSmoothFields :=
  ((GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod × GlobalPairedAbelianBRSTState period hPeriod) ×
    GlobalFullLLSmooth period hPeriod analysis) × ProgramPPrimitiveSpinCMatterFiniteCoefficients

def candidateAFourSectorSmooth : CandidateAFourSectorSmoothFields period hPeriod configuration analysis →ₗ[Real]
    CandidateAFourSectorHilbert period hPeriod configuration data :=
  (WithLp.prodContinuousLinearEquiv 2 Real
    (CandidateABRSTLLHilbert period hPeriod configuration data)
    (ProgramPPrimitiveSpinCMatterL2GraphDomain period hPeriod couplings.matterMassSquared)).symm.toLinearMap.comp
      ((candidateABRSTLLSmooth period hPeriod configuration data analysis).prodMap
        (matterFiniteGraphEmbedding period hPeriod couplings.matterMassSquared))

theorem candidateAFourSectorSmooth_denseRange :
    DenseRange (candidateAFourSectorSmooth period hPeriod configuration data analysis) := by
  have h := (candidateABRSTLLSmooth_denseRange period hPeriod configuration data analysis).prodMap
    (matterFiniteGraphEmbedding_denseRange period hPeriod couplings.matterMassSquared)
  exact (WithLp.prodContinuousLinearEquiv 2 Real
    (CandidateABRSTLLHilbert period hPeriod configuration data)
    (ProgramPPrimitiveSpinCMatterL2GraphDomain period hPeriod couplings.matterMassSquared)).symm.surjective.denseRange.comp h
    (WithLp.prodContinuousLinearEquiv 2 Real
      (CandidateABRSTLLHilbert period hPeriod configuration data)
      (ProgramPPrimitiveSpinCMatterL2GraphDomain period hPeriod couplings.matterMassSquared)).symm.continuous
...
def candidateAFourSectorSmoothDomain
    (field : CandidateAFourSectorSmoothFields period hPeriod configuration analysis) :
    (candidateAFourSectorOperator period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights).domain :=
  ⟨WithLp.toLp 2
      ((candidateABRSTLLSmoothDomain period hPeriod configuration data analysis realization
        plusBase minusBase hBase hCenter physical hMetric hWeights field.1.1 field.1.2).val,
        matterFiniteGraphEmbedding period hPeriod couplings.matterMassSquared field.2),
    (productOperator_domain_iff _ _ _).mpr
      ⟨(candidateABRSTLLSmoothDomain period hPeriod configuration data analysis realization
        plusBase minusBase hBase hCenter physical hMetric hWeights field.1.1 field.1.2).property,
        Submodule.mem_top⟩⟩

theorem candidateAFourSectorSmoothDomain_value
    (field : CandidateAFourSectorSmoothFields period hPeriod configuration analysis) :
    (candidateAFourSectorSmoothDomain period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights field).val =
      candidateAFourSectorSmooth period hPeriod configuration data analysis field := by
  apply WithLp.ofLp_injective 2
  exact Prod.ext (candidateABRSTLLSmoothDomain_value period hPeriod configuration data analysis realization
    plusBase minusBase hBase hCenter physical hMetric hWeights field.1.1 field.1.2) rfl

end
end
end P0EFTJanusProgramPT12CandidateAFourSectorSmooth4D
end JanusFormal