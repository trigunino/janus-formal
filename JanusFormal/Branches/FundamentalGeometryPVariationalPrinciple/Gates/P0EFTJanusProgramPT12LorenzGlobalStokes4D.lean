...
namespace P0EFTJanusProgramPT12LorenzGlobalStokes4D
...
theorem globalLorenz_eq_metricCurrentDivergence
    (metric : RegularGeneralLorentzMetric period hPeriod)
    (potential : SmoothAbelianGaugePotential period hPeriod) (component : Fin 2)
    (point : EffectiveQuotient period hPeriod) :
    globalGeneralMetricAbelianLorenzCodifferential period hPeriod metric.metric potential point component =
      canonicalMetricVolumeDivergence period hPeriod metric
        (lorenzRaisedCurrent period hPeriod metric potential component) point := by
  ...
  rw [← witness.coordinate_eq, globalGeneralMetricAbelianLorenzCodifferential_apply,
    ...
    metricCurrentDivergence_eq_local]
  have hField : pulledRegularFrameExpansion period hPeriod metric witness.patch
      (lorenzRaisedCurrent period hPeriod metric potential component) =
      localRaisedAbelianGaugePotential period hPeriod metric.metric potential component witness.patch := by
    funext coordinate
    exact lorenzRaisedCurrent_pullback period hPeriod metric potential component witness.patch coordinate
  rw [hField]

/-- Genuine global integration by parts for the supplied regular metric. -/
theorem globalLorenz_weak_stokes
    (metric : RegularGeneralLorentzMetric period hPeriod)
    (potential : SmoothAbelianGaugePotential period hPeriod) (component : Fin 2)
    ...
    -∫ point, scalarDifferential period hPeriod test point
      (inverseMetricSharp period hPeriod metric.metric point (potential.toFun component point))
      ∂generalLorentzVolumeMeasure period hPeriod metric.metric := by
  simp_rw [globalLorenz_eq_metricCurrentDivergence]
  rw [canonicalMetricVolumeDivergence_weak_stokes]
  congr 1
  apply integral_congr_ae
  filter_upwards [] with point
  rw [lorenzRaisedCurrent_apply]
  rfl

theorem globalLorenz_integral_eq_zero
    (metric : RegularGeneralLorentzMetric period hPeriod)
    (potential : SmoothAbelianGaugePotential period hPeriod) (component : Fin 2) :
    (∫ point, globalGeneralMetricAbelianLorenzCodifferential
      period hPeriod metric.metric potential point component
      ∂generalLorentzVolumeMeasure period hPeriod metric.metric) = 0 := by
  simp_rw [globalLorenz_eq_metricCurrentDivergence]
  exact canonicalMetricVolumeDivergence_integral_eq_zero period hPeriod metric _

/-- The FP component is paired by the true inverse-metric gradient contraction. -/
theorem globalFP_component_green
    (metric : RegularGeneralLorentzMetric period hPeriod)
    ...
    (∫ point, test point * globalGeneralMetricAbelianFaddeevPopov
      period hPeriod metric.metric ghost point component
      ∂generalLorentzVolumeMeasure period hPeriod metric.metric) =
    -∫ point, inverseMetricContraction period hPeriod metric.metric point
      ...
      ∂generalLorentzVolumeMeasure period hPeriod metric.metric :=
  globalLorenz_weak_stokes period hPeriod metric (exactGaugePotential period hPeriod ghost) component test

/-- Symmetry is proved for the actual FP operator in its metric volume. -/
theorem globalFP_component_metric_symmetry
    (metric : RegularGeneralLorentzMetric period hPeriod)
    ...
    (∫ point, first point component * globalGeneralMetricAbelianFaddeevPopov
      period hPeriod metric.metric second point component
      ∂generalLorentzVolumeMeasure period hPeriod metric.metric) =
    ∫ point, second point component * globalGeneralMetricAbelianFaddeevPopov
      period hPeriod metric.metric first point component
      ∂generalLorentzVolumeMeasure period hPeriod metric.metric := by
  have hFirst := globalFP_component_green period hPeriod metric second component
    (ghostComponent period hPeriod first component)
  have hSecond := globalFP_component_green period hPeriod metric first component
  ...
  exact hFirst.trans ((congrArg Neg.neg (integral_congr_ae (Filter.Eventually.of_forall
    (fun point => inverseMetricContraction_comm period hPeriod metric.metric point _ _)))).trans hSecond.symm)

end
end P0EFTJanusProgramPT12LorenzGlobalStokes4D
end JanusFormal
