...
namespace JanusFormal.P0EFTJanusProgramPT12RectangularDoubleAdjoint4D
...
theorem closedRectangularSubmodule_adjoint_adjoint (graph : Submodule Real (E × F))
    (hClosed : IsClosed (graph : Set (E × F))) : graph.adjoint.adjoint = graph := by
  apply le_antisymm
  · intro pair hPair
    let hilbertGraph : Submodule Real (WithLp 2 (E × F)) :=
      graph.comap (WithLp.linearEquiv 2 Real (E × F)).toLinearMap
    have hHilbertClosed : IsClosed (hilbertGraph : Set (WithLp 2 (E × F))) :=
...
theorem closedRectangularOperator_adjoint_adjoint (operator : E →ₗ.[Real] F)
  ...
  rw [LinearPMap.adjoint_graph_eq_graph_adjoint hAdjointDense,
    ...
    closedRectangularSubmodule_adjoint_adjoint operator.graph hClosed]

end
end JanusFormal.P0EFTJanusProgramPT12RectangularDoubleAdjoint4D
