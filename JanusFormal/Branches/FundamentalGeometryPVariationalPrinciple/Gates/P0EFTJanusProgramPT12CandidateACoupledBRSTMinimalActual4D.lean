...
namespace P0EFTJanusProgramPT12CandidateACoupledBRSTMinimalActual4D
...
open P0EFTJanusProgramPT12CandidateACoupledBRSTActual4D
...
theorem candidateACoupledBRSTMinimal_smooth_mem :
  ...
  (candidateACoupledBRSTMinimal_hasCore period hPeriod configuration data analysis realization
    plusBase minusBase hBase hCenter physical).le_domain ⟨pair, rfl⟩

theorem candidateACoupledBRSTMinimal_smooth_apply :
    candidateACoupledBRSTMinimal period hPeriod configuration data analysis realization
    ...
    candidateACoupledBRSTOperator period hPeriod configuration data analysis realization
      ...
      ⟨candidateACoupledBRSTSmooth period hPeriod configuration data pair,
        candidateACoupledBRSTSmooth_mem period hPeriod configuration data analysis realization
          plusBase minusBase hBase hCenter physical pair⟩ := by
  let A := candidateACoupledBRSTOperator period hPeriod configuration data analysis realization
    plusBase minusBase hBase hCenter physical
  let S := (candidateACoupledBRSTSmooth period hPeriod configuration data).range
  have hLe := (A.domRestrict S).le_closure
  rw [show (A.domRestrict S).closure = _ from
    candidateACoupledBRST_smoothClosure_eq_minimal period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical] at hLe
  let x : (A.domRestrict S).domain :=
    ⟨candidateACoupledBRSTSmooth period hPeriod configuration data pair,
      ⟨⟨pair, rfl⟩, candidateACoupledBRSTSmooth_mem period hPeriod configuration data analysis realization
        plusBase minusBase hBase hCenter physical pair⟩⟩
  let M := candidateACoupledBRSTMinimal period hPeriod configuration data analysis realization
    plusBase minusBase hBase hCenter physical
  let y : M.domain := ⟨candidateACoupledBRSTSmooth period hPeriod configuration data pair,
    candidateACoupledBRSTMinimal_smooth_mem period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical pair⟩
  let z : A.domain := ⟨candidateACoupledBRSTSmooth period hPeriod configuration data pair,
    candidateACoupledBRSTSmooth_mem period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical pair⟩
  change M y = A z
  have hValue : (A.domRestrict S) x = M y := hLe.2 (x := x) (y := y) rfl
  exact hValue.symm.trans (LinearPMap.domRestrict_apply (x := x) (y := z) rfl)

/-- The computed minimal closure still realizes the original augmented Hessian on genuine fields. -/
theorem candidateACoupledBRSTMinimal_smooth_actual :
    inner Real
      (candidateACoupledBRSTMinimal period hPeriod configuration data analysis realization
        plusBase minusBase hBase hCenter physical
        ⟨candidateACoupledBRSTSmooth period hPeriod configuration data pair,
  ...
  rw [candidateACoupledBRSTMinimal_smooth_apply]
  exact candidateACoupledBRSTOperator_state_actual period hPeriod configuration data analysis realization
    plusBase minusBase hBase hCenter physical _ _ pair.2 test.2

end
end
end P0EFTJanusProgramPT12CandidateACoupledBRSTMinimalActual4D
end JanusFormal
