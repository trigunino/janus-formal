...
/-!
# Candidate-A spectral atlas from one unitary actual heat family

The preceding Candidate-A spectral derivation still accepted an actual zeta
family and a separate proof that its connection coefficient vanishes.  This
file replaces both by one `UnitaryActualZetaFamilyData` on the fixed H12 reduced
Hilbert space.

The actual finite part, Mellin continuation family, determinant and zero
connection coefficient are therefore all generated from

* one nuclear heat family transported by unitary conjugation;
* one H12 finite-part packet;
* one H12 Mellin continuation.

The base and local reference families, their analytic difference comparisons
and their standalone inverse-trace identities remain the only spectral inputs.
-/

namespace JanusFormal
namespace P0EFTJanusProgramPGlobalHessianPreferredFiveSectorH14D11UnitaryHeatSpectralAtlas4D

set_option autoImplicit false
set_option maxHeartbeats 202000000
set_option synthInstance.maxHeartbeats 101000000
...
open P0EFTJanusProgramPGlobalHessianPreferredFiveSectorH14D11DerivedSpectralAtlas4D
...
/-- Candidate-A spectral derivation whose actual zeta side is generated from a
unitary nuclear heat family. -/
structure GlobalHessianPreferredFiveSectorH14D11UnitaryHeatSpectralDerivationData
    (input : GlobalHessianPreferredFiveSectorNamedKernelFamilyClosure4D
      period hPeriod configuration data analysis einsteinScale hTransverse family
...
namespace GlobalHessianPreferredFiveSectorH14D11UnitaryHeatSpectralDerivationData

/-- Forget the unitary actual construction after deriving its zeta family and
zero connection coefficient. -/
def toSpectralDerivation
    (input : GlobalHessianPreferredFiveSectorNamedKernelFamilyClosure4D
      period hPeriod configuration data analysis einsteinScale hTransverse family
        chartBound Metric Abelian Matter Longitudinal Boundary ZeroMode fold Index)
    (spectral :
    ...
    GlobalHessianPreferredFiveSectorH14D11SpectralDerivationData period hPeriod
      input where
  actualFamily := spectral.actual.family
  actualCoefficient_zero := spectral.actual.connectionCoefficient_zero
...
/-- Base relative coefficient generated analytically from the unitary actual
family and the base reference. -/
theorem baseCoefficientAgreement
    ...
    relativeZetaConnectionCoefficient
      ...
      ((OldAtlas period hPeriod input).baseFamily.familyIndex.referenceTrace.trace
        parameter : Real) :=
  (spectral.toSpectralDerivation period hPeriod input).baseCoefficientAgreement
    period hPeriod input parameter

/-- Every local spectral-cut coefficient is generated analytically. -/
theorem localCoefficientAgreement
    ...
    relativeZetaConnectionCoefficient
      ...
      ((OldAtlas period hPeriod input).referenceTrace index).trace parameter :=
  (spectral.toSpectralDerivation period hPeriod input).localCoefficientAgreement
    period hPeriod input index parameter

/-- Physical D11 closure with a fully generated actual zeta family. -/
def toPhysicalReferenceClosure
  ...
  (spectral.toSpectralDerivation period hPeriod input).toPhysicalReferenceClosure
    period hPeriod input natural frame zeroTrace

/-- Public Candidate-A unitary-heat spectral checkpoint. -/
theorem global_hessian_preferred_five_sector_H14_D11_unitary_heat_spectral_atlas_gate
    (input : GlobalHessianPreferredFiveSectorNamedKernelFamilyClosure4D
      period hPeriod configuration data analysis einsteinScale hTransverse family
        chartBound Metric Abelian Matter Longitudinal Boundary ZeroMode fold Index)
    (natural : GlobalHessianPreferredFiveSectorNaturalEllipticSectorOperatorFamily4D
    ...
    GlobalHessianPreferredFiveSectorDifferentiableNamedKernelFamily4D period
      hPeriod closure ∧
    (∀ parameter,
      relativeZetaConnectionCoefficient spectral.actual.family.toZetaFamily
          parameter = 0) ∧
    (∀ parameter,
      relativeHeatMellinZetaFamilyDeterminant spectral.actual.family parameter =
        relativeHeatMellinZetaDeterminant
          spectral.actual.baseContinuation) ∧
    (∀ index parameter,
      relativeZetaConnectionCoefficient
          (closure.familyIndex.localFamily index).toZetaFamily parameter =
        (closure.familyIndex.referenceTrace index).trace parameter) := by
  dsimp only
  let derived := spectral.toSpectralDerivation period hPeriod input
  ...
  exact
    ⟨physicalReferenceResolvedKernelFamily period hPeriod input natural frame
      ...
      physicalReferenceRegularity period hPeriod input natural frame zeroTrace
        (derived.baseCoefficientAgreement period hPeriod input)
        (derived.localCoefficientAgreement period hPeriod input),
      spectral.actual.connectionCoefficient_zero,
      spectral.actual.determinant_eq_base,
      spectral.localCoefficientAgreement period hPeriod input⟩

end GlobalHessianPreferredFiveSectorH14D11UnitaryHeatSpectralDerivationData

end
end P0EFTJanusProgramPGlobalHessianPreferredFiveSectorH14D11UnitaryHeatSpectralAtlas4D
end JanusFormal
