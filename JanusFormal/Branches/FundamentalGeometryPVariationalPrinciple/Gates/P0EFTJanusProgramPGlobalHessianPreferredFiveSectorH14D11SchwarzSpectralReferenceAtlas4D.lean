...
/-!
# Candidate-A atlas from Schwarz-reflected reference zeta families

The complete spectral Candidate-A atlas still received, inside every standalone
reference assembly, the scalar reality of `zeta'(0)`.  This file replaces that
fact for the base reference and every local spectral-cut reference by analytic
Schwarz-reflection data.

Each reference continuation is compared with

```text
s ↦ conj (zeta (conj s))
```

on a common open preconnected domain joining the Mellin half-plane to zero.
The Mellin candidate's conjugation symmetry and analytic uniqueness generate
the real-axis germ and reality of the derivative.  The complete physical D11
kernel/reference atlas is then inherited from the fully spectral operator
construction.
-/

namespace JanusFormal
namespace P0EFTJanusProgramPGlobalHessianPreferredFiveSectorH14D11SchwarzSpectralReferenceAtlas4D

set_option autoImplicit false
set_option maxHeartbeats 316000000
set_option synthInstance.maxHeartbeats 158000000
...
/-- Candidate-A reference data with Schwarz-reflected base and local zeta
families. -/
structure GlobalHessianPreferredFiveSectorH14D11SchwarzSpectralReferenceData
  ...
  baseAssembly : ReferenceNuclearHeatFinitePartSchwarzSpectralAssemblyData
...
namespace GlobalHessianPreferredFiveSectorH14D11SchwarzSpectralReferenceData

/-- Convert Schwarz-reflected reference data to the complete spectral
Candidate-A packet. -/
def toCompleteSpectralReferenceData
    ...
    (input : GlobalHessianPreferredFiveSectorNamedKernelFamilyClosure4D
      period hPeriod configuration data analysis einsteinScale hTransverse family
        chartBound Metric Abelian Matter Longitudinal Boundary ZeroMode fold Index)
    (spectral : GlobalHessianPreferredFiveSectorH14D11SchwarzSpectralReferenceData
...
/-- Physical D11 kernel/reference closure generated from Schwarz-reflected
reference zeta families. -/
...
/-- Public Candidate-A Schwarz-spectral checkpoint. -/
theorem global_hessian_preferred_five_sector_H14_D11_schwarz_spectral_reference_atlas_gate
    (sliceMeasure : Measure Slice) [IsProbabilityMeasure sliceMeasure]
    (shortCutoffFilter : Filter ShortCutoff) [NeBot shortCutoffFilter]
    (longCutoffFilter : Filter LongCutoff) [NeBot longCutoffFilter]
    (input : GlobalHessianPreferredFiveSectorNamedKernelFamilyClosure4D
    ...
    (∀ parameter,
      Set.EqOn (spectral.baseReferenceFamily.continuation parameter).zeta
        ...
        (spectral.baseAssembly.zetaSchwarz parameter).domain) ∧
    (∀ index parameter,
      Set.EqOn ((spectral.localReferenceFamily index).continuation parameter).zeta
        ...
        ((spectral.localAssembly index).zetaSchwarz parameter).domain) ∧
  ...
  exact
    ⟨fun parameter =>
        (spectral.baseAssembly.zetaSchwarz parameter).
          zeta_eqOn_schwarz_domain,
      fun index parameter =>
        ((spectral.localAssembly index).zetaSchwarz parameter).
          zeta_eqOn_schwarz_domain,
      hComplete.2.2.2.2.1,
      hComplete.2.2.2.2.2⟩

end GlobalHessianPreferredFiveSectorH14D11SchwarzSpectralReferenceData

end
end P0EFTJanusProgramPGlobalHessianPreferredFiveSectorH14D11SchwarzSpectralReferenceAtlas4D
end JanusFormal
