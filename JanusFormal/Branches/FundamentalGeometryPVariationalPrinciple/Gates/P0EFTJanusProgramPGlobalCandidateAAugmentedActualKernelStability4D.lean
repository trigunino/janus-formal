...
import JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple.Gates.P0EFTJanusProgramPSelfAdjointKernelComplementStability4D

/-!
# Stability of the Candidate-A Hessian off its actual zero modes

A Candidate-A actual-kernel gap gives a quantitative open ball of invertible
reduced Hessians.  Every self-adjoint perturbation on `(ker H)ᗮ` bounded by
`delta < gap` keeps the reduced operator bijective and has Green norm at most
`(gap - delta)⁻¹`.
-/

namespace JanusFormal
namespace P0EFTJanusProgramPGlobalCandidateAAugmentedActualKernelStability4D
...
private abbrev StabilityHilbert
...
local instance (priority := 30000) stabilityNormedAddCommGroup
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    (configuration : GlobalGaugeFixedFieldConfiguration period hPeriod)
...
local instance (priority := 30000) stabilityInnerProductSpace
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    (configuration : GlobalGaugeFixedFieldConfiguration period hPeriod)
...
local instance (priority := 30000) stabilityNormedSpace
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    (configuration : GlobalGaugeFixedFieldConfiguration period hPeriod)
...
local instance (priority := 30000) stabilityModule
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    (configuration : GlobalGaugeFixedFieldConfiguration period hPeriod)
...
local instance (priority := 30000) stabilityCompleteSpace
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    (configuration : GlobalGaugeFixedFieldConfiguration period hPeriod)
...
structure GlobalCandidateAActualKernelPerturbation4D
  ...
  analytic : SelfAdjointKernelComplementPerturbationData
    (globalCandidateAActualKernelOperator period hPeriod configuration data
      analysis chart sameAction physical)
    (globalCandidateAActualKernelOperator_isSelfAdjoint period hPeriod
      configuration data analysis chart sameAction physical)
    gap.gapData perturbation

/-- Perturbed Candidate-A reduced Green operator. -/
noncomputable def globalCandidateAActualKernelPerturbedGreen
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    {measure : Measure (EffectiveQuotient period hPeriod)}
...
end P0EFTJanusProgramPGlobalCandidateAAugmentedActualKernelStability4D
end JanusFormal
