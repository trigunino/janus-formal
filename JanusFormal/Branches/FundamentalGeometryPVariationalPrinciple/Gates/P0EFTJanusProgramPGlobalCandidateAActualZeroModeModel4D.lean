...
/-!
# Explicit zero-mode model for the augmented Candidate-A Hessian

This file replaces the anonymous finite-dimensional-kernel field of H12 by an
explicit finite type of physical zero modes and a linear equivalence from its
coordinate space to the genuine augmented Hessian kernel.

Once the modes are classified, only the coercive gap on their orthogonal
complement remains analytic.  The kernel dimension is exactly the number of
classified modes.
-/

namespace JanusFormal
namespace P0EFTJanusProgramPGlobalCandidateAActualZeroModeModel4D
...
private abbrev ZeroModeHilbert
...
local instance (priority := 30000) zeroModeNormedAddCommGroup
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    (configuration : GlobalGaugeFixedFieldConfiguration period hPeriod)
...
local instance (priority := 30000) zeroModeInnerProductSpace
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    (configuration : GlobalGaugeFixedFieldConfiguration period hPeriod)
...
local instance (priority := 30000) zeroModeNormedSpace
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    (configuration : GlobalGaugeFixedFieldConfiguration period hPeriod)
...
local instance (priority := 30000) zeroModeModule
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    (configuration : GlobalGaugeFixedFieldConfiguration period hPeriod)
...
local instance (priority := 30000) zeroModeCompleteSpace
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    (configuration : GlobalGaugeFixedFieldConfiguration period hPeriod)
...
/-- Forget names but retain the exact analytic gap packet. -/
def GlobalCandidateAActualZeroModeGap4D.toActualKernelGap
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    {measure : Measure (EffectiveQuotient period hPeriod)}
  ...
  gapData := zeroModes.modeledGap.toGapData
  ll_stationary := zeroModes.ll_stationary

/-- The physical count of zero modes is the actual kernel dimension. -/
theorem GlobalCandidateAActualZeroModeGap4D.kernel_finrank_eq_card
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    {measure : Measure (EffectiveQuotient period hPeriod)}
  ...
  letI : DecidableEq zeroModes.modeledGap.model.ZeroMode :=
    zeroModes.modeledGap.model.zeroModeDecidableEq
  letI : FiniteDimensional Real
      (globalCandidateAActualKernelOperator period hPeriod configuration data
        analysis chart sameAction physical).ker :=
    zeroModes.modeledGap.model.kernelFiniteDimensional
  exact zeroModes.modeledGap.model.kernel_finrank_eq_card

/-- Public zero-mode-classified H12 and reduced-Green gate. -/
theorem global_candidateA_actual_zeroMode_model_gate
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    {measure : Measure (EffectiveQuotient period hPeriod)}
