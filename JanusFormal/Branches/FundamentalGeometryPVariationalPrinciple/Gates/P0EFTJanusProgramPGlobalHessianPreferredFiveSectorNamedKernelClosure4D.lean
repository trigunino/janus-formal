    ...
    SelfAdjointKernelComplementGapWithNamedModes
      ...
      (globalCandidateAActualKernelOperator_isSelfAdjoint period hPeriod
        ...
        (CanonicalPhysical period hPeriod einsteinScale hTransverse family
          chartBound))
      ZeroMode where
  family := closure.namedFamily
  gap := closure.frontier.analytic.toActualKernelGap.gapData.gap
  gap_pos := closure.frontier.analytic.toActualKernelGap.gapData.gap_pos
  lowerBound := closure.frontier.analytic.toActualKernelGap.gapData.lowerBound
...
theorem global_hessian_preferred_five_sector_named_kernel_closure_gate
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    {configuration : GlobalGaugeFixedFieldConfiguration period hPeriod}
