...
import JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple.Gates.P0EFTJanusProgramPGlobalHessianPreferredFiveSectorH14D11GeneratedSchwarzSpectralReferenceAtlas4D

/-!
# Candidate-A atlas from unnormalized Mellin-integral symmetry

This is the strongest current Candidate-A reference-zeta frontend.  Gamma
conjugation is canonical and no longer appears in the data.  For the base
reference and every local spectral-cut reference, the sole heat-dependent
Schwarz input is
...
for the unnormalized Mellin heat integral.  Normalization, analytic reflection,
real-axis reality, reference coefficients and all relative coefficients are
outputs.
-/

namespace JanusFormal
namespace P0EFTJanusProgramPGlobalHessianPreferredFiveSectorH14D11IntegralSchwarzSpectralReferenceAtlas4D

set_option autoImplicit false
set_option maxHeartbeats 340000000
set_option synthInstance.maxHeartbeats 170000000
...
open P0EFTJanusProgramPGlobalHessianPreferredFiveSectorH14D11GeneratedSchwarzSpectralReferenceAtlas4D
...
/-- Candidate-A data whose only per-reference Schwarz input is unnormalized
Mellin-integral conjugation. -/
structure GlobalHessianPreferredFiveSectorH14D11IntegralSchwarzSpectralReferenceData
  ...
  baseAssembly : ReferenceNuclearHeatFinitePartIntegralSchwarzAssemblyData
...
namespace GlobalHessianPreferredFiveSectorH14D11IntegralSchwarzSpectralReferenceData

/-- Convert integral-only Schwarz data to the preceding generated packet. -/
def toGeneratedSchwarzSpectralReferenceData
    {sliceMeasure : Measure Slice} [IsProbabilityMeasure sliceMeasure]
    {shortCutoffFilter : Filter ShortCutoff} [NeBot shortCutoffFilter]
    {longCutoffFilter : Filter LongCutoff} [NeBot longCutoffFilter]
    (input : GlobalHessianPreferredFiveSectorNamedKernelFamilyClosure4D
  ...
  baseAssembly := spectral.baseAssembly.toGeneratedSchwarzAssembly
  ...
  localAssembly := fun index =>
    (spectral.localAssembly index).toGeneratedSchwarzAssembly
...
/-- Physical D11 closure from unnormalized Mellin-integral symmetry. -/
def toPhysicalReferenceClosure
  ...
  (spectral.toGeneratedSchwarzSpectralReferenceData period hPeriod input).
    toPhysicalReferenceClosure period hPeriod input natural frame zeroTrace

/-- Public integral-Schwarz Candidate-A checkpoint. -/
theorem global_hessian_preferred_five_sector_H14_D11_integral_schwarz_spectral_reference_atlas_gate
    (sliceMeasure : Measure Slice) [IsProbabilityMeasure sliceMeasure]
    (shortCutoffFilter : Filter ShortCutoff) [NeBot shortCutoffFilter]
    (longCutoffFilter : Filter LongCutoff) [NeBot longCutoffFilter]
    (input : GlobalHessianPreferredFiveSectorNamedKernelFamilyClosure4D
  ...
  have hGenerated :=
    GlobalHessianPreferredFiveSectorH14D11GeneratedSchwarzSpectralReferenceData.
      global_hessian_preferred_five_sector_H14_D11_generated_schwarz_spectral_reference_atlas_gate
        period hPeriod sliceMeasure shortCutoffFilter longCutoffFilter input
          natural frame generated zeroTrace
  exact
    ⟨fun parameter =>
        (spectral.baseAssembly.zetaIntegralSchwarz parameter).
          mellinIntegralSchwarz.mellinIntegral_schwarz,
      fun index parameter =>
        ((spectral.localAssembly index).zetaIntegralSchwarz parameter).
          mellinIntegralSchwarz.mellinIntegral_schwarz,
      hGenerated.2.2.1,
      hGenerated.2.2.2⟩

end GlobalHessianPreferredFiveSectorH14D11IntegralSchwarzSpectralReferenceData

end
end P0EFTJanusProgramPGlobalHessianPreferredFiveSectorH14D11IntegralSchwarzSpectralReferenceAtlas4D
end JanusFormal
