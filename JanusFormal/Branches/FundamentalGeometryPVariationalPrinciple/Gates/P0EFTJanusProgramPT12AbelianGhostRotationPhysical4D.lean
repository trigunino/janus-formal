...
/-! The actual physical Riesz column depends on the Abelian potential alone.
In particular, smooth ghost signed coordinates preserve every H11 coupling. -/
namespace JanusFormal
namespace P0EFTJanusProgramPT12AbelianGhostRotationPhysical4D
...
/-- Nonminimal changes preserve the actual physical tangent. -/
theorem abelianCore_physicalTangent_eq_of_potential_eq
    ...
    diagonalExtendedBulkMinimalPhysicalTangentLinearMap period hPeriod configuration data analysis
      (abelianCore period hPeriod configuration analysis first) =
    diagonalExtendedBulkMinimalPhysicalTangentLinearMap period hPeriod configuration data analysis
      (abelianCore period hPeriod configuration analysis second) := by
  change (diagonalDiffeomorphismGaugeFixedTangentLinearMap period hPeriod configuration 0 +
    globalPairedAbelianBRSTStateGaugeFixedTangentLinearMap period hPeriod configuration data first +
    ...
    (diagonalDiffeomorphismGaugeFixedTangentLinearMap period hPeriod configuration 0 +
    globalPairedAbelianBRSTStateGaugeFixedTangentLinearMap period hPeriod configuration data second +
    extendedMatterGaugeFixedTangentLinearMap period hPeriod configuration 0 +
    extendedLLGaugeFixedTangentLinearMap period hPeriod configuration analysis 0).1
  simp only [map_zero, zero_add, add_zero]
  change globalCandidateAPairedGaugePotentialMinimalTangentLinearMap period hPeriod data first.potential =
    globalCandidateAPairedGaugePotentialMinimalTangentLinearMap period hPeriod data second.potential
  rw [hPotential]

/-- Column equality is against all completed global tests, not only ghost tests. -/
theorem abelianPhysicalRiesz_eq_of_potential_eq
    (first second : GlobalPairedAbelianBRSTState period hPeriod)
    (hPotential : first.potential = second.potential) :
  ...
  have hTangent := abelianCore_physicalTangent_eq_of_potential_eq period hPeriod
    configuration data analysis first second hPotential
  have hCore := physicalRiesz_core_zero_of_hessian_column_zero period hPeriod configuration data analysis
    ...
    (strongBridge (measure := measure) period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter) physical
    (abelianCore period hPeriod configuration analysis (first - second)) (by
      intro test
  ...
  have hZero := (congrArg
    (strongPhysicalRiesz (measure := measure) period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical)
    (actualAbelianInclusion_smooth period hPeriod configuration data analysis (first - second))).trans hCore
  rw [map_sub, map_sub, map_sub] at hZero
  exact sub_eq_zero.mp hZero

theorem abelianGhostRotation_physicalRiesz
  ...
  abelianPhysicalRiesz_eq_of_potential_eq period hPeriod configuration data analysis realization
    plusBase minusBase hBase hCenter physical _ _ rfl

theorem quotientAbelianPhysicalColumn_ghostRotation
    (state : GlobalPairedAbelianBRSTState period hPeriod) :
...
end P0EFTJanusProgramPT12AbelianGhostRotationPhysical4D
end JanusFormal
