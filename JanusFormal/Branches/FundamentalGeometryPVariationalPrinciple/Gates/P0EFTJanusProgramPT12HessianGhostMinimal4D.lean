import JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple.Gates.P0EFTJanusProgramPT12HessianGhostCommutation4D
...
/-! Ghost and boson projections preserve the minimal Hessian graph and its exact action. -/
namespace JanusFormal.P0EFTJanusProgramPT12HessianGhostMinimal4D
...
theorem hessianL2OperatorCore_graph_ghost
    (pair : DiffeomorphismL2 period hPeriod (metric .plus) × DiffeomorphismL2 period hPeriod (metric .plus))
    (hPair : pair ∈ (hessianL2OperatorCore period hPeriod reference metric couplings).graph) :
    (diffeomorphismGhostProjection period hPeriod (metric .plus) pair.1,
      diffeomorphismGhostProjection period hPeriod (metric .plus) pair.2) ∈
        (hessianL2OperatorCore period hPeriod reference metric couplings).graph := by
  ...
  refine ⟨⟨diffeomorphismL2Smooth period hPeriod (metric .plus) (hessianGhostSmooth period hPeriod smooth),
    ⟨hessianGhostSmooth period hPeriod smooth, rfl⟩⟩, ?_, ?_⟩
  · exact (hessianGhostSmooth_L2 period hPeriod (metric .plus) smooth).symm.trans
      (congrArg (diffeomorphismGhostProjection period hPeriod (metric .plus)) hInput)
  · exact (hessianL2OperatorCore_smooth period hPeriod reference metric couplings (hessianGhostSmooth period hPeriod smooth)).trans
      ((hessianSmoothRiesz_ghost_commutes period hPeriod reference metric couplings smooth).trans
        (congrArg (diffeomorphismGhostProjection period hPeriod (metric .plus))
          ((hessianL2OperatorCore_smooth period hPeriod reference metric couplings smooth).symm.trans hOutput)))

theorem hessianL2Minimal_graph_ghost
    (pair : DiffeomorphismL2 period hPeriod (metric .plus) × DiffeomorphismL2 period hPeriod (metric .plus))
    (hPair : pair ∈ (hessianL2Minimal period hPeriod reference metric couplings).graph) :
    (diffeomorphismGhostProjection period hPeriod (metric .plus) pair.1,
      diffeomorphismGhostProjection period hPeriod (metric .plus) pair.2) ∈
        (hessianL2Minimal period hPeriod reference metric couplings).graph := by
  rw [hessianL2Minimal_graph] at hPair ⊢
  have hMap : Set.MapsTo
      (fun value : DiffeomorphismL2 period hPeriod (metric .plus) × DiffeomorphismL2 period hPeriod (metric .plus) =>
        (diffeomorphismGhostProjection period hPeriod (metric .plus) value.1,
          diffeomorphismGhostProjection period hPeriod (metric .plus) value.2))
      (hessianL2OperatorCore period hPeriod reference metric couplings).graph
      (hessianL2OperatorCore period hPeriod reference metric couplings).graph :=
    hessianL2OperatorCore_graph_ghost period hPeriod reference metric couplings
  exact hMap.closure (by fun_prop) hPair

def hessianMinimalGhostDomain (field : (hessianL2Minimal period hPeriod reference metric couplings).domain) :
    (hessianL2Minimal period hPeriod reference metric couplings).domain := by
  refine ⟨diffeomorphismGhostProjection period hPeriod (metric .plus) field.val, ?_⟩
  have hPair := hessianL2Minimal_graph_ghost period hPeriod reference metric couplings
    (field.val, hessianL2Minimal period hPeriod reference metric couplings field)
    ((hessianL2Minimal period hPeriod reference metric couplings).mem_graph field)
...
theorem hessianL2Minimal_ghost_commutes
    ...
    diffeomorphismGhostProjection period hPeriod (metric .plus)
      (hessianL2Minimal period hPeriod reference metric couplings field) := by
  obtain ⟨projected, hInput, hOutput⟩ := (LinearPMap.mem_graph_iff _).mp
    (hessianL2Minimal_graph_ghost period hPeriod reference metric couplings _
      ((hessianL2Minimal period hPeriod reference metric couplings).mem_graph field))
  have hEqual : projected = hessianMinimalGhostDomain period hPeriod reference metric couplings field := Subtype.ext hInput
  exact (congrArg (hessianL2Minimal period hPeriod reference metric couplings) hEqual).symm.trans hOutput

theorem hessianL2Minimal_graph_boson
    (pair : DiffeomorphismL2 period hPeriod (metric .plus) × DiffeomorphismL2 period hPeriod (metric .plus))
    (hPair : pair ∈ (hessianL2Minimal period hPeriod reference metric couplings).graph) :
...
by
  change (pair - (diffeomorphismGhostProjection period hPeriod (metric .plus) pair.1,
    diffeomorphismGhostProjection period hPeriod (metric .plus) pair.2)) ∈
      (hessianL2Minimal period hPeriod reference metric couplings).graph
  exact (hessianL2Minimal period hPeriod reference metric couplings).graph.sub_mem hPair
    (hessianL2Minimal_graph_ghost period hPeriod reference metric couplings pair hPair)

end
end JanusFormal.P0EFTJanusProgramPT12HessianGhostMinimal4D