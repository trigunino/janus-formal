...
Together with the normal/tangential Riesz PDE package, the remaining analytic
input is therefore exactly:

* one closed Lagrangian boundary condition;
* one external positive-square decomposition of a real shifted form;
...
theorem denseDomain
  ...
  analytic.boundary.triple.lagrangianInclusion_denseRange_of_minimalCore
    analytic.condition
    analytic.boundary.toIntrinsicWaveRieszReducedPDEData.rieszBoundaryData.minimalCoreDense
