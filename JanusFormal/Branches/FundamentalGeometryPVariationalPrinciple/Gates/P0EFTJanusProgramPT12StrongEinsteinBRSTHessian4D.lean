...
namespace P0EFTJanusProgramPT12StrongEinsteinBRSTHessian4D
...
def strongEinsteinPlusBRSTHessian : Real := by
  ...
  exact fderiv Real (actionGradient (regularGeneralMetricC2PairedMinimalPhysicalAdmissibleActionBlocks
    period hPeriod configuration.physical couplings data plusBase minusBase hBase
      (intrinsicCanonicalLorentzVolumeMeasure period hPeriod)).einsteinHilbertPlus) 0
...
theorem strongEinsteinPlusBRSTHessian_eq_native :
    ...
    nativeEinsteinHilbertHessian period hPeriod plusBase couplings.plusEinstein
      (regularGeneralMetricC2SmoothDirection period hPeriod plusBase (field.metricPerturbation .plus))
      (regularGeneralMetricC2SmoothDirection period hPeriod plusBase (test.metricPerturbation .plus)) := by
  letI := globalMinimalPhysicalPairedMetricGaugeLLStrongNormedAddCommGroup
    period hPeriod configuration data analysis realization plusBase minusBase
      (canonicalDivergenceFreeLLFrame period hPeriod)
  letI := globalMinimalPhysicalPairedMetricGaugeLLStrongNormedSpace
    period hPeriod configuration data analysis realization plusBase minusBase
      (canonicalDivergenceFreeLLFrame period hPeriod)
  have hProjection (input : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod) :
      pairedEinsteinPlusProjection period hPeriod plusBase minusBase
        (globalMinimalPhysicalPairedMetricGaugeLLStrongMetricCLM
          period hPeriod configuration data analysis realization plusBase minusBase
          (diagonalExtendedBulkMinimalPhysicalTangentLinearMap period hPeriod configuration data analysis
            (diffeomorphismCore period hPeriod configuration analysis input))) =
      regularGeneralMetricC2SmoothDirection period hPeriod plusBase (input.metricPerturbation .plus) := by
    change regularGeneralMetricC2SmoothDirection period hPeriod plusBase
      ((diagonalExtendedBulkMinimalPhysicalTangentLinearMap period hPeriod configuration data analysis
        (diffeomorphismCore period hPeriod configuration analysis input)).1.completeVariation.fullMetricPerturbation .plus) = _
    rw [diagonalExtendedBulkMinimalPhysicalTangent_metric]
    rfl
  exact (strongEinsteinPlusActionHessian_eq_native period hPeriod configuration data analysis realization
    plusBase minusBase hBase _ _).trans (congrArg₂
      (fun first second => nativeEinsteinHilbertHessian period hPeriod plusBase couplings.plusEinstein first second)
      (hProjection field) (hProjection test))

def strongEinsteinMinusBRSTHessian : Real := by
  letI := globalMinimalPhysicalPairedMetricGaugeLLStrongNormedAddCommGroup
    period hPeriod configuration data analysis realization plusBase minusBase
      (canonicalDivergenceFreeLLFrame period hPeriod)
  letI := globalMinimalPhysicalPairedMetricGaugeLLStrongNormedSpace
  ...
  exact fderiv Real (actionGradient (regularGeneralMetricC2PairedMinimalPhysicalAdmissibleActionBlocks
    period hPeriod configuration.physical couplings data plusBase minusBase hBase
      (intrinsicCanonicalLorentzVolumeMeasure period hPeriod)).einsteinHilbertMinus) 0
    (diagonalExtendedBulkMinimalPhysicalTangentLinearMap period hPeriod configuration data analysis
      (diffeomorphismCore period hPeriod configuration analysis field))
    (diagonalExtendedBulkMinimalPhysicalTangentLinearMap period hPeriod configuration data analysis
      (diffeomorphismCore period hPeriod configuration analysis test))

theorem strongEinsteinMinusBRSTHessian_eq_native :
    ...
    nativeEinsteinHilbertHessian period hPeriod minusBase couplings.minusEinstein
      (regularGeneralMetricC2SmoothDirection period hPeriod minusBase (field.metricPerturbation .minus))
      (regularGeneralMetricC2SmoothDirection period hPeriod minusBase (test.metricPerturbation .minus)) := by
  letI := globalMinimalPhysicalPairedMetricGaugeLLStrongNormedAddCommGroup
    period hPeriod configuration data analysis realization plusBase minusBase
      (canonicalDivergenceFreeLLFrame period hPeriod)
  letI := globalMinimalPhysicalPairedMetricGaugeLLStrongNormedSpace
  ...
  have hProjection (input : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod) :
      pairedEinsteinMinusProjection period hPeriod plusBase minusBase
        (globalMinimalPhysicalPairedMetricGaugeLLStrongMetricCLM
          ...
          (diagonalExtendedBulkMinimalPhysicalTangentLinearMap period hPeriod configuration data analysis
            (diffeomorphismCore period hPeriod configuration analysis input))) =
      regularGeneralMetricC2SmoothDirection period hPeriod minusBase (input.metricPerturbation .minus) := by
    change regularGeneralMetricC2SmoothDirection period hPeriod minusBase
      ((diagonalExtendedBulkMinimalPhysicalTangentLinearMap period hPeriod configuration data analysis
        (diffeomorphismCore period hPeriod configuration analysis input)).1.completeVariation.fullMetricPerturbation .minus) = _
    rw [diagonalExtendedBulkMinimalPhysicalTangent_metric]
    rfl
  exact (strongEinsteinMinusActionHessian_eq_native period hPeriod configuration data analysis realization
    plusBase minusBase hBase _ _).trans (congrArg₂
      (fun first second => nativeEinsteinHilbertHessian period hPeriod minusBase couplings.minusEinstein first second)
      (hProjection field) (hProjection test))

end
end P0EFTJanusProgramPT12StrongEinsteinBRSTHessian4D
end JanusFormal
