...
/-!
# Candidate-A zero modes from the four Schur blocks

This is the preferred finite-mode input for the augmented Candidate-A Hessian.
Instead of supplying triangular reductions, one supplies the actual four blocks
of the Hessian in a finite-mode/complement decomposition and an inverse for the
complement block.  Gaussian elimination then constructs the finite Schur
operator and the exact full-kernel equivalence automatically.
-/

namespace JanusFormal
namespace P0EFTJanusProgramPGlobalCandidateAActualSchurBlock4D
...
local instance (priority := 30000) blockNormedAddCommGroup
    ...
    (data : GlobalCandidateAActionData period hPeriod configuration.physical
      couplings NonNullFace NullFace)
    (analysis : GlobalAnalysisData period hPeriod configuration.physical) :
    NormedAddCommGroup (BlockHilbert period hPeriod configuration data analysis) :=
...
local instance (priority := 30000) blockInnerProductSpace
    ...
    InnerProductSpace Real
      (BlockHilbert period hPeriod configuration data analysis) :=
...
local instance (priority := 30000) blockNormedSpace
    ...
    (data : GlobalCandidateAActionData period hPeriod configuration.physical
      couplings NonNullFace NullFace)
    (analysis : GlobalAnalysisData period hPeriod configuration.physical) :
    NormedSpace Real (BlockHilbert period hPeriod configuration data analysis) :=
...
local instance (priority := 30000) blockModule
    ...
    (data : GlobalCandidateAActionData period hPeriod configuration.physical
      couplings NonNullFace NullFace)
    (analysis : GlobalAnalysisData period hPeriod configuration.physical) :
    Module Real (BlockHilbert period hPeriod configuration data analysis) :=
...
structure GlobalCandidateAActualSchurBlockData4D
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
...
/-- Forget the displayed four blocks after constructing their Schur
factorization. -/
def GlobalCandidateAActualSchurBlockData4D.toSchurZeroModeData
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    {measure : Measure (EffectiveQuotient period hPeriod)}
    ...
    (blockData : GlobalCandidateAActualSchurBlockData4D period hPeriod
  ...
  schur := blockData.blocks.toKernelData
  range_closed := blockData.range_closed
...
theorem global_candidateA_actual_schur_block_gate
  ...
  global_candidateA_actual_schur_zeroMode_gate period hPeriod configuration data
    analysis chart sameAction physical Mode Complement
      (blockData.toSchurZeroModeData period hPeriod)

end
end P0EFTJanusProgramPGlobalCandidateAActualSchurBlock4D
end JanusFormal
