import JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple.Gates.P0EFTJanusProgramPT12HessianGhostReducedCore4D
...
/-! The actual ghost Hessian adjoint is the restriction of the full maximal Hessian. -/
namespace JanusFormal.P0EFTJanusProgramPT12HessianGhostReducedAdjoint4D
...
attribute [local irreducible] hessianGhostReduced

def hessianGhostReducedAdjoint :
...
theorem hessianGhostReducedAdjoint_graph_iff_smooth
    (input output : DiffeomorphismGhostPairL2 period hPeriod (metric .plus)) :
    (input, output) ∈ (hessianGhostReducedAdjoint period hPeriod reference metric couplings).graph ↔
      ∀ test, inner Real (hessianGhostReducedSmoothOutput period hPeriod reference metric couplings test) input =
        inner Real (diffeomorphismGhostPairSmooth period hPeriod (metric .plus) test) output := by
  unfold hessianGhostReducedAdjoint
  rw [LinearPMap.adjoint_graph_eq_graph_adjoint
    (T := hessianGhostReduced period hPeriod reference metric couplings) (𝕜 := Real)
    (E := DiffeomorphismGhostPairL2 period hPeriod (metric .plus))
    (F := DiffeomorphismGhostPairL2 period hPeriod (metric .plus))
    (hessianGhostReduced_denseDomain period hPeriod reference metric couplings),
    hessianGhostReduced_graph_closure, Submodule.mem_adjoint_iff]
  constructor
  · intro h test
    have hMem := hessianGhostReduced_smooth_graph period hPeriod reference metric couplings test
    rw [hessianGhostReduced_graph_closure] at hMem
    exact sub_eq_zero.mp (h
      (diffeomorphismGhostPairSmooth period hPeriod (metric .plus) test)
      (hessianGhostReducedSmoothOutput period hPeriod reference metric couplings test) hMem)
  · intro h first second hGraph
    exact sub_eq_zero.mpr (featureClosure_pairing_of_smooth
      ...
      (H := DiffeomorphismGhostPairL2 period hPeriod (metric .plus))
      (diffeomorphismGhostPairSmooth period hPeriod (metric .plus))
      (hessianGhostReducedSmoothLinearMap period hPeriod reference metric couplings)
      input output h first second hGraph)
theorem hessianGhostReducedAdjoint_graph_iff
    (input output : DiffeomorphismGhostPairL2 period hPeriod (metric .plus)) :
    (input, output) ∈ (hessianGhostReducedAdjoint period hPeriod reference metric couplings).graph ↔
      (input.val, output.val) ∈ (hessianL2Maximal period hPeriod reference metric couplings).graph := by
  rw [hessianGhostReducedAdjoint_graph_iff_smooth, hessianL2Maximal_graph_iff]
  have hFixed (field : DiffeomorphismGhostPairL2 period hPeriod (metric .plus)) :
      diffeomorphismGhostProjection period hPeriod (metric .plus) field.val = field.val := by
  ...
  rw [diffeomorphismGhostProjection_pairing, diffeomorphismGhostProjection_pairing,
    hFixed input, hFixed output]
  constructor <;> intro h
  · exact (real_inner_comm _ _).trans (h.symm.trans (real_inner_comm _ _))
  · exact (real_inner_comm _ _).trans (h.symm.trans (real_inner_comm _ _))

theorem hessianGhostReducedAdjoint_domain_iff
    (input : DiffeomorphismGhostPairL2 period hPeriod (metric .plus)) :
    input ∈ (hessianGhostReducedAdjoint period hPeriod reference metric couplings).domain ↔
  ...
  · intro hInput
    obtain ⟨field, hField, _⟩ := (LinearPMap.mem_graph_iff _).mp
      ((hessianGhostReducedAdjoint_graph_iff period hPeriod reference metric couplings _ _).mp
        ((hessianGhostReducedAdjoint period hPeriod reference metric couplings).mem_graph ⟨input, hInput⟩))
  ...
  · intro hInput
    ...
    let output : DiffeomorphismGhostPairL2 period hPeriod (metric .plus) :=
      (diffeomorphismGhostProjection period hPeriod (metric .plus)).rangeRestrict
        (hessianL2Maximal period hPeriod reference metric couplings field)
    have hGraph := hessianL2Maximal_graph_ghost period hPeriod reference metric couplings _
      ((hessianL2Maximal period hPeriod reference metric couplings).mem_graph field)
    have hFixed : diffeomorphismGhostProjection period hPeriod (metric .plus) input.val = input.val := by
      exact (Set.ext_iff.mp (diffeomorphismGhostProjection_range_eq_fixed period hPeriod (metric .plus)) input.val).mp input.property
    change (diffeomorphismGhostProjection period hPeriod (metric .plus) input.val, output.val) ∈ _ at hGraph
    rw [hFixed] at hGraph
    obtain ⟨vector, hVector, _⟩ := (LinearPMap.mem_graph_iff _).mp
      ((hessianGhostReducedAdjoint_graph_iff period hPeriod reference metric couplings input output).mpr hGraph)
    change vector.val = input at hVector
    rw [← hVector]
    exact vector.property

end
end JanusFormal.P0EFTJanusProgramPT12HessianGhostReducedAdjoint4D
