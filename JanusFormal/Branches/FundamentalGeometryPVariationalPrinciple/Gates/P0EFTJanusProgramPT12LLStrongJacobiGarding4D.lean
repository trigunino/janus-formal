...
/-! A frame-gradient Gårding estimate for the smooth LL Jacobi core. -/

namespace JanusFormal
namespace P0EFTJanusProgramPT12LLStrongJacobiGarding4D
...
private theorem raw_hessian_gradient_lower_bound
    (frame : SmoothThroatGeneratingFrame period hPeriod)
    (fields : IndependentFields period hPeriod)
    (direction : LLWeakTestSpace period hPeriod)
    (bound : Real)
    (hLower : ∀ point, -bound ≤ fields.llMeasure point) :
    (∫ point, throatDerivativeEnergy period hPeriod frame direction point
  ...
  have hEnergyIntegrable : Integrable
      (throatDerivativeEnergy period hPeriod frame direction) mu :=
    (throatDerivativeEnergy_continuous period hPeriod frame direction).integrable_of_hasCompactSupport
      (HasCompactSupport.of_compactSpace _)
  have hNormIntegrable : Integrable (fun point => ‖direction point‖ ^ 2) mu :=
    (direction.contMDiff_toFun.continuous.norm.pow 2).integrable_of_hasCompactSupport
      (HasCompactSupport.of_compactSpace _)
  ...
  have hPoint (point : EffectiveThroat period hPeriod) :
    ...
    have hEnergy : 0 ≤ throatDerivativeEnergy period hPeriod frame direction point := by
      unfold throatDerivativeEnergy
      exact Finset.sum_nonneg fun index _ => sq_nonneg _
    have hWeight : 1 ≤ llAuxiliaryKineticWeight period hPeriod fields point := by
  ...
  have hIntegral := integral_mono
    (hEnergyIntegrable.sub (hNormIntegrable.const_mul (2 * bound)))
    hDensityIntegrable hPoint
  change (∫ point,
      throatDerivativeEnergy period hPeriod frame direction point -
        (2 * bound) * ‖direction point‖ ^ 2 ∂mu) ≤
    globalDifferentialLLFluxHessian period hPeriod frame fields
      direction direction mu at hIntegral
  rw [integral_sub hEnergyIntegrable (hNormIntegrable.const_mul (2 * bound)),
    integral_const_mul] at hIntegral
  simpa [mu, mul_assoc] using hIntegral

private theorem pt_hessian_gradient_lower_bound
    (fields : IndependentFields period hPeriod)
    (direction : LLWeakTestSpace period hPeriod)
    (bound : Real)
    (hLower : ∀ point, -bound ≤ fields.llMeasure point) :
    (1 / 2 : Real) *
        (∫ point, throatDerivativeEnergy period hPeriod
          (canonicalDivergenceFreeLLFrame period hPeriod) direction point
          ∂(intrinsicCanonicalThroatVolumeMeasure period hPeriod)) -
      2 * bound *
        (∫ point, ‖direction point‖ ^ 2
          ∂(intrinsicCanonicalThroatVolumeMeasure period hPeriod)) ≤
  ...
  have hRaw := raw_hessian_gradient_lower_bound period hPeriod frame fields
  ...
  have hPulledLower : ∀ point,
    ...
    exact hLower _
  have hPulled := raw_hessian_gradient_lower_bound period hPeriod frame
  ...
  have hPulledEnergy : 0 ≤
      ∫ point, throatDerivativeEnergy period hPeriod frame
...
/-- Smooth-core Gårding bound for the canonical frame derivative energy. -/
theorem llStrongJacobi_smooth_l2_garding
    (fields : IndependentFields period hPeriod) :
    ∃ constant : Real, 0 ≤ constant ∧
      ∀ direction : LLWeakTestSpace period hPeriod,
        (1 / 2 : Real) *
          ...
          inner Real (llStrongJacobiToL2 period hPeriod fields direction)
            (llSmoothToL2 period hPeriod direction) +
          constant * ‖llSmoothToL2 period hPeriod direction‖ ^ 2 := by
  obtain ⟨bound, hBound⟩ :=
  ...
  intro direction
  have hGarding := pt_hessian_gradient_lower_bound period hPeriod fields
    direction nonnegativeBound hLower
  rw [← smooth_l2_norm_sq_eq_integral period hPeriod direction,
    ← strong_l2_inner_eq_hessian period hPeriod fields direction] at hGarding
  linarith

end
end P0EFTJanusProgramPT12LLStrongJacobiGarding4D
end JanusFormal
