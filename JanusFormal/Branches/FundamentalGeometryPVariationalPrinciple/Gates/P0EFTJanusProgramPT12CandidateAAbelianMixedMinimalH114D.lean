...
namespace P0EFTJanusProgramPT12CandidateAAbelianMixedMinimalH114D
...
/-- The actual minimal mixed BRST closure including the complete H11 correction. -/
def candidateAAbelianMixedMinimalH11 : CandidateAAbelianMixedHilbert period hPeriod data →ₗ.[Real]
    CandidateAAbelianMixedHilbert period hPeriod data :=
  boundedPerturbation (candidateAAbelianMixedMinimal period hPeriod data)
...
theorem candidateAAbelianMixedMinimalH11_isClosed :
    (candidateAAbelianMixedMinimalH11 period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical).IsClosed :=
  boundedPerturbation_isClosed _ _ (candidateAAbelianMixedMinimal_isClosed period hPeriod data)

theorem candidateAAbelianMixedMinimalH11_hasCore :
    (candidateAAbelianMixedMinimalH11 period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical).HasCore
      (candidateAAbelianMixedSmoothLinearMap period hPeriod data).range :=
  boundedPerturbation_hasCore _ _ (candidateAAbelianMixedMinimal_isClosed period hPeriod data).isClosable _
    (candidateAAbelianMixedMinimal_hasCore period hPeriod data)

/-- The closure is computed without equating the minimal and maximal FP adjoints. -/
theorem candidateAAbelianMixedAugmented_smoothClosure_eq_minimalH11 :
  ...
  change ((boundedPerturbation _ _).domRestrict _).closure = _
  rw [boundedPerturbation_restriction, boundedPerturbation_closure _ _
    ((candidateAAbelianMixedOperator_selfAdjoint period hPeriod data).isClosed.isClosable.leIsClosable LinearPMap.domRestrict_le),
    candidateAAbelianMixed_smoothClosure_eq_minimal]
  rfl

end
end
end P0EFTJanusProgramPT12CandidateAAbelianMixedMinimalH114D
end JanusFormal
