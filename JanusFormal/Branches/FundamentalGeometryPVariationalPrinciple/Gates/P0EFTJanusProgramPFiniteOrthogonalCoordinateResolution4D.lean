...
theorem projection_symmetric
  ...
  calc
    (∑ x,
      inner Real (Pi.single sector (data.decomposition first sector) x)
        (data.decomposition second x)) =
        inner Real (data.decomposition first sector)
          (data.decomposition second sector) := by
      refine (Finset.sum_eq_single_of_mem sector (Finset.mem_univ _) ?_).trans ?_
      · intro other _ hOther
        simp [Pi.single_eq_of_ne hOther]
      · rw [Pi.single_eq_same]
    _ = ∑ x,
        inner Real (data.decomposition first x)
          (Pi.single sector (data.decomposition second sector) x) := by
      symm
      refine (Finset.sum_eq_single_of_mem sector (Finset.mem_univ _) ?_).trans ?_
      · intro other _ hOther
        simp [Pi.single_eq_of_ne hOther]
