import JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple.Gates.P0EFTJanusProgramPT12NativeMaxwellHessianL24D
...
/-! Both native Maxwell Hessians as one actual BRST L2 covector for each fixed first field. -/
namespace JanusFormal.P0EFTJanusProgramPT12PairedMaxwellHessianL24D
...
open P0EFTJanusProgramPT12NativeMaxwellHessianL24D
...
def pairedMaxwellHessianCovector : DiffeomorphismL2 period hPeriod normalization →L[Real] Real :=
  ∑ sector, scale sector • (innerSL Real (nativeMaxwellHessianL2 period hPeriod
    (metric sector) (potential sector) (first.metricPerturbation sector))).comp
      (diffeomorphismTensorReadout period hPeriod normalization sector)

theorem pairedMaxwellHessianCovector_smooth
    (test : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod) :
    pairedMaxwellHessianCovector period hPeriod normalization metric potential scale first
      (diffeomorphismL2Smooth period hPeriod normalization test) =
    ∑ sector, scale sector * nativeMobileMaxwellHessian period hPeriod (metric sector) (potential sector)
      (regularGeneralMetricC2SmoothDirection period hPeriod (metric sector) (first.metricPerturbation sector), 0)
      (regularGeneralMetricC2SmoothDirection period hPeriod (metric sector) (test.metricPerturbation sector), 0) := by
  simp only [pairedMaxwellHessianCovector, sum_apply, smul_apply, smul_eq_mul, ContinuousLinearMap.comp_apply,
    diffeomorphismTensorReadout_smooth, innerSL_apply_apply, nativeMaxwellHessianL2_pairing]

theorem pairedMaxwellHessian_bound
    (test : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod) :
    ‖∑ sector, scale sector * nativeMobileMaxwellHessian period hPeriod (metric sector) (potential sector)
      (regularGeneralMetricC2SmoothDirection period hPeriod (metric sector) (first.metricPerturbation sector), 0)
      (regularGeneralMetricC2SmoothDirection period hPeriod (metric sector) (test.metricPerturbation sector), 0)‖ ≤
    ‖pairedMaxwellHessianCovector period hPeriod normalization metric potential scale first‖ *
      ‖diffeomorphismL2Smooth period hPeriod normalization test‖ := by
  rw [← pairedMaxwellHessianCovector_smooth period hPeriod normalization metric potential scale first test]
  exact ContinuousLinearMap.le_opNorm _ _

theorem pairedMaxwellHessian_metric_bound
    (test : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod) :
    ‖∑ sector, scale sector * nativeMobileMaxwellHessian period hPeriod (metric sector) (potential sector)
      (regularGeneralMetricC2SmoothDirection period hPeriod (metric sector) (first.metricPerturbation sector), 0)
      (regularGeneralMetricC2SmoothDirection period hPeriod (metric sector) (test.metricPerturbation sector), 0)‖ ≤
  ...
  simpa only [diffeomorphismMetricTransfer_metric] using
    pairedMaxwellHessian_bound period hPeriod normalization metric potential scale first
      (diffeomorphismMetricTransfer period hPeriod normalization test)

end
end JanusFormal.P0EFTJanusProgramPT12PairedMaxwellHessianL24D
