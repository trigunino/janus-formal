...
import JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple.Gates.P0EFTJanusProgramPT12LLReducedMinimalCore4D
...
namespace P0EFTJanusProgramPT12CandidateABRSTLLMinimal4D
...
open P0EFTJanusProgramPT12LLReducedMinimalCore4D
...
theorem candidateABRSTLLSmooth_range :
    (candidateABRSTLLSmooth period hPeriod configuration data analysis).range =
      productCore (candidateAReducedBRSTSmooth period hPeriod configuration data).range
...
def candidateABRSTLLMinimal : CandidateABRSTLLHilbert period hPeriod configuration data →ₗ.[Real]
...
theorem candidateABRSTLLMinimal_isClosed :
  ...
  productOperator_isClosed _ _
    (candidateAReducedBRSTMinimal_isClosed period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights)
    (fullLLReducedJacobi_isClosed period hPeriod data analysis)

variable (hZero : (data.boundary.llFields period hPeriod).llField = 0)

include hZero in
/-- The simultaneous genuine BRST and LL fields form a core of the minimal product. -/
theorem candidateABRSTLLMinimal_hasCore :
    (candidateABRSTLLMinimal period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights).HasCore
      (candidateABRSTLLSmooth period hPeriod configuration data analysis).range := by
  rw [candidateABRSTLLSmooth_range]
  exact productOperator_hasCore _ _
    (candidateAReducedBRSTMinimal_isClosed period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights)
    (fullLLReducedJacobi_isClosed period hPeriod data analysis) _ _
...
include hZero in
/-- Both minimal factors are computed from the same smooth restriction of the existing realization. -/
theorem candidateABRSTLLOperator_smoothClosure_eq_minimal :
    ((candidateABRSTLLOperator period hPeriod configuration data analysis realization
      ...
      (candidateABRSTLLSmooth period hPeriod configuration data analysis).range).closure =
    candidateABRSTLLMinimal period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights := by
  rw [candidateABRSTLLSmooth_range]
  exact productOperator_restriction_closure_eq _ _ _ _
    (candidateAReducedBRSTOperator_selfAdjoint period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights).isClosed
    (quotientLLFriedrichs_isClosed period hPeriod analysis) _ _
    (candidateAReducedBRST_smoothClosure_eq_minimal period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights)
    (quotientLLFriedrichs_smoothClosure_eq_minimal period hPeriod data analysis hZero)

end
end
end P0EFTJanusProgramPT12CandidateABRSTLLMinimal4D
end JanusFormal
