...
namespace P0EFTJanusProgramPT12DiffeomorphismH11MetricTests4D
...
def metricPhysicalTestBoundConstant (test : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod) : Real :=
  ‖strongInteractionBRSTHessianCovector period hPeriod configuration data analysis realization plusBase minusBase
      (globalCandidateAMetricBySector period hPeriod data .plus) test‖ +
  ‖strongEinsteinBRSTHessianCovector period hPeriod plusBase minusBase
      (globalCandidateAMetricBySector period hPeriod data .plus) test (couplings := couplings)‖ +
  ‖strongMaxwellBRSTHessianCovector period hPeriod configuration plusBase minusBase
      (globalCandidateAMetricBySector period hPeriod data .plus) test (couplings := couplings)‖

variable (test field : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod)

theorem diffeomorphismH11_metric_pairing_bound :
    ‖inner Real (metricPhysicalSmoothTest period hPeriod configuration data analysis test)
    ...
    metricPhysicalTestBoundConstant period hPeriod configuration data analysis realization plusBase minusBase test *
      ‖diffeomorphismMetricSmooth period hPeriod (globalCandidateAMetricBySector period hPeriod data .plus) field‖ := by
  have hEM := diffeomorphismH11_metric_pairing_sub_interaction_bound
    period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical test field
  have hInteraction := strongInteractionBRSTHessian_metric_bound
    period hPeriod configuration data analysis realization plusBase minusBase
      (globalCandidateAMetricBySector period hPeriod data .plus) hBase test field
  have hSum := (norm_add_le
    (inner Real (metricPhysicalSmoothTest period hPeriod configuration data analysis test)
      (diffeomorphismH11Smooth period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical field) -
      strongInteractionBRSTHessian period hPeriod configuration data analysis realization plusBase minusBase hBase test field)
    (strongInteractionBRSTHessian period hPeriod configuration data analysis realization plusBase minusBase hBase test field)).trans
      (add_le_add hEM hInteraction)
  rw [sub_add_cancel] at hSum
  dsimp only [metricPhysicalTestBoundConstant]
  apply hSum.trans_eq
  ring

theorem diffeomorphismH11_metric_test_bound :
    diffeomorphismH11MetricTestBound period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical
      (metricPhysicalSmoothTest period hPeriod configuration data analysis test) := by
  refine ⟨metricPhysicalTestBoundConstant period hPeriod configuration data analysis realization plusBase minusBase test, ?_⟩
  intro field
  exact diffeomorphismH11_metric_pairing_bound period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical test field

/-- Smooth metric physical tests belong to the actual complete H11 adjoint domain. -/
theorem diffeomorphismH11_metric_test_mem_adjoint :
    metricPhysicalSmoothTest period hPeriod configuration data analysis test ∈
  ...
  apply (diffeomorphismH11Adjoint_domain_iff_metricBound period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical _).mpr
  exact diffeomorphismH11_metric_test_bound period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical test

end
end
end P0EFTJanusProgramPT12DiffeomorphismH11MetricTests4D
end JanusFormal
