...
namespace P0EFTJanusProgramPT12CandidateAFourSectorRealization4D
...
/-- Both BRST sectors, quotient LL, and the signed matter graph. -/
abbrev CandidateAFourSectorHilbert := WithLp 2
  (CandidateABRSTLLHilbert period hPeriod configuration data ×
    ProgramPPrimitiveSpinCMatterL2GraphDomain period hPeriod couplings.matterMassSquared)
...
def candidateAFourSectorOperator : CandidateAFourSectorHilbert period hPeriod configuration data →ₗ.[Real]
  ...
  productOperator
    ...
    ((matterL2GraphRiesz period hPeriod couplings.matterMassSquared).toPMap ⊤)

theorem candidateAFourSectorOperator_selfAdjoint : IsSelfAdjoint
  ...
  productOperator_selfAdjoint _ _
    (candidateABRSTLLOperator_selfAdjoint period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights)
    (matterL2GraphRiesz_toPMap_selfAdjoint period hPeriod couplings.matterMassSquared)

theorem candidateAFourSectorOperator_domain_iff
    (x : CandidateAFourSectorHilbert period hPeriod configuration data) :
    x ∈ (candidateAFourSectorOperator period hPeriod configuration data analysis realization
    ...
    x.fst.fst.snd ∈ (candidateAAbelianMixedOperator period hPeriod data).domain ∧
      x.fst.snd ∈ (quotientLLFriedrichs period hPeriod analysis).domain := by
  change x ∈ (productOperator _ _).domain ↔ _
  rw [productOperator_domain_iff, candidateABRSTLLOperator_domain_iff]
  exact and_iff_left (Submodule.mem_top : x.snd ∈ (⊤ : Submodule Real _))

theorem candidateAFourSectorOperator_pairing
    (x : (candidateAFourSectorOperator period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights).domain)
    (y : CandidateAFourSectorHilbert period hPeriod configuration data) :
    ...
    inner Real (candidateABRSTLLOperator period hPeriod configuration data analysis realization
      ...
      matterL2GraphForm period hPeriod couplings.matterMassSquared x.val.snd y.snd := by
  exact (P0EFTJanusProgramPT12ProductOperatorPairing4D.productOperator_pairing _ _ x y).trans
    (congrArg (fun r : Real => _ + r)
      (matterL2GraphRiesz_pairing period hPeriod couplings.matterMassSquared x.val.snd y.snd))

end
end
end P0EFTJanusProgramPT12CandidateAFourSectorRealization4D
end JanusFormal