...
/-! The projected smooth fields form an operator core of the actual metric–B Hessian. -/
namespace JanusFormal.P0EFTJanusProgramPT12HessianBosonReducedCore4D
...
def hessianBosonReducedSmoothLinearMap :
  ...
  (diffeomorphismBosonProjection period hPeriod (metric .plus)).rangeRestrict.toLinearMap.comp
    (hessianSmoothRieszLinearMap period hPeriod reference metric couplings)

theorem hessianBosonReduced_graph_closure :
    (hessianBosonReduced period hPeriod reference metric couplings).graph =
      linearFeatureGraphClosure (H := DiffeomorphismBosonPairL2 period hPeriod (metric .plus))
        (diffeomorphismBosonPairSmooth period hPeriod (metric .plus))
        (hessianBosonReducedSmoothLinearMap period hPeriod reference metric couplings) := by
  apply le_antisymm
  · intro pair hPair
    have hFull := (hessianBosonReduced_graph_iff period hPeriod reference metric couplings _ _).mp hPair
    ...
    let project := fun pair : DiffeomorphismL2 period hPeriod (metric .plus) ×
      ...
      ((diffeomorphismBosonProjection period hPeriod (metric .plus)).rangeRestrict pair.1,
       (diffeomorphismBosonProjection period hPeriod (metric .plus)).rangeRestrict pair.2)
    have hMap : Set.MapsTo project
        ((hessianL2OperatorCore period hPeriod reference metric couplings).graph : Set _)
        (((diffeomorphismBosonPairSmooth period hPeriod (metric .plus)).prod
          (hessianBosonReducedSmoothLinearMap period hPeriod reference metric couplings)).range : Set _) := by
      intro value hValue
      ...
      · exact congrArg (diffeomorphismBosonProjection period hPeriod (metric .plus))
          (hSmooth.trans hInput)
      · exact congrArg (diffeomorphismBosonProjection period hPeriod (metric .plus)) hOutput
    have hProjected := hMap.closure (by fun_prop) hFull
    have hFixed (field : DiffeomorphismBosonPairL2 period hPeriod (metric .plus)) :
        (diffeomorphismBosonProjection period hPeriod (metric .plus)).rangeRestrict field.val = field := by
      apply Subtype.ext
  ...
  · exact closure_minimal
      (by rintro pair ⟨field, rfl⟩; exact hessianBosonReduced_smooth_graph period hPeriod reference metric couplings field)
      (hessianBosonReduced_isClosed period hPeriod reference metric couplings)

theorem hessianBosonReduced_feature_single :
    Function.Injective (fun graph : linearFeatureGraphClosure (H := DiffeomorphismBosonPairL2 period hPeriod (metric .plus))
      (diffeomorphismBosonPairSmooth period hPeriod (metric .plus))
      (hessianBosonReducedSmoothLinearMap period hPeriod reference metric couplings) => graph.val.1) :=
  featureClosure_injective_of_closed_extension (H := DiffeomorphismBosonPairL2 period hPeriod (metric .plus)) _ _
    (hessianBosonReduced period hPeriod reference metric couplings)
    (hessianBosonReduced_isClosed period hPeriod reference metric couplings)
    (hessianBosonReduced_smooth_graph period hPeriod reference metric couplings)

theorem hessianBosonReduced_eq_closedFeature :
    hessianBosonReduced period hPeriod reference metric couplings =
      closedFeatureOperator (H := DiffeomorphismBosonPairL2 period hPeriod (metric .plus)) (diffeomorphismBosonPairSmooth period hPeriod (metric .plus))
        (hessianBosonReducedSmoothLinearMap period hPeriod reference metric couplings) := by
  apply LinearPMap.eq_of_eq_graph
  rw [hessianBosonReduced_graph_closure,
    closedFeatureOperator_graph (H := DiffeomorphismBosonPairL2 period hPeriod (metric .plus)) _ _ (hessianBosonReduced_feature_single period hPeriod reference metric couplings)]

theorem hessianBosonReduced_hasCore :
    (hessianBosonReduced period hPeriod reference metric couplings).HasCore
      (diffeomorphismBosonPairSmooth period hPeriod (metric .plus)).range := by
  rw [hessianBosonReduced_eq_closedFeature]
  exact closedFeatureOperator_hasCore (H := DiffeomorphismBosonPairL2 period hPeriod (metric .plus)) _ _
    (hessianBosonReduced_feature_single period hPeriod reference metric couplings)

end
end JanusFormal.P0EFTJanusProgramPT12HessianBosonReducedCore4D
