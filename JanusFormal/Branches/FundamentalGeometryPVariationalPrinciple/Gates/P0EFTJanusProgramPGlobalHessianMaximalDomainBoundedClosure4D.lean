...
map.  The smooth matter graph realization is constructed from:

* maximal-domain membership of every smooth primitive section;
* agreement of the maximal geometric operator with the smooth `2D + m²`
  expression;
* the same-action completion pairing.
...
/-- Canonical smooth matter graph realization from the maximal-domain data. -/
def maximalDomainClosureMatterRealization
  ...
  programPPrimitiveSpinCMatterSmoothGraphRealization_of_maximalDomain period
    hPeriod massSquared domain sameAction
    ...
    (family : ProgramPGlobalMinimalPhysicalLocalActionFamilyReducedData4D period
      hPeriod configuration data analysis
        (maximalDomainClosureMatterRealization period hPeriod
          couplings.matterMassSquared domain spinCSameAction)) :=
  globalCandidateAMinimalPhysicalActionChartData_of_reducedFamily period hPeriod
    configuration data analysis
      (maximalDomainClosureMatterRealization period hPeriod
        couplings.matterMassSquared domain spinCSameAction) family
...
structure GlobalCandidateAHessianMaximalDomainBoundedInputs4D
  ...
  spinCDomain : ProgramPPrimitiveSpinCSmoothMaximalDomainData4D period hPeriod
    couplings.matterMassSquared
  spinCSameAction :
    ProgramPPrimitiveSpinCMatterSmoothMaximalSameActionData4D period hPeriod
      couplings.matterMassSquared spinCDomain
  family : ProgramPGlobalMinimalPhysicalLocalActionFamilyReducedData4D period
    hPeriod configuration data analysis
      (maximalDomainClosureMatterRealization period hPeriod
        couplings.matterMassSquared spinCDomain spinCSameAction)
  physicalBound :
    GlobalCandidateASevenPhysicalCoreBound4D period hPeriod configuration data
      analysis
        (maximalDomainClosureChart period hPeriod configuration data analysis
          spinCDomain spinCSameAction family)
        (maximalDomainClosureMatterLLSameAction period hPeriod configuration data
          analysis spinCDomain spinCSameAction family)
  parametrix :
      ...
      configuration data analysis
        ...
        (globalCandidateASevenPhysicalCommonDomainExtension_of_bound period
          ...
          (maximalDomainClosureMatterLLSameAction period hPeriod configuration
            data analysis spinCDomain spinCSameAction family)
          physicalBound)
...
def GlobalCandidateAHessianMaximalDomainBoundedInputs4D.physical
  ...
  globalCandidateASevenPhysicalCommonDomainExtension_of_bound period hPeriod
    configuration data analysis
      (maximalDomainClosureChart period hPeriod configuration data analysis
        inputs.spinCDomain inputs.spinCSameAction inputs.family)
      (maximalDomainClosureMatterLLSameAction period hPeriod configuration data
        analysis inputs.spinCDomain inputs.spinCSameAction inputs.family)
