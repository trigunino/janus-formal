...
namespace P0EFTJanusProgramPT12CandidateAReducedBRSTMinimalPairing4D
...
theorem candidateACoupledBRSTMinimal_domain_iff
    ...
    x ∈ (candidateACoupledBRSTMinimal period hPeriod configuration data analysis realization
      ...
      x.snd ∈ (candidateAAbelianMixedMinimal period hPeriod data).domain :=
  coupledBRSTOperator_domain_iff _ _ _ _ _ x

/-- Pairing on the entire operator domain, before specializing to smooth fields. -/
theorem candidateACoupledBRSTMinimal_pairing
    (x : (candidateACoupledBRSTMinimal period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical).domain)
    (y : CandidateACoupledBRSTHilbert period hPeriod configuration data) :
    inner Real (candidateACoupledBRSTMinimal period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical x) y =
    inner Real (diagonalDiffeomorphismSignedRiesz period hPeriod couplings
      (globalCandidateAMetricBySector period hPeriod data) x.val.fst) y.fst +
    inner Real (candidateAAbelianMixedMinimal period hPeriod data
      ⟨x.val.snd, (candidateACoupledBRSTMinimal_domain_iff period hPeriod configuration data analysis realization
        plusBase minusBase hBase hCenter physical x.val).mp x.property⟩) y.snd +
    inner Real (candidateACoupledBRSTPhysicalColumn period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical x.val)
      (candidateACoupledBRSTPhysicalLift period hPeriod configuration data analysis y) :=
  coupledBRSTOperator_pairing _ _ _ _ _ x y
...
theorem candidateAReducedBRSTMinimal_domain_iff
    (x : CandidateAReducedBRSTHilbert period hPeriod configuration data) :
    x ∈ (candidateAReducedBRSTMinimal period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights).domain ↔
      x.snd ∈ (candidateAAbelianMixedMinimal period hPeriod data).domain :=
  coupledBRSTOperator_domain_iff _ _ _ _ _ x

theorem candidateAReducedBRSTMinimal_pairing
    (x : (candidateAReducedBRSTMinimal period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights).domain)
    (y : CandidateAReducedBRSTHilbert period hPeriod configuration data) :
    inner Real (candidateAReducedBRSTMinimal period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights x) y =
    inner Real (reducedDiffeomorphismSignedRiesz period hPeriod configuration data hMetric hWeights x.val.fst) y.fst +
    inner Real (candidateAAbelianMixedMinimal period hPeriod data
      ⟨x.val.snd, (candidateAReducedBRSTMinimal_domain_iff period hPeriod configuration data analysis realization
        plusBase minusBase hBase hCenter physical hMetric hWeights x.val).mp x.property⟩) y.snd +
    inner Real (candidateAReducedBRSTPhysicalColumn period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical x.val)
      (candidateAReducedBRSTPhysicalLift period hPeriod configuration data analysis y) :=
  coupledBRSTOperator_pairing _ _ _ _ _ x y

/-- Quotienting preserves the pairing on the full unbounded domain and every completed test. -/
theorem candidateAReducedBRSTMinimal_pairing_project
    (x : (candidateACoupledBRSTMinimal period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical).domain)
    (y : CandidateACoupledBRSTHilbert period hPeriod configuration data) :
    inner Real (candidateAReducedBRSTMinimal period hPeriod configuration data analysis realization
      ...
      (candidateABRSTQuotientMap period hPeriod configuration data y) =
    inner Real (candidateACoupledBRSTMinimal period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical x) y := by
  conv_rhs => rw [candidateACoupledBRSTMinimal_pairing]
  rw [candidateAReducedBRSTMinimal_pairing]
  apply congrArg₂ (fun r s : Real => r + s)
  · simp only [candidateABRSTQuotientMap_apply, WithLp.toLp_fst, WithLp.toLp_snd]
    apply congrArg₂ (fun r s : Real => r + s)
    · exact reducedDiffeomorphismSignedRiesz_pairing period hPeriod configuration data hMetric hWeights x.val.fst y.fst
    · rfl
  · exact candidateAReducedBRSTPhysical_pairing_project period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical x.val y

end
end
end P0EFTJanusProgramPT12CandidateAReducedBRSTMinimalPairing4D
end JanusFormal
