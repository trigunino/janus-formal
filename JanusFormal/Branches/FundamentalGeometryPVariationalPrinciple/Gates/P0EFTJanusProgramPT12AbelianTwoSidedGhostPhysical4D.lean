...
/-! H11 is unchanged by continuous ghost rotation on the actual strengthened graph. -/
namespace JanusFormal
namespace P0EFTJanusProgramPT12AbelianTwoSidedGhostPhysical4D
...
/-- A continuous dense map from signed two-sided coordinates to the actual graph. -/
def abelianTwoSidedSignedReadout :
    AbelianTwoSidedGraph period hPeriod (globalCandidateAMetricBySector period hPeriod data) →L[Real]
      ActualAbelianHilbert period hPeriod configuration data :=
  (abelianTwoSidedForget period hPeriod (globalCandidateAMetricBySector period hPeriod data)).comp
    (abelianTwoSidedGhostRotation period hPeriod
      (globalCandidateAMetricBySector period hPeriod data)).symm.toContinuousLinearMap

theorem abelianTwoSidedSignedReadout_denseRange :
    DenseRange (abelianTwoSidedSignedReadout period hPeriod configuration data) :=
  (abelianTwoSidedForget_denseRange period hPeriod (globalCandidateAMetricBySector period hPeriod data)).comp
    (abelianTwoSidedGhostRotation period hPeriod
      (globalCandidateAMetricBySector period hPeriod data)).symm.surjective.denseRange
    (abelianTwoSidedForget period hPeriod (globalCandidateAMetricBySector period hPeriod data)).continuous

/-- The whole H11 column, with arbitrary completed global tests, is preserved. -/
theorem abelianTwoSidedGhostRotation_physicalColumn
    (vector : AbelianTwoSidedGraph period hPeriod (globalCandidateAMetricBySector period hPeriod data)) :
    quotientAbelianPhysicalColumn period hPeriod configuration data analysis realization
      ...
      (abelianTwoSidedForget period hPeriod (globalCandidateAMetricBySector period hPeriod data)
        (abelianTwoSidedGhostRotation period hPeriod
          (globalCandidateAMetricBySector period hPeriod data) vector)) =
    quotientAbelianPhysicalColumn period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical
      (abelianTwoSidedForget period hPeriod (globalCandidateAMetricBySector period hPeriod data) vector) := by
  let column := quotientAbelianPhysicalColumn period hPeriod configuration data analysis realization
    plusBase minusBase hBase hCenter physical
  let forget := abelianTwoSidedForget period hPeriod (globalCandidateAMetricBySector period hPeriod data)
  let rotation := abelianTwoSidedGhostRotation period hPeriod (globalCandidateAMetricBySector period hPeriod data)
  have hAll : (fun x => column (forget (rotation x))) = (fun x => column (forget x)) := by
    apply (abelianTwoSidedSmoothEmbedding_denseRange period hPeriod
      (globalCandidateAMetricBySector period hPeriod data)).equalizer
    · exact column.continuous.comp (forget.continuous.comp rotation.continuous)
    · exact column.continuous.comp forget.continuous
    · funext state
      change column (forget (abelianTwoSidedGhostRotation period hPeriod
        (globalCandidateAMetricBySector period hPeriod data)
        (abelianTwoSidedSmoothEmbedding period hPeriod
          (globalCandidateAMetricBySector period hPeriod data) state))) = _
      rw [abelianTwoSidedGhostRotation_smooth]
      exact congrArg (jointGhostLLNullSpace period hPeriod configuration data analysis).mkQ
        (abelianPhysicalRiesz_eq_of_potential_eq period hPeriod configuration data analysis realization
          plusBase minusBase hBase hCenter physical (abelianGhostSignedState period hPeriod state) state rfl)
  exact congrFun hAll vector

theorem abelianTwoSidedSignedReadout_physicalColumn
    (vector : AbelianTwoSidedGraph period hPeriod (globalCandidateAMetricBySector period hPeriod data)) :
    quotientAbelianPhysicalColumn period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical
      (abelianTwoSidedSignedReadout period hPeriod configuration data vector) =
    quotientAbelianPhysicalColumn period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical
      (abelianTwoSidedForget period hPeriod (globalCandidateAMetricBySector period hPeriod data) vector) := by
  have h := abelianTwoSidedGhostRotation_physicalColumn period hPeriod configuration data analysis realization
    plusBase minusBase hBase hCenter physical
    ((abelianTwoSidedGhostRotation period hPeriod (globalCandidateAMetricBySector period hPeriod data)).symm vector)
  rw [ContinuousLinearEquiv.apply_symm_apply] at h
  exact h.symm

/-- Exact reduced augmented column on every vector of the strengthened graph. -/
theorem abelianTwoSidedSignedReadout_augmented_column
    ...
    quotientAbelianPhysicalColumn period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical
      (abelianTwoSidedForget period hPeriod (globalCandidateAMetricBySector period hPeriod data) vector) := by
  rw [quotientAbelian_augmented_column, abelianTwoSidedSignedReadout_physicalColumn]

end
end
end P0EFTJanusProgramPT12AbelianTwoSidedGhostPhysical4D
end JanusFormal
