import JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple.Gates.P0EFTJanusProgramPGlobalHessianQuillenCertificate4D
import JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple.Gates.P0EFTJanusProgramPGlobalQuillenFrontier4D

/-!
# Conditional closure of the global Candidate-A Quillen package

The existing circle certificate supplies the actual rank-one determinant line,
Hermitian metric, flat connection, isometric parallel transport, unitary
holonomy and endpoint clutching.  The global Hessian bridge supplies a nonzero
Candidate-A zeta determinant section which is parallel for that connection.

Their product is the complete `QUILLEN-GLOBAL-01` output conditional on the
explicit family bridge.  No circle fact is re-assumed and no separate global
line is invented.
-/

namespace JanusFormal
namespace P0EFTJanusProgramPGlobalHessianQuillenClosure4D

set_option autoImplicit false
set_option maxHeartbeats 11000000
set_option synthInstance.maxHeartbeats 5500000
...
open P0EFTJanusProgramPGlobalHessianQuillenCertificate4D
open P0EFTJanusProgramPGlobalQuillenFrontier4D
...
/-- Complete topological and analytic Quillen certificate. -/
structure GlobalCandidateAHessianQuillenClosureCertificate4D
  ...
  globalAnalytic : GlobalCandidateAHessianQuillenCertificate4D period hPeriod
    configuration data analysis einsteinScale hBoundaryTransverse family
      extensions shift fold bridge

/-- Construct the full closure from the one remaining family bridge. -/
def globalCandidateAHessianQuillenClosureCertificate
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    {measure : Measure (EffectiveQuotient period hPeriod)}
    ...
    GlobalCandidateAHessianQuillenClosureCertificate4D period hPeriod
      configuration data analysis einsteinScale hBoundaryTransverse family
        extensions shift fold bridge where
  circleGeometry := programPGlobalQuillenFrontierCertificate4D fold
...
theorem global_candidateA_hessian_quillen_global_closure_gate
    ...
    GlobalCandidateAHessianQuillenClosureCertificate4D period hPeriod
      configuration data analysis einsteinScale hBoundaryTransverse family
        extensions shift fold bridge :=
  globalCandidateAHessianQuillenClosureCertificate bridge

/-- Convenient projection: the determinant line is rank one in every fiber. -/
theorem GlobalCandidateAHessianQuillenClosureCertificate4D.line_rank_one
    {couplings : GlobalCandidateAActionCouplings}
    {NonNullFace NullFace : Type*}
    [Fintype NonNullFace] [Fintype NullFace]
    {measure : Measure (EffectiveQuotient period hPeriod)}
    ...
    {bridge : GlobalCandidateAHessianQuillenFamilyBridgeData4D period hPeriod
      configuration data analysis einsteinScale hBoundaryTransverse family
        extensions shift fold}
    (certificate : GlobalCandidateAHessianQuillenClosureCertificate4D period
      hPeriod configuration data analysis einsteinScale hBoundaryTransverse
        family extensions shift fold bridge) :
    ∀ holonomy : CircleHolonomyQuotient,
      Module.finrank Complex (CircleDeterminantQuotientFiber holonomy) = 1 :=
  certificate.circleGeometry.quotientLineRankOne

end
end P0EFTJanusProgramPGlobalHessianQuillenClosure4D
end JanusFormal
