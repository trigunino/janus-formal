...
namespace P0EFTJanusProgramPT12CandidateAAbelianMixedH114D
...
/-- The internal H11 block; the separate column still retains all other output sectors. -/
def candidateAAbelianMixedH11 : CandidateAAbelianMixedHilbert period hPeriod data →L[Real]
    CandidateAAbelianMixedHilbert period hPeriod data :=
  (candidateAAbelianMixedPhysicalLift period hPeriod configuration data analysis).adjoint.comp
...
theorem candidateAAbelianMixedH11_pairing
    (x y : CandidateAAbelianMixedHilbert period hPeriod data) :
    inner Real (candidateAAbelianMixedH11 period hPeriod configuration data analysis realization
    ...
    inner Real (candidateAAbelianMixedPhysicalColumn period hPeriod configuration data analysis realization
      ...
      (candidateAAbelianMixedPhysicalLift period hPeriod configuration data analysis y) :=
  (candidateAAbelianMixedPhysicalLift period hPeriod configuration data analysis).adjoint_inner_left y _

theorem candidateAAbelianMixedH11_selfAdjoint : IsSelfAdjoint
  ...
  intro x y
  change inner Real (candidateAAbelianMixedH11 period hPeriod configuration data analysis realization
    plusBase minusBase hBase hCenter physical x) y =
    inner Real x (candidateAAbelianMixedH11 period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical y)
  rw [← real_inner_comm x (candidateAAbelianMixedH11 period hPeriod configuration data analysis realization
    plusBase minusBase hBase hCenter physical y), candidateAAbelianMixedH11_pairing,
    candidateAAbelianMixedH11_pairing]
  have hSym := ContinuousLinearMap.isSelfAdjoint_iff_isSymmetric.mp
    (strongPhysicalRiesz_isSelfAdjoint period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical)
  exact (hSym _ _).trans (real_inner_comm _ _)

theorem candidateAAbelianMixedH11_smooth_pairing
  ...
  let lift := candidateAAbelianMixedPhysicalLift period hPeriod configuration data analysis
  let riesz := strongPhysicalRiesz (measure := measure) period hPeriod configuration data analysis realization
    plusBase minusBase hBase hCenter physical
  let original := fun state => actualAbelianInclusion period hPeriod configuration data analysis
    (globalPairedAbelianOffShellSmoothEmbedding period hPeriod (globalCandidateAMetricBySector period hPeriod data) state)
  have hSym := ContinuousLinearMap.isSelfAdjoint_iff_isSymmetric.mp
    (strongPhysicalRiesz_isSelfAdjoint period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical)
  have hFirst : riesz (lift (candidateAAbelianMixedSmooth period hPeriod data first)) = riesz (original first) :=
    candidateAAbelianMixedPhysicalColumn_smooth period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical first
  have hSecond : riesz (lift (candidateAAbelianMixedSmooth period hPeriod data second)) = riesz (original second) :=
    candidateAAbelianMixedPhysicalColumn_smooth period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical second
  change ∀ x y, inner Real (riesz x) y = inner Real x (riesz y) at hSym
  rw [candidateAAbelianMixedH11_pairing]
  change inner Real (riesz (lift (candidateAAbelianMixedSmooth period hPeriod data first)))
    (lift (candidateAAbelianMixedSmooth period hPeriod data second)) = _
  rw [hFirst, hSym, hSecond, ← hSym]
...
end P0EFTJanusProgramPT12CandidateAAbelianMixedH114D
end JanusFormal
