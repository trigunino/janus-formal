...
/-! The full H11 adjoint as an exact equation in the original differential graph. -/
namespace JanusFormal
namespace P0EFTJanusProgramPT12DiffeomorphismH11GraphBridge4D
...
/-- The transpose physical column in the original differential graph. -/
def diffeomorphismH11GraphTranspose : CommonAugmentedHilbert period hPeriod configuration data analysis →L[Real]
    ActualDiffeomorphismHilbert period hPeriod configuration data :=
  (actualDiffeomorphismReadout period hPeriod configuration data analysis).comp (strongPhysicalRiesz (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical)

theorem diffeomorphismH11GraphTranspose_pairing
    (input : CommonAugmentedHilbert period hPeriod configuration data analysis) (field : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod) :
    inner Real (diffeomorphismH11Smooth period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical field) input =
      inner Real (globalCandidateADiagonalDiffeomorphismOffShellSmoothEmbedding period hPeriod (globalCandidateAMetricBySector period hPeriod data) field)
        (diffeomorphismH11GraphTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical input) := by
  unfold diffeomorphismH11Smooth diffeomorphismPhysicalSmoothLift
  change inner Real ((strongPhysicalRiesz (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical) (actualDiffeomorphismInclusion period hPeriod configuration data analysis
    (globalCandidateADiagonalDiffeomorphismOffShellSmoothEmbedding period hPeriod (globalCandidateAMetricBySector period hPeriod data) field))) input = _
  erw [(ContinuousLinearMap.isSelfAdjoint_iff_isSymmetric.mp
    (strongPhysicalRiesz_isSelfAdjoint period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical)) _ _, actualDiffeomorphismInclusion_inner]
  rfl

/-- Exact operator equation replacing all smooth tests of the H11 adjoint. -/
theorem diffeomorphismH11Adjoint_graph_iff_graphTranspose (input : CommonAugmentedHilbert period hPeriod configuration data analysis)
    ...
    (input, output) ∈ (diffeomorphismH11Adjoint period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical).graph ↔
      (diffeomorphismGraphToL2 period hPeriod (globalCandidateAMetricBySector period hPeriod data)).adjoint output = diffeomorphismH11GraphTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical input := by
  rw [diffeomorphismH11Adjoint_graph_iff]
  constructor
  · intro hTest
    apply ext_inner_left Real
    intro test
    refine (globalCandidateADiagonalDiffeomorphismOffShellSmoothEmbedding_denseRange period hPeriod (globalCandidateAMetricBySector period hPeriod data)).induction_on test ?_ ?_
    · apply isClosed_eq <;> fun_prop
    · intro field
      rw [ContinuousLinearMap.adjoint_inner_right, diffeomorphismGraphToL2_smooth,
        ← hTest, diffeomorphismH11GraphTranspose_pairing]
  · intro hEquation field
    rw [diffeomorphismH11GraphTranspose_pairing, ← hEquation,
      ContinuousLinearMap.adjoint_inner_right, diffeomorphismGraphToL2_smooth]

theorem diffeomorphismH11Adjoint_domain_iff_graphRange (input : CommonAugmentedHilbert period hPeriod configuration data analysis) :
    input ∈ (diffeomorphismH11Adjoint period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical).domain ↔
      diffeomorphismH11GraphTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical input ∈ (diffeomorphismGraphToL2 period hPeriod (globalCandidateAMetricBySector period hPeriod data)).adjoint.range := by
  rw [diffeomorphismH11Adjoint_domain_iff]
  constructor
  · rintro ⟨output, hTest⟩
    exact ⟨output, (diffeomorphismH11Adjoint_graph_iff_graphTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical input output).mp
      ((diffeomorphismH11Adjoint_graph_iff period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical input output).mpr hTest)⟩
  · rintro ⟨output, hOutput⟩
    exact ⟨output, (diffeomorphismH11Adjoint_graph_iff period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical input output).mp
      ((diffeomorphismH11Adjoint_graph_iff_graphTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical input output).mpr hOutput)⟩

theorem diffeomorphismH11MetricBound_iff_graphRange (input : CommonAugmentedHilbert period hPeriod configuration data analysis) :
    diffeomorphismH11MetricTestBound period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical input ↔
      diffeomorphismH11GraphTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical input ∈ (diffeomorphismGraphToL2 period hPeriod (globalCandidateAMetricBySector period hPeriod data)).adjoint.range := by
  rw [← diffeomorphismH11Adjoint_domain_iff_metricBound,
    diffeomorphismH11Adjoint_domain_iff_graphRange]

theorem diffeomorphismH11Adjoint_zero_graph_iff (input : CommonAugmentedHilbert period hPeriod configuration data analysis) :
    (input, 0) ∈ (diffeomorphismH11Adjoint period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical).graph ↔
      diffeomorphismH11GraphTranspose period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical input = 0 := by
  rw [diffeomorphismH11Adjoint_graph_iff_graphTranspose, map_zero, eq_comm]

end
end
end P0EFTJanusProgramPT12DiffeomorphismH11GraphBridge4D
end JanusFormal
