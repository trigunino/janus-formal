...
theorem projectedKernelGramMatrix_toSquareBlock
    ...
    (finiteFamilyGramMatrix
        (fun mode =>
          projectedNamedKernelVector period hPeriod input parameter mode)).toSquareBlock
      (namedModeFiveSector period hPeriod input) sector =
      ...
      (fun mode =>
        ...
        ∏ sector : FivePhysicalSector,
          ((finiteFamilyGramMatrix
              (fun mode =>
                projectedNamedKernelVector period hPeriod input parameter mode)).toSquareBlock
            (namedModeFiveSector period hPeriod input) sector).det :=
...
theorem mem_projectedKernelRegularSet_iff_forall_sector
  ...
  rw [projectedKernelGramDeterminant_eq_sectorProduct period hPeriod input
    parameter]
  constructor
  · intro hProduct sector hSector
    apply hProduct
    exact Finset.prod_eq_zero (Finset.mem_univ sector) hSector
  · intro hSector
    exact Finset.prod_ne_zero_iff.mpr fun sector _ => hSector sector
