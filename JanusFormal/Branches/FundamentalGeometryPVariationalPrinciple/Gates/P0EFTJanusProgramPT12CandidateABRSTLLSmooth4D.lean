...
namespace P0EFTJanusProgramPT12CandidateABRSTLLSmooth4D
...
/-- Actual fields, including arbitrary LL auxiliary and measure representatives. -/
def candidateABRSTLLSmooth :
    ((GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod × GlobalPairedAbelianBRSTState period hPeriod) ×
      GlobalFullLLSmooth period hPeriod analysis) →ₗ[Real] CandidateABRSTLLHilbert period hPeriod configuration data :=
  (WithLp.prodContinuousLinearEquiv 2 Real
    (CandidateAReducedBRSTHilbert period hPeriod configuration data)
    (FullLLReducedHilbert period hPeriod)).symm.toLinearMap.comp
      ((candidateAReducedBRSTSmooth period hPeriod configuration data).prodMap
        (fullLLReducedHilbertCore period hPeriod analysis))

theorem candidateABRSTLLSmooth_denseRange :
    DenseRange (candidateABRSTLLSmooth period hPeriod configuration data analysis) := by
  have h := (candidateAReducedBRSTSmooth_denseRange period hPeriod configuration data).prodMap
    (fullLLReducedHilbertCore_denseRange period hPeriod data analysis)
  exact (WithLp.prodContinuousLinearEquiv 2 Real
    (CandidateAReducedBRSTHilbert period hPeriod configuration data)
    (FullLLReducedHilbert period hPeriod)).symm.surjective.denseRange.comp h
    (WithLp.prodContinuousLinearEquiv 2 Real
      (CandidateAReducedBRSTHilbert period hPeriod configuration data)
      (FullLLReducedHilbert period hPeriod)).symm.continuous
...
def candidateABRSTLLSmoothDomain
  ...
  ⟨WithLp.toLp 2 (candidateAReducedBRSTSmooth period hPeriod configuration data pair,
      (quotientLLFriedrichsSmoothDomainElement period hPeriod analysis ll).val),
    (productOperator_domain_iff _ _ _).mpr
      ⟨candidateAReducedBRSTSmooth_mem period hPeriod configuration data analysis realization
        plusBase minusBase hBase hCenter physical hMetric hWeights pair,
        (quotientLLFriedrichsSmoothDomainElement period hPeriod analysis ll).property⟩⟩

theorem candidateABRSTLLSmoothDomain_value
    (pair : GlobalCandidateADiagonalDiffeomorphismBRSTState period hPeriod × GlobalPairedAbelianBRSTState period hPeriod)
    (ll : GlobalFullLLSmooth period hPeriod analysis) :
    (candidateABRSTLLSmoothDomain period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights pair ll).val =
      candidateABRSTLLSmooth period hPeriod configuration data analysis (pair, ll) := by
  apply WithLp.ofLp_injective 2
  exact Prod.ext rfl (quotientLLFriedrichsSmoothDomainElement_value period hPeriod analysis ll)

end
end
end P0EFTJanusProgramPT12CandidateABRSTLLSmooth4D
end JanusFormal
