...
namespace P0EFTJanusProgramPT12CandidateABRSTLLRealization4D
...
/-- Both BRST sectors and the LL quotient; the physical BRST block retains H11. -/
abbrev CandidateABRSTLLHilbert := WithLp 2
  (CandidateAReducedBRSTHilbert period hPeriod configuration data × FullLLReducedHilbert period hPeriod)
...
def candidateABRSTLLOperator : CandidateABRSTLLHilbert period hPeriod configuration data →ₗ.[Real]
...
theorem candidateABRSTLLOperator_selfAdjoint : IsSelfAdjoint
  ...
  productOperator_selfAdjoint _ _
    (candidateAReducedBRSTOperator_selfAdjoint period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights)
    (quotientLLFriedrichs_isSelfAdjoint period hPeriod analysis)

theorem candidateABRSTLLOperator_domain_iff
    (x : CandidateABRSTLLHilbert period hPeriod configuration data) :
    x ∈ (candidateABRSTLLOperator period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights).domain ↔
    x.fst.snd ∈ (candidateAAbelianMixedOperator period hPeriod data).domain ∧
      x.snd ∈ (quotientLLFriedrichs period hPeriod analysis).domain := by
  change x ∈ (productOperator _ _).domain ↔ _
  rw [productOperator_domain_iff, candidateAReducedBRSTOperator_domain_iff]

/-- Domain pairing before specializing to actual smooth representatives. -/
theorem candidateABRSTLLOperator_pairing
    (x : (candidateABRSTLLOperator period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical hMetric hWeights).domain)
    (y : CandidateABRSTLLHilbert period hPeriod configuration data) :
    ...
    inner Real (quotientLLFriedrichs period hPeriod analysis
      ⟨x.val.snd, ((productOperator_domain_iff _ _ _).mp x.property).2⟩) y.snd := by
  exact P0EFTJanusProgramPT12ProductOperatorPairing4D.productOperator_pairing _ _ x y

end
end
end P0EFTJanusProgramPT12CandidateABRSTLLRealization4D
end JanusFormal
