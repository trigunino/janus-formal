...
def toReferenceVariationSpectralData
  ...
  baseHasDerivAt_finitePartLog :=
    (spectral.baseTermwiseTrace period hPeriod input).toReferenceFinitePartTraceVariation.hasDerivAt_finitePartLog
  ...
  localHasDerivAt_finitePartLog := fun index =>
    (spectral.localTermwiseTrace period hPeriod input index).toReferenceFinitePartTraceVariation.hasDerivAt_finitePartLog
...
def toPhysicalReferenceClosure
  ...
  (spectral.toReferenceVariationSpectralData period hPeriod input).toPhysicalReferenceClosure
...
theorem global_hessian_preferred_five_sector_H14_D11_termwise_reference_spectral_atlas_gate
  ...
  exact
    ⟨physicalReferenceResolvedKernelFamily period hPeriod input natural frame
        zeroTrace
        ((direct.toRealReferenceSpectralData period hPeriod input).baseCoefficientAgreement
          period hPeriod input)
        ((direct.toRealReferenceSpectralData period hPeriod input).localCoefficientAgreement
          period hPeriod input),
