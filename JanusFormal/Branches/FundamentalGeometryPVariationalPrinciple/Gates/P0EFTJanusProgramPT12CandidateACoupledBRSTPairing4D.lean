...
namespace P0EFTJanusProgramPT12CandidateACoupledBRSTPairing4D
...
/-- Physical cross terms agree on arbitrary completed diffeomorphism inputs. -/
theorem candidateACoupledPhysical_state_pairing
    ...
    physical.form (candidateACoupledBRSTOriginal period hPeriod configuration data analysis d a)
      (candidateACoupledBRSTOriginal period hPeriod configuration data analysis e b) := by
  let lift := candidateACoupledBRSTPhysicalLift period hPeriod configuration data analysis
  let R := strongPhysicalRiesz (measure := measure) period hPeriod configuration data analysis realization
    plusBase minusBase hBase hCenter physical
  have hf : R (lift (candidateACoupledBRSTState period hPeriod configuration data d a)) =
      R (candidateACoupledBRSTOriginal period hPeriod configuration data analysis d a) :=
    candidateACoupledBRSTPhysicalColumn_state period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical d a
  have hg : R (lift (candidateACoupledBRSTState period hPeriod configuration data e b)) =
      R (candidateACoupledBRSTOriginal period hPeriod configuration data analysis e b) :=
    candidateACoupledBRSTPhysicalColumn_state period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical e b
  have hSym := ContinuousLinearMap.isSelfAdjoint_iff_isSymmetric.mp
    (strongPhysicalRiesz_isSelfAdjoint period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical)
  change ∀ x y, inner Real (R x) y = inner Real x (R y) at hSym
  change inner Real (R (lift (candidateACoupledBRSTState period hPeriod configuration data d a)))
    (lift (candidateACoupledBRSTState period hPeriod configuration data e b)) = _
  rw [hf, hSym, hg, ← hSym]
...
/-- The exact signed sector forms plus all four H11 blocks, including the mixed ones. -/
theorem candidateACoupledBRSTOperator_state_pairing
    (d e : ActualDiffeomorphismHilbert period hPeriod configuration data)
    (a b : GlobalPairedAbelianBRSTState period hPeriod) :
    ...
    globalCandidateADiagonalDiffeomorphismOffShellHessian period hPeriod couplings
      (globalCandidateAMetricBySector period hPeriod data) d e +
    ...
    physical.form (candidateACoupledBRSTOriginal period hPeriod configuration data analysis d a)
      (candidateACoupledBRSTOriginal period hPeriod configuration data analysis e b) := by
  rw [P0EFTJanusProgramPT12CandidateACoupledBRSTDomainPairing4D.candidateACoupledBRSTOperator_pairing,
    candidateACoupledPhysical_state_pairing]
  simp only [candidateACoupledBRSTState, WithLp.toLp_fst, WithLp.toLp_snd]
  rw [diagonalDiffeomorphismSignedRiesz_pairing, candidateAAbelianMixedOperator_smooth_hessian]

end
end
end P0EFTJanusProgramPT12CandidateACoupledBRSTPairing4D
end JanusFormal
