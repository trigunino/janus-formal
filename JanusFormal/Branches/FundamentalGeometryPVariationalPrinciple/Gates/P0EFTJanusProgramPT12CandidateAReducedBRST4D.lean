...
namespace P0EFTJanusProgramPT12CandidateAReducedBRST4D
...
abbrev CandidateAReducedBRSTHilbert := WithLp 2
  (ReducedDiffeomorphismHilbert period hPeriod configuration data × CandidateAAbelianMixedHilbert period hPeriod data)

/-- Quotient the shared diffeomorphism ghosts; retain every abelian coordinate. -/
def candidateABRSTQuotientMap : CandidateACoupledBRSTHilbert period hPeriod configuration data →L[Real]
  ...
  (WithLp.prodContinuousLinearEquiv 2 Real _ _).symm.toContinuousLinearMap.comp
    (((diffeomorphismGhostNullSpace period hPeriod configuration data).mkQL.prodMap
      (ContinuousLinearMap.id Real (CandidateAAbelianMixedHilbert period hPeriod data))).comp
      (WithLp.prodContinuousLinearEquiv 2 Real _ _).toContinuousLinearMap)

theorem candidateABRSTQuotientMap_apply (x : CandidateACoupledBRSTHilbert period hPeriod configuration data) :
    candidateABRSTQuotientMap period hPeriod configuration data x = WithLp.toLp 2
      ((diffeomorphismGhostNullSpace period hPeriod configuration data).mkQ x.fst, x.snd) := rfl

theorem candidateABRSTQuotientMap_surjective :
    Function.Surjective (candidateABRSTQuotientMap period hPeriod configuration data) := by
  intro y
  obtain ⟨d, hd⟩ := (diffeomorphismGhostNullSpace period hPeriod configuration data).mkQ_surjective y.fst
  refine ⟨WithLp.toLp 2 (d, y.snd), ?_⟩
  apply WithLp.ofLp_injective 2
  exact Prod.ext hd rfl

def candidateAReducedBRSTPhysicalLift : CandidateAReducedBRSTHilbert period hPeriod configuration data →L[Real]
    JointGhostLLQuotient period hPeriod configuration data analysis :=
  coupledPhysicalLift (quotientDiffeomorphismInclusion period hPeriod configuration data analysis).toContinuousLinearMap
...
theorem candidateAReducedBRSTPhysicalLift_project
    (x : CandidateACoupledBRSTHilbert period hPeriod configuration data) :
    candidateAReducedBRSTPhysicalLift period hPeriod configuration data analysis
      (candidateABRSTQuotientMap period hPeriod configuration data x) =
    (jointGhostLLNullSpace period hPeriod configuration data analysis).mkQ
      (candidateACoupledBRSTPhysicalLift period hPeriod configuration data analysis x) := by
  change quotientDiffeomorphismInclusion period hPeriod configuration data analysis
      ((diffeomorphismGhostNullSpace period hPeriod configuration data).mkQ x.fst) +
    (jointGhostLLNullSpace period hPeriod configuration data analysis).mkQ
      (candidateAAbelianMixedPhysicalLift period hPeriod configuration data analysis x.snd) =
    (jointGhostLLNullSpace period hPeriod configuration data analysis).mkQ
      (actualDiffeomorphismInclusion period hPeriod configuration data analysis x.fst +
        candidateAAbelianMixedPhysicalLift period hPeriod configuration data analysis x.snd)
  rw [quotientDiffeomorphismInclusion_mk, map_add]

def candidateAReducedBRSTPhysicalColumn : CandidateAReducedBRSTHilbert period hPeriod configuration data →L[Real]
    JointGhostLLQuotient period hPeriod configuration data analysis :=
...
theorem candidateAReducedBRSTPhysicalColumn_project
    (x : CandidateACoupledBRSTHilbert period hPeriod configuration data) :
    candidateAReducedBRSTPhysicalColumn period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical (candidateABRSTQuotientMap period hPeriod configuration data x) =
    (jointGhostLLNullSpace period hPeriod configuration data analysis).mkQ
      (candidateACoupledBRSTPhysicalColumn period hPeriod configuration data analysis realization
        plusBase minusBase hBase hCenter physical x) := by
  change quotientPhysicalRiesz period hPeriod configuration data analysis realization
    plusBase minusBase hBase hCenter physical
    (candidateAReducedBRSTPhysicalLift period hPeriod configuration data analysis
      (candidateABRSTQuotientMap period hPeriod configuration data x)) = _
  rw [candidateAReducedBRSTPhysicalLift_project, quotientPhysicalRiesz_mk]
  rfl
...
def candidateAReducedBRSTOperator : CandidateAReducedBRSTHilbert period hPeriod configuration data →ₗ.[Real]
    CandidateAReducedBRSTHilbert period hPeriod configuration data :=
  coupledBRSTOperator
...
theorem candidateAReducedBRSTOperator_selfAdjoint : IsSelfAdjoint
  ...
  coupledBRSTOperator_selfAdjoint _ _ _ _ _
    (reducedDiffeomorphismSignedRiesz_isSelfAdjoint period hPeriod configuration data hMetric hWeights)
...
theorem candidateAReducedBRSTOperator_domain_iff
    (x : CandidateAReducedBRSTHilbert period hPeriod configuration data) :
    x ∈ (candidateAReducedBRSTOperator period hPeriod configuration data analysis realization
...
theorem candidateAReducedBRSTOperator_project_domain_iff
    (x : CandidateACoupledBRSTHilbert period hPeriod configuration data) :
    candidateABRSTQuotientMap period hPeriod configuration data x ∈
      (candidateAReducedBRSTOperator period hPeriod configuration data analysis realization
        plusBase minusBase hBase hCenter physical hMetric hWeights).domain ↔
  ...
  rw [candidateAReducedBRSTOperator_domain_iff, candidateACoupledBRSTOperator_domain_iff]
  rfl

end
end
end P0EFTJanusProgramPT12CandidateAReducedBRST4D
end JanusFormal
