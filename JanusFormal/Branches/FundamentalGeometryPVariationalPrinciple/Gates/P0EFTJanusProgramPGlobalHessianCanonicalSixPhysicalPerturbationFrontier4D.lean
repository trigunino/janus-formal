...
noncomputable local instance physicalPerturbationZeroModeDecidableEq
    (ZeroMode : Type*) : DecidableEq ZeroMode :=
  Classical.decEq _
  ...
  global_candidateA_hessian_canonicalSix_stablePerturbation_frontier_gate period
    hPeriod (measure := measure) configuration data analysis einsteinScale
      hTransverse family chartBound ZeroMode
        (canonical.toActualStable period hPeriod (measure := measure))
