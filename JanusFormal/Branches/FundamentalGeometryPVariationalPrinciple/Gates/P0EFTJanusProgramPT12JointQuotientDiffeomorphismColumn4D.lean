import JanusFormal.Branches.FundamentalGeometryPVariationalPrinciple.Gates.P0EFTJanusProgramPT12ReducedDiffeomorphismSignedRiesz4D
...
/-! Exact reduced diffeomorphism operator column with the full descended H11 operator. -/
namespace JanusFormal
namespace P0EFTJanusProgramPT12JointQuotientDiffeomorphismColumn4D
...
/-- The completed diagonal source retains the full physical column in all target sectors. -/
theorem actualDiffeomorphism_augmented_pairing
    ...
    inner Real (strongAugmentedRiesz (measure := measure) period hPeriod configuration data analysis
      ...
      (actualDiffeomorphismInclusion period hPeriod configuration data analysis vector)) test =
    ...
    physical.form (actualDiffeomorphismInclusion period hPeriod configuration data analysis vector) test := by
  have h := actualAugmented_pairing_eq_signedBRST_matterLL_physical period hPeriod
    configuration data analysis
    (strongChart (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase)
    (strongBridge (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter)
    physical (actualDiffeomorphismInclusion period hPeriod configuration data analysis vector) test
  change inner Real (strongAugmentedRiesz (measure := measure) period hPeriod configuration data analysis
    realization plusBase minusBase hBase hCenter physical
    (actualDiffeomorphismInclusion period hPeriod configuration data analysis vector)) test =
      inner Real (diagonalDiffeomorphismSignedRiesz period hPeriod couplings
        (globalCandidateAMetricBySector period hPeriod data) vector) test.fst +
      inner Real (pairedAbelianSignedRiesz period hPeriod
        (globalCandidateAMetricBySector period hPeriod data) 0) test.snd.fst +
      ...
      physical.form (actualDiffeomorphismInclusion period hPeriod configuration data analysis vector) test at h
  simp only [map_zero, inner_zero_left, add_zero, zero_apply] at h
  exact h

theorem actualDiffeomorphism_augmented_column
    (vector : ActualDiffeomorphismHilbert period hPeriod configuration data) :
    ...
    strongPhysicalRiesz (measure := measure) period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical
      (actualDiffeomorphismInclusion period hPeriod configuration data analysis vector) := by
  apply ext_inner_right Real
  intro test
  rw [inner_add_left, actualDiffeomorphismInclusion_inner]
  exact (actualDiffeomorphism_augmented_pairing period hPeriod configuration data analysis realization
    plusBase minusBase hBase hCenter physical vector test).trans
      (congrArg (fun value => _ + value)
        (globalCandidateASevenPhysicalCommonRieszOperator_pairing period hPeriod configuration data analysis
          (strongChart (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase)
          (strongBridge (measure := measure) period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter)
          physical (actualDiffeomorphismInclusion period hPeriod configuration data analysis vector) test).symm)

/-- Actual reduced vector equality; no assumed smooth-core intertwiner is used. -/
theorem quotientDiffeomorphism_augmented_column
    ...
    jointGhostLLRiesz period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter
      ...
      (quotientDiffeomorphismInclusion period hPeriod configuration data analysis vector) =
    quotientDiffeomorphismInclusion period hPeriod configuration data analysis
      (reducedDiffeomorphismSignedRiesz period hPeriod configuration data hMetric hWeights vector) +
    quotientPhysicalRiesz period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical
      (quotientDiffeomorphismInclusion period hPeriod configuration data analysis vector) := by
  ...
  change (jointGhostLLNullSpace period hPeriod configuration data analysis).mkQ
    (strongAugmentedRiesz (measure := measure) period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical
      (actualDiffeomorphismInclusion period hPeriod configuration data analysis vector)) = _
  rw [actualDiffeomorphism_augmented_column, map_add]
  rfl

theorem quotientDiffeomorphism_augmented_pairing
    (vector : ReducedDiffeomorphismHilbert period hPeriod configuration data)
    (test : JointGhostLLQuotient period hPeriod configuration data analysis) :
    inner Real
      (jointGhostLLRiesz period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter
        physical hZero hMetric hWeights
        (quotientDiffeomorphismInclusion period hPeriod configuration data analysis vector)) test =
    inner Real (reducedDiffeomorphismSignedRiesz period hPeriod configuration data hMetric hWeights vector)
      (quotientDiffeomorphismReadout period hPeriod configuration data analysis test) +
    inner Real (quotientPhysicalRiesz period hPeriod configuration data analysis realization
      plusBase minusBase hBase hCenter physical
      (quotientDiffeomorphismInclusion period hPeriod configuration data analysis vector)) test := by
  rw [quotientDiffeomorphism_augmented_column, inner_add_left, quotientDiffeomorphismInclusion_inner]

/-- The existing Abelian column uses this very same descended physical operator. -/
theorem quotientAbelianPhysicalColumn_eq_quotientPhysicalRiesz
    ...
    quotientPhysicalRiesz period hPeriod configuration data analysis realization plusBase minusBase hBase hCenter physical
      (quotientAbelianInclusion period hPeriod configuration data analysis vector) := rfl

end
end
end P0EFTJanusProgramPT12JointQuotientDiffeomorphismColumn4D
end JanusFormal
